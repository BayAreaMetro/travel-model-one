...
# Simplified mode groups (same as tour_mode but for trip_mode)
...
def _group_modes(df: pl.DataFrame) -> pl.DataFrame:
    """Aggregate trip_mode into simplified mode groups."""
...
def _mode_share_table(
    ...
    obs_totals = obs_g.group_by("simple_purpose").agg(pl.col("num_trips").sum().alias("total"))
    mod_totals = mod_g.group_by("simple_purpose").agg(pl.col("num_trips").sum().alias("total"))
