...
Five pieces are ported, matching the legacy call order:
...
``config_host_ip``, ``config_distribution`` and ``config_shadowprice`` are
already native in :mod:`tm1.steps.simulate_ctramp` (``patch_properties`` +
JPPF patching), which owns those keys; ``config_logsums`` is deferred with
logsums.  This step and ``simulate_ctramp`` therefore patch *disjoint* key sets
in the same ``mtcTourBased.properties``.
