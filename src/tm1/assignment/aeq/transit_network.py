...
def bus_time_table(links: pd.DataFrame, period: str, bus_cfg) -> dict:
    ...
    ``bus_cfg`` is :class:`tm1.assignment.params.BusTime` (params.yaml).
    """
    per = period.upper()
...
def build_ride_links(
    ...
    *,
    ...
    bus_cfg
        :class:`tm1.assignment.params.BusTime` (missing-link time fallbacks).
    freq_field
        ``period -> TRNBUILD headway attribute`` (params.yaml periods.freq_field).
