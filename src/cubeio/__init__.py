...
- :func:`read_net` - NET highway nodes and links -> Python (:mod:`cubeio.net_read`)
- :func:`read_net_nodes` - NET highway nodes -> Python
- :func:`read_net_links` - NET highway links -> Python
- :func:`iter_net_links` - stream NET highway links
...
The NET reader's row, parameter, and result types (``Node``, ``Link``,
``NetResult``, ...) are re-exported for annotating callers.

Internal modules import each other by submodule (e.g. ``cubeio.tpp_read``);
external callers should use this package-level API. Importing the package
imports numpy through the TPP modules; :mod:`cubeio.net_read` itself needs
...
def find_latest_iter(directory: str | Path, stem: str, suffix: str = ".tpp") -> Path | None:
