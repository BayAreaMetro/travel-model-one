...
ext_zone_prod = total_trips[total_trips['origin']>6593].groupby(['origin']).agg(PROD=('total', 'sum')).reset_index()
ext_zone_attr = total_trips[total_trips['destination']>6593].groupby(['destination']).agg(ATTR=('total', 'sum')).reset_index()
...
iz_ez_map=pd.read_table('../../../nonres/IZ_OZ_PCT_EXT.dat', names=['iz','ez','col_pct','row_pct'])
