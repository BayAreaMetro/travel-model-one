...
add_distance <- function(this_timeperiod, tours) {
  ...
  # Set the time code
  distSkims <- mutate(distSkims, timeCode = this_timeperiod)

  # Left join tours to the skims
  tours <- left_join(tours, distSkims, by=c("orig_taz","dest_taz","timeCode"))
  
  # assign distance value if we can to new column distance2
  tours <- mutate(tours,
  ...
  tours <- mutate(tours, distance2 = ifelse(distance2 == -999, 0, distance2))
...
```{r VehicleMilesTraveled_Climate, error=FALSE}
mandatory_locations <- tbl_df(read.table(file=file.path(TARGET_DIR,"main",
                                                        paste0("wsLocResults_",ITER,".csv")), 
                                         header=TRUE, sep=","))
