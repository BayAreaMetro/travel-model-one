...
loadednet_df = pd.read_csv(
    os.path.join(os.getcwd(), "hwy", "iter3", "avgload5period_vehclasses.csv"),
    index_col=False,
    sep=",",
)
...
loadednet_df["countyName"] = np.select(
    GL_conditions, CountyName_choices, default="null"
)
...
for tp in timeperiodList:
    ...
    loadednet_df[" speedBin"] = np.select(
        Speed_conditions, SpeedBin_choices, default="null"
    )

    # reset speeds to 25 mph for dummy links (these are dummy/centroid connector-access links)
    loadednet_df[" speedBin"] = np.where(
        loadednet_df["ft"] == 6, 6, loadednet_df[" speedBin"]
    ).astype(int)

    # calculate vmt
    loadednet_df["vmt_" + tp] = (
        loadednet_df["distance"] * loadednet_df["vol" + tp + "_tot"]
    )
    ...
    key_name = loadednet1_df.groupby(
        ["countyName", " arbCounty", " speedBin"], as_index=False
    ).sum()
    ...
    if tp == "EA":
        ...
        VMTBySpeedBin_allTP_df = pd.merge(
            VMTBySpeedBin_allTP_df,
            key_name,
            left_on=["countyName", " arbCounty", " speedBin"],
            right_on=["countyName", " arbCounty", " speedBin"],
            how="outer",
        )
...
VMTBySpeedBin_allTP_df["hour07"] = (
    0.164 * VMTBySpeedBin_allTP_df["vmt_AM"]
)  # note missing space
...
VMTBySpeedBin_allTP_df["hour17"] = (
    0.261 * VMTBySpeedBin_allTP_df["vmt_PM"]
)  # note missing space
...
for cnty in CountyList:
    ...
    if cnty == "San Francisco":
        ...
        dummy_df = dummy_df.append(speedBin_dummy_df)


VMTBySpeedBin_allTP_df = pd.merge(
    VMTBySpeedBin_allTP_df,
    dummy_df,
    left_on=["countyName", " speedBin"],
    right_on=["county_dummy", "dummy_col"],
    how="outer",
)
...
VMTBySpeedBin_allTP_df = VMTBySpeedBin_allTP_df[
    [
        "countyName",
        " arbCounty",
        " speedBin",
