...
def updateLinesOfInterest(
    timeperiod, trnAssignIter, complexAccessModes, currentTad, currentNet
):
    """
    ...
    if os.path.exists(LOIfile):
        ...
        for row in reader:
            ...
            if len(row) > 0:
                lineStopToRowIdx[row[0] + row[1]] = len(linesOfInterest) - 1
    else:
    ...
    for line in currentNet.line(re.compile(comp_modes_re)):
        ...
        if line.getFreq(timeperiod, Network.MODEL_TYPE_TM1) == 0.0:
            continue
        ...
        for nodeIdx in range(len(line.n) - 1):
            ...
            if newFile:
                linesOfInterest.append(
                    [line.name, node.num, node.description(), vol, boardsDisallowed]
                )
            elif line.name:
...
def updatePHT(timeperiod, trnAssignIter, maxTrnAssignIter, PHTDiffCond):
    ...
    for phtFile in phtFiles:
        ...
        for (
            ...
            if Mode in Modes[phtIteration][phtTimeperiod]:
                continue
            ...
            if phtTimeperiod == timeperiod and phtIteration == trnAssignIter:
                ...
                allPHTFile.write(
                    ...
                    % (
                        ...
                        PathFromBothStr,
                        PathFromIterOnlyStr,
                        PathFromAvgOnlyStr,  # ,PhTCriteriaMet
                    )
                )
    ...
    if trnAssignIter == 0:
        ...
        if trnAssignIter == maxTrnAssignIter:
            criteriaMet = True
    ...
    # otherwise -- Check convergence
        ...
        if trnAssignIter == maxTrnAssignIter or (
            PHTDiffCond > 0 and abs(pctdiffPHT) <= PHTDiffCond
        ):
            criteriaMet = True

        # Put the total in the PHT_total.csv as well
        allPHTFile.write(
            "%d,%s,%s,%f,%f,%f,%f,%d,%d,%f,%d,%d,%d,%d\n"
            % (
                trnAssignIter,
                timeperiod,
                "Total",
...
def checkMSAcriteriaMet(timeperiod, trnAssignIter):
    ...
    for (
        ...
        if timeperiodStr == timeperiod and int(trnAssignIterStr) == trnAssignIter:
            ...
            print(
                "RouteLinkMSALog criteria {} met for {} iteration {}".format(
    ...
    routelinkFile.close()
    print(
        "Line not found for {} iteration {} in RouteLinkMSALog.csv!".format(
            timeperiod, trnAssignIter
        )
    )
    raise
    ...
    for line in net:
        for nodeIdx in range(len(line.n)):
            if (
                ...
                timeperiod,
            ) in extraDelayMapping:
                curDelay = 0.0
                if "DELAY" in line.n[nodeIdx].attr:
                    curDelay = float(line.n[nodeIdx].attr["DELAY"])
                line.n[nodeIdx].attr["DELAY"] = "%.3f" % (
                    curDelay
                    + extraDelayMapping[
                        (line.name, abs(int(line.n[nodeIdx].num)), timeperiod)
                    ]
                )
...
if __name__ == "__main__":
    ...
    if runmode == "Simple":
        ...
        setupLogging(
            infoLogFilename=None,
            debugLogFilename="transitDwellAccess_SimpleLog.txt",
            logToConsole=False,
        )
        ...
        for timeperiod in ["AM", "MD", "PM", "EV", "EA"]:
            ...
            try:
                ...
                for lineidx in range(len(net.lines) - 1, -1, -1):
                    if not isinstance(net.lines[lineidx], TransitLine):
                        continue
                    frequency = net.lines[lineidx].getFreqs()
                    freqsum = sum([float(i) for i in frequency])
                    if freqsum == 0:
                        WranglerLogger.info(
                            "Line %s has no frequencies: %s -- deleting"
                            % (
                                net.lines[lineidx].name,
                                str(net.lines[lineidx].getFreqs()),
                            )
                        )
                ...
                net.addDelay(
                    timeperiod="Simple",
                    additionalLinkFile="transitLines.link",
                ...
                net.write(
                    name="transitOriginal" + timeperiod,
            ...
            if not net.checkCapacityConfiguration(
                complexDwellModes, complexAccessModes
            ):
                exit(2)
            ...
            for line in net:
                ...
                for stop in line:
                    if prevStop:
                        AB_to_trnvehvol[(prevStop, abs(stop))] += trnvehvol
                    prevStop = abs(stop)

            outTable = DataTable(
                numRecords=len(AB_to_trnvehvol),
                header=(
                    FieldType("A", "N", 7, 0),
                    FieldType("B", "N", 7, 0),
                    FieldType("AB", "C", 15, 0),
            ...
            WranglerLogger.info(
                "Wrote %s with %d rows" % (outfile, len(AB_to_trnvehvol))
            )
    ...
    setupLogging(
        infoLogFilename=None,
        debugLogFilename="transitDwellAccess_%s.%d.log" % (timeperiod, trnAssignIter),
        logToConsole=False,
    )
    ...
    if PHTDiffCond == 0:
        criteriaMet = (trnAssignIter >= maxTrnAssignIter) or checkMSAcriteriaMet(
            timeperiod, trnAssignIter
        )

    # MINIMUM iterations = 4
    if (
        len(complexDwellModes) > 0 or len(complexAccessModes) > 0
    ) and trnAssignIter < 4:
        criteriaMet = False
    ...
    if PHTDiffCond == 0:
        curRouteFileName = os.path.join(
            curSubdir, "trnlink{}_ALLMSA.dbf".format(timeperiod)
        )
    else:
    ...
    if criteriaMet:
        WranglerLogger.info(
            "Criteria met for %s in transit assignment iteration %d"
            % (timeperiod, trnAssignIter)
        )
        exit(0)
