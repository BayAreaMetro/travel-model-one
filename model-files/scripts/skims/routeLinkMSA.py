...
setupLogging(
    ...
    debugLogFilename="routeLinkMSA_%s.%d.log" % (timeperiod, trnAssignIter),
    logToConsole=False,
)
...
if trnAssignIter == 0:
    ...
    prevRouteFileName = os.path.join(
        prevSubdir, "trnlink{}_ALLMSA.dbf".format(timeperiod)
    )
    # Read the MSA assignment from the previous iteration
    prevTad = TransitAssignmentData(
        timeperiod=timeperiod,
        modelType=Network.MODEL_TYPE_TM1,
        ignoreModes=[1, 2, 3, 4, 5, 6, 7],
        tpfactor="constant_with_peaked_muni",
...
for row in curTad.trnAsgnTable:
    ...
    if prevTad:
        try:
            row["AB_VOL"] = (LAMBDA * row["AB_VOL"]) + (
                (1.0 - LAMBDA) * prevTad.trnAsgnTable[key]["AB_VOL"]
            )
            row["AB_BRDA"] = (LAMBDA * row["AB_BRDA"]) + (
                (1.0 - LAMBDA) * prevTad.trnAsgnTable[key]["AB_BRDA"]
            )
            row["AB_XITA"] = (LAMBDA * row["AB_XITA"]) + (
                (1.0 - LAMBDA) * prevTad.trnAsgnTable[key]["AB_XITA"]
            )
            row["AB_BRDB"] = (LAMBDA * row["AB_BRDB"]) + (
                (1.0 - LAMBDA) * prevTad.trnAsgnTable[key]["AB_BRDB"]
            )
            row["AB_XITB"] = (LAMBDA * row["AB_XITB"]) + (
                (1.0 - LAMBDA) * prevTad.trnAsgnTable[key]["AB_XITB"]
            )

            row["BA_VOL"] = (LAMBDA * row["BA_VOL"]) + (
                (1.0 - LAMBDA) * prevTad.trnAsgnTable[key]["BA_VOL"]
            )
            row["BA_BRDA"] = (LAMBDA * row["BA_BRDA"]) + (
                (1.0 - LAMBDA) * prevTad.trnAsgnTable[key]["BA_BRDA"]
            )
            row["BA_XITA"] = (LAMBDA * row["BA_XITA"]) + (
                (1.0 - LAMBDA) * prevTad.trnAsgnTable[key]["BA_XITA"]
            )
            row["BA_BRDB"] = (LAMBDA * row["BA_BRDB"]) + (
                (1.0 - LAMBDA) * prevTad.trnAsgnTable[key]["BA_BRDB"]
            )
            row["BA_XITB"] = (LAMBDA * row["BA_XITB"]) + (
                (1.0 - LAMBDA) * prevTad.trnAsgnTable[key]["BA_XITB"]
            )
