...
def replace_in_file(filepath, regex_dict):
    ...
    for pattern, newstr in regex_dict.items():
        (file_contents, numsubs) = re.subn(
            pattern, newstr, file_contents, flags=re.IGNORECASE
        )
...
def config_popsyn_files(replacements):
    ...
    filepath = os.path.join("CTRAMP", "runtime", "mtcTourBased.properties")
    replacements[filepath][
        "(\nPopulationSynthesizer.InputToCTRAMP.HouseholdFile[ \t]*=[ \t]*)(\S*)"
    ] = (r"\g<1>%s" % hhfile)
    replacements[filepath][
        "(\nPopulationSynthesizer.InputToCTRAMP.PersonFile[ \t]*=[ \t]*)(\S*)"
    ] = (r"\g<1>%s" % perfile)
...
def config_mobility_params(params_filename, params_contents, for_logsums, replacements):
    ...
    BikeInfra_CIVT_Mult = float(
        get_property(params_filename, params_contents, "Bike_Infra_C_IVT_Multiplier")
    )

    SharingPrefFactor = float(
        get_property(params_filename, params_contents, "Sharing_Preferences_factor")
    )

    MeansBasedTollsQ1Factor = float(
        get_property(params_filename, params_contents, "Means_Based_Tolling_Q1Factor")
    )
    MeansBasedTollsQ2Factor = float(
        get_property(params_filename, params_contents, "Means_Based_Tolling_Q2Factor")
    )
    MeansBasedFareQ1Factor = float(
        get_property(params_filename, params_contents, "Means_Based_Fare_Q1Factor")
    )
    MeansBasedFareQ2Factor = float(
        get_property(params_filename, params_contents, "Means_Based_Fare_Q2Factor")
    )

    Adjust_TNCsingle_TourMode = float(
        get_property(params_filename, params_contents, "Adjust_TNCsingle_TourMode")
    )
    Adjust_TNCshared_TourMode = float(
        get_property(params_filename, params_contents, "Adjust_TNCshared_TourMode")
    )
    Adjust_TNCsingle_TripMode = float(
        get_property(params_filename, params_contents, "Adjust_TNCsingle_TripMode")
    )
    Adjust_TNCshared_TripMode = float(
        get_property(params_filename, params_contents, "Adjust_TNCshared_TripMode")
    )

    avShare = float(get_property(params_filename, params_contents, "Mobility.AV.Share"))
    pBoostAutosLTDrivers = float(
        get_property(
            params_filename,
            params_contents,
    ...
    avIVTFactor = float(
        get_property(params_filename, params_contents, "Mobility.AV.IVTFactor")
    )
    avparkCostFactor = float(
        get_property(params_filename, params_contents, "Mobility.AV.ParkingCostFactor")
    )
    avCPMFactor = float(
        get_property(params_filename, params_contents, "Mobility.AV.CostPerMileFactor")
    )
    avTermTimeFactor = float(
        get_property(params_filename, params_contents, "Mobility.AV.TerminalTimeFactor")
    )
    tncIVTFactor = float(
        get_property(params_filename, params_contents, "Mobility.TNC.shared.IVTFactor")
    )

    taxiBaseFare = float(
        get_property(params_filename, params_contents, "taxi.baseFare")
    )
    taxiCPMile = float(
        get_property(params_filename, params_contents, "taxi.costPerMile")
    )
    taxiCPMin = float(
        get_property(params_filename, params_contents, "taxi.costPerMinute")
    )

    tncSingleBaseFare = float(
        get_property(params_filename, params_contents, "TNC.single.baseFare")
    )
    tncSingleCPMile = float(
        get_property(params_filename, params_contents, "TNC.single.costPerMile")
    )
    tncSingleCPMin = float(
        get_property(params_filename, params_contents, "TNC.single.costPerMinute")
    )
    tncSingleMinCost = float(
        get_property(params_filename, params_contents, "TNC.single.costMinimum")
    )

    tncSharedBaseFare = float(
        get_property(params_filename, params_contents, "TNC.shared.baseFare")
    )
    tncSharedCPMile = float(
        get_property(params_filename, params_contents, "TNC.shared.costPerMile")
    )
    tncSharedCPMin = float(
        get_property(params_filename, params_contents, "TNC.shared.costPerMinute")
    )
    tncSharedMinCost = float(
        get_property(params_filename, params_contents, "TNC.shared.costMinimum")
    )

    tncSingleMeanWaitTime = get_property(
        params_filename, params_contents, "TNC.single.waitTime.mean"
    )
    tncSingleSDWaitTime = get_property(
        params_filename, params_contents, "TNC.single.waitTime.sd"
    )

    tncSharedMeanWaitTime = get_property(
        params_filename, params_contents, "TNC.shared.waitTime.mean"
    )
    tncSharedSDWaitTime = get_property(
        params_filename, params_contents, "TNC.shared.waitTime.sd"
    )

    taxiMeanWaitTime = get_property(
        params_filename, params_contents, "Taxi.waitTime.mean"
    )
    ...
    tncSingleDaShare = float(
        get_property(params_filename, params_contents, "TNC.single.da.share")
    )
    tncSingleS2Share = float(
        get_property(params_filename, params_contents, "TNC.single.s2.share")
    )
    tncSingleS3Share = float(
        get_property(params_filename, params_contents, "TNC.single.s3.share")
    )

    tncSharedDaShare = float(
        get_property(params_filename, params_contents, "TNC.shared.da.share")
    )
    tncSharedS2Share = float(
        get_property(params_filename, params_contents, "TNC.shared.s2.share")
    )
    tncSharedS3Share = float(
        get_property(params_filename, params_contents, "TNC.shared.s3.share")
    )
    ...
    replacements[filepath]["(\nBike_Infra_C_IVT_Multiplier[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % BikeInfra_CIVT_Mult
    )

    replacements[filepath]["(\nSharing_Preferences_factor[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % SharingPrefFactor
    )

    replacements[filepath]["(\nMeans_Based_Tolling_Q1Factor[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % MeansBasedTollsQ1Factor
    )
    replacements[filepath]["(\nMeans_Based_Tolling_Q2Factor[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % MeansBasedTollsQ2Factor
    )
    replacements[filepath]["(\nMeans_Based_Fare_Q1Factor[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % MeansBasedFareQ1Factor
    )
    replacements[filepath]["(\nMeans_Based_Fare_Q2Factor[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % MeansBasedFareQ2Factor
    )

    replacements[filepath]["(\nAdjust_TNCsingle_TourMode[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % Adjust_TNCsingle_TourMode
    )
    replacements[filepath]["(\nAdjust_TNCshared_TourMode[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % Adjust_TNCshared_TourMode
    )
    replacements[filepath]["(\nAdjust_TNCsingle_TripMode[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % Adjust_TNCsingle_TripMode
    )
    replacements[filepath]["(\nAdjust_TNCshared_TripMode[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % Adjust_TNCshared_TripMode
    )
    ...
    replacements[filepath][
        "(\nMobility.AV.ProbabilityBoost.AutosLTDrivers[ \t]*=[ \t]*)(\S*)"
    ] = (r"\g<1>%.2f" % pBoostAutosLTDrivers)
    replacements[filepath][
        "(\nMobility.AV.ProbabilityBoost.AutosGEDrivers[ \t]*=[ \t]*)(\S*)"
    ] = (r"\g<1>%.2f" % pBoostAutosGEDrivers)
    replacements[filepath]["(\nMobility.AV.IVTFactor[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % avIVTFactor
    )
    replacements[filepath]["(\nMobility.AV.ParkingCostFactor[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % avparkCostFactor
    )
    replacements[filepath]["(\nMobility.AV.CostPerMileFactor[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % avCPMFactor
    )
    replacements[filepath]["(\nMobility.AV.TerminalTimeFactor[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % avTermTimeFactor
    )
    replacements[filepath]["(\nMobility.TNC.shared.IVTFactor[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % tncIVTFactor
    )

    replacements[filepath]["(\ntaxi.baseFare[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % taxiBaseFare
    )
    replacements[filepath]["(\ntaxi.costPerMile[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % taxiCPMile
    )
    replacements[filepath]["(\ntaxi.costPerMinute[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % taxiCPMin
    )

    replacements[filepath]["(\nTNC.single.baseFare[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % tncSingleBaseFare
    )
    replacements[filepath]["(\nTNC.single.costPerMile[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % tncSingleCPMile
    )
    replacements[filepath]["(\nTNC.single.costPerMinute[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % tncSingleCPMin
    )
    replacements[filepath]["(\nTNC.single.costMinimum[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % tncSingleMinCost
    )

    replacements[filepath]["(\nTNC.shared.baseFare[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % tncSharedBaseFare
    )
    replacements[filepath]["(\nTNC.shared.costPerMile[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % tncSharedCPMile
    )
    replacements[filepath]["(\nTNC.shared.costPerMinute[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % tncSharedCPMin
    )
    replacements[filepath]["(\nTNC.shared.costMinimum[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % tncSharedMinCost
    )

    replacements[filepath]["(\nTNC.single.waitTime.mean[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%s" % tncSingleMeanWaitTime
    )
    replacements[filepath]["(\nTNC.single.waitTime.sd[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%s" % tncSingleSDWaitTime
    )

    replacements[filepath]["(\nTNC.shared.waitTime.mean[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%s" % tncSharedMeanWaitTime
    )
    replacements[filepath]["(\nTNC.shared.waitTime.sd[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%s" % tncSharedSDWaitTime
    )

    replacements[filepath]["(\nTaxi.waitTime.mean[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%s" % taxiMeanWaitTime
    )
    replacements[filepath]["(\nTaxi.waitTime.sd[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%s" % taxiSDWaitTime
    )
...
def config_auto_opcost(params_filename, params_contents, for_logsums, replacements):
    ...
    av_pcefac_ft01 = float(
        get_property(params_filename, params_contents, "AV_PCE_FAC_FT01")
    )
    av_pcefac_ft02 = float(
        get_property(params_filename, params_contents, "AV_PCE_FAC_FT02")
    )
    av_pcefac_ft03 = float(
        get_property(params_filename, params_contents, "AV_PCE_FAC_FT03")
    )
    av_pcefac_ft04 = float(
        get_property(params_filename, params_contents, "AV_PCE_FAC_FT04")
    )
    av_pcefac_ft05 = float(
        get_property(params_filename, params_contents, "AV_PCE_FAC_FT05")
    )
    av_pcefac_ft06 = float(
        get_property(params_filename, params_contents, "AV_PCE_FAC_FT06")
    )
    av_pcefac_ft07 = float(
        get_property(params_filename, params_contents, "AV_PCE_FAC_FT07")
    )
    av_pcefac_ft08 = float(
        get_property(params_filename, params_contents, "AV_PCE_FAC_FT08")
    )
    av_pcefac_ft09 = float(
        get_property(params_filename, params_contents, "AV_PCE_FAC_FT09")
    )
    av_pcefac_ft10 = float(
        get_property(params_filename, params_contents, "AV_PCE_FAC_FT10")
    )

    OwnedAV_zpv = float(
        get_property(params_filename, params_contents, "OwnedAV_ZPV_fac")
    )
    ...
    HSRInterregionalDisable = int(
        get_property(params_filename, params_contents, "HSR_Interregional_Disable")
    )

    # put the av pce factors into the CTRAMP\scripts\block\hwyParam.block
    filepath = os.path.join("CTRAMP", "scripts", "block", "hwyParam.block")
    replacements[filepath]["(\nAV_PCE_FT01[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % av_pcefac_ft01
    )
    replacements[filepath]["(\nAV_PCE_FT02[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % av_pcefac_ft02
    )
    replacements[filepath]["(\nAV_PCE_FT03[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % av_pcefac_ft03
    )
    replacements[filepath]["(\nAV_PCE_FT04[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % av_pcefac_ft04
    )
    replacements[filepath]["(\nAV_PCE_FT05[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % av_pcefac_ft05
    )
    replacements[filepath]["(\nAV_PCE_FT06[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % av_pcefac_ft06
    )
    replacements[filepath]["(\nAV_PCE_FT07[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % av_pcefac_ft07
    )
    replacements[filepath]["(\nAV_PCE_FT08[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % av_pcefac_ft08
    )
    replacements[filepath]["(\nAV_PCE_FT09[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % av_pcefac_ft09
    )
    replacements[filepath]["(\nAV_PCE_FT10[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % av_pcefac_ft10
    )

    replacements[filepath]["(\nOwnedAV_ZPV_factor[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % OwnedAV_zpv
    )
    replacements[filepath]["(\nTNC_ZPV_factor[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%.2f" % TNC_zpv
    )
    ...
    replacements[filepath]["(\nHSR_Interregional_Disable[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%d" % HSRInterregionalDisable
    )
...
def config_logsums(replacements, append):
    ...
    replacements[filepath][
        "(\nPopulationSynthesizer.InputToCTRAMP.HouseholdFile[ \t]*=[ \t]*)(\S*)"
    ] = (r"\g<1>%s" % "logsums/accessibilities_dummy_households.csv")
    replacements[filepath][
        "(\nPopulationSynthesizer.InputToCTRAMP.PersonFile[ \t]*=[ \t]*)(\S*)"
    ] = (r"\g<1>%s" % "logsums/accessibilities_dummy_persons.csv")

    # turn off some submodels
    replacements[filepath][
        "(\nRunModel.AutoOwnership[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>false"
    replacements[filepath]["(\nRunModel.FreeParking[ \t]*=[ \t]*)(\S*)"] = r"\g<1>false"
    replacements[filepath][
        "(\nRunModel.CoordinatedDailyActivityPattern[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>false"
    replacements[filepath][
        "(\nRunModel.IndividualMandatoryTourFrequency[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>false"
    replacements[filepath][
        "(\nRunModel.MandatoryTourDepartureTimeAndDuration[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>false"
    replacements[filepath][
        "(\nRunModel.MandatoryTourModeChoice[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>false"
    replacements[filepath][
        "(\nRunModel.JointTourFrequency[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>false"
    replacements[filepath][
        "(\nRunModel.JointTourLocationChoice[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>false"
    replacements[filepath][
        "(\nRunModel.JointTourDepartureTimeAndDuration[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>false"
    replacements[filepath][
        "(\nRunModel.JointTourModeChoice[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>false"
    replacements[filepath][
        "(\nRunModel.IndividualNonMandatoryTourFrequency[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>false"
    replacements[filepath][
        "(\nRunModel.IndividualNonMandatoryTourDepartureTimeAndDuration[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>false"
    replacements[filepath][
        "(\nRunModel.IndividualNonMandatoryTourModeChoice[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>false"
    replacements[filepath][
        "(\nRunModel.AtWorkSubTourFrequency[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>false"
    replacements[filepath][
        "(\nRunModel.AtWorkSubTourDepartureTimeAndDuration[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>false"
    replacements[filepath][
        "(\nRunModel.AtWorkSubTourModeChoice[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>false"
    replacements[filepath][
        "(\nRunModel.StopFrequency[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>false"
    replacements[filepath][
        "(\nRunModel.StopLocation[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>false"

    # sample size
    replacements[filepath][
        "(\nUsualWorkAndSchoolLocationChoice.SampleOfAlternatives.SampleSize[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>4362"
    replacements[filepath][
        "(\nJointTourLocationChoice.SampleOfAlternatives.SampleSize[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>30"
    replacements[filepath][
        "(\nIndividualNonMandatoryTourLocationChoice.SampleOfAlternatives.SampleSize[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>4362"
    replacements[filepath][
        "(\nAtWorkSubtourLocationChoice.SampleOfAlternatives.SampleSize[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>30"

    # use logsums from base run
    replacements[filepath][
        "(\nUsualWorkAndSchoolLocationChoice.ShadowPrice.Input.File[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>logsums/ShadowPricing_7.csv"
    replacements[filepath][
        "(\nUsualWorkAndSchoolLocationChoice.ShadowPricing.MaximumIterations[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>1"
    replacements[filepath][
        "(\nUsualWorkAndSchoolLocationChoice.ShadowPricing.OutputFile[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>logsums/ShadowPricing.csv"
    replacements[filepath][
        "(\nUsualWorkAndSchoolLocationChoice.SizeTerms.OutputFile[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>logsums/DestinationChoiceSizeTerms.csv"

    replacements[filepath][
        "(\nResults.HouseholdDataFile[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>logsums/householdData.csv"
    replacements[filepath][
        "(\nResults.PersonDataFile[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>logsums/personData.csv"
    replacements[filepath][
        "(\nResults.IndivTourDataFile[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>logsums/indivTourData.csv"
    replacements[filepath][
        "(\nResults.JointTourDataFile[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>logsums/jointTourData.csv"
    replacements[filepath][
        "(\nResults.IndivTripDataFile[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>logsums/indivTripData.csv"
    replacements[filepath][
        "(\nResults.JointTripDataFile[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>logsums/jointTripData.csv"
    replacements[filepath][
        "(\nResults.WriteDataToDatabase[ \t]*=[ \t]*)(\S*)"
    ] = r"\g<1>false"
...
def config_host_ip(for_logsums, replacements):
    ...
    if host_ip_address not in ips_here:
        print(
            "FATAL: HOST_IP_ADDRESS {} does not match the IP addresses for this machine {}".format(
                host_ip_address, str(ips_here)
            )
        )
        sys.exit(2)
    ...
    for nodenum in range(5):
        filenames.append("JavaOnly_runNode%d.cmd" % nodenum)
    ...
    for nodenum in range(5):
        filenames.append("jppf-node%d.properties" % nodenum)
    for filename in filenames:
        filepath = os.path.join("CTRAMP", "runtime", "config", filename)
        replacements[filepath]["(jppf.server.host[ \t]*=[ \t]*)(\S*)"] = (
            r"\g<1>%s" % host_ip_address
        )
    ...
    replacements[filepath]["(jppf.management.host[ \t]*=[ \t]*)(\S*)"] = (
        r"\g<1>%s" % host_ip_address
    )
    ...
    if for_logsums:
        ...
        filepath = os.path.join("CTRAMP", "runtime", "logsums.properties")
        replacements[filepath][
            "(\nRunModel.HouseholdServerAddress[ \t]*=[ \t]*)(\S*)"
        ] = (r"\g<1>%s" % host_ip_address)
        replacements[filepath]["(\nRunModel.MatrixServerAddress[ \t]*=[ \t]*)(\S*)"] = (
            r"\g<1>%s" % host_ip_address
        )
...
def config_distribution(replacements):
    ...
    if hostname == "mainmodel":
        ...
        shutil.copy2(
            os.path.join("CTRAMP", "scripts", "block", "HwyIntraStep_64.block"),
        ...
        shutil.copy2(
            os.path.join("CTRAMP", "scripts", "block", "HwyIntraStep_48.block"),
            os.path.join("CTRAMP", "scripts", "block", "HwyIntraStep.block"),
        )

    else:
        raise Exception(
            "RuntimeConfiguration.py does not recognize hostname [%s] for distribution configuration"
            % hostname
        )
...
def config_uec(auto_operating_cost):
    ...
    for bookname in [
        ...
        rb = xlrd.open_workbook(
            "%s.original" % filepath, formatting_info=True, on_demand=True
        )
        ...
        for sheet_num in range(rb.nsheets):
            ...
            for rownum in range(rs.nrows):
                ...
                if rs.cell(rownum, 1).value == "costPerMile":
                    print(
                        "  Sheet '{}': replacing costPerMile '{}' -> {:.2f}".format(
                            rs.name, rs.cell(rownum, 4).value, auto_op_cost_float
                        )
                    )
                    wb.get_sheet(sheet_num).write(
                        rownum, 4, auto_op_cost_float, xlwt.easyxf("align: horiz left")
                    )
                # print(rs.cell(rownum,1))
...
def config_cdap(params_filename, params_contents):
    ...
    TelecommuteConstant_FT = float(
        get_property(params_filename, params_contents, "Telecommute_constant_FT")
    )
    TelecommuteConstant_PT = float(
        get_property(params_filename, params_contents, "Telecommute_constant_PT")
    )
    ...
    for bookname in ["CoordinatedDailyActivityPattern.xls"]:
        ...
        for sheet_num in range(rb.nsheets):
            ...
            for rownum in range(rs.nrows):
                # print(rs.cell(rownum,1))
                if (
                    rs.cell(rownum, 2).value
                    == "Simulate telecommuting by reducing mandatory patterns - global_FT"
                ):
                    print(
                        "  Sheet '{}': replacing telecommute constant '{}' -> {:.2f}".format(
                    ...
                    wb.get_sheet(sheet_num).write(
                        rownum,
                        6,
                        TelecommuteConstant_FT,
                        xlwt.easyxf("align: horiz right"),
                    )
                if (
                    rs.cell(rownum, 2).value
                    == "Simulate telecommuting by reducing mandatory patterns - global_PT"
                ):
                    print(
                        "  Sheet '{}': replacing telecommute constant '{}' -> {:.2f}".format(
...
def config_freeparking(params_filename, params_contents):
    ...
    # read the telecommute constant from the properties file
    Free_Parking_Eligibility_OnOff = float(
        get_property(params_filename, params_contents, "Free_Parking_Eligibility_OnOff")
    )
        ...
        for sheet_num in range(rb.nsheets):
            ...
            for rownum in range(rs.nrows):
                ...
                if rs.cell(rownum, 2).value == "Free parking eligibility OnOff dummy":
                    print(
                        "  Sheet '{}': replacing Free parking eligibility OnOff dummy '{}' -> {:.4f}".format(
                            rs.name,
                            rs.cell(rownum, 6).value,
                            Free_Parking_Eligibility_OnOff,
                        )
                    )
                    wb.get_sheet(sheet_num).write(
                        rownum,
...
if __name__ == "__main__":
    ...
    if my_args.logsums:
        ...
        shutil.copyfile(
            os.path.join("CTRAMP", "runtime", "mtcTourBased.properties"),
