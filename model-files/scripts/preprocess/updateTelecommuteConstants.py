...
if __name__ == "__main__":
    ...
    if int(TELECOMMUTE_CALIBRATION) == 1:
        ...
        if MODEL_DIR.upper().find("FBP") >= 0:
            if (
                (int(MODEL_YEAR) < 2035)
                or (MODEL_DIR.upper().find("NOPROJECT") >= 0)
                or (MODEL_DIR.upper().find("NOTRANSPORTPROJECT") >= 0)
            ):
                UPDATE_CONSTANT = False
            else:
                UPDATE_CONSTANT = True
    ...
    tazdata_df = pandas.read_csv(
        TAZDATA_FILE, index_col=False, sep=",", usecols=TAZDATA_COLS
    )
    print(
        "Read {} lines from {}; head:\n{}".format(
            len(tazdata_df), TAZDATA_FILE, tazdata_df.head()
        )
    )
    ...
    print(
        "Read {} lines from {}; head:\n{}".format(
            len(telecommute_rate_df), TELERATE_FILE, telecommute_rate_df.head()
        )
    )

    # join to tazdata to figure out effective max telecommute rate for each county
    telecommute_rate_df = pandas.merge(
        left=tazdata_df, right=telecommute_rate_df, how="left", on="COUNTY"
    )
    ...
    telecommute_rate_df = (
        telecommute_rate_df.groupby("COUNTY")
        .agg({"max_telecommuters": "sum", "TOTEMP": "sum"})
        .reset_index()
    )
    telecommute_rate_df["max_telecommute_rate"] = (
        telecommute_rate_df["max_telecommuters"] / telecommute_rate_df["TOTEMP"]
    )
    ...
    if int(ITER) == 0:
        if UPDATE_CONSTANT:
            ...
            if not os.path.exists(TELECOMMUTE_CONSTANTS_FILE.format(0)):
                print(
                    "However - the file {} is not found -- core will hang!".format(
                        TELECOMMUTE_CONSTANTS_FILE.format(0)
                    )
                )
                sys.exit(2)
            sys.exit(0)
    ...
    # read results, create metrics and update
        ...
        print(
            "Read {} lines from {}; head:\n{}".format(
                len(tours_df), TOUR_FILE.format(ITER), tours_df.head()
            )
        )
        ...
        print(
            "  Filtered to {} rows of work tours_df; head:\n{}".format(
                len(tours_df), tours_df.head()
            )
        )
        num_work_tours_df = len(tours_df)

        # deduplicate -- some people have multiple work tours_df
        tours_df.drop_duplicates(
            subset=["hh_id", "person_id", "person_num"], keep="first", inplace=True
        )
        print(
            "  Dropped {} second work tours_df".format(
                num_work_tours_df - len(tours_df)
            )
        )

        # read work locations to get people who don't make work tours_df
        wslocs_df = pandas.read_csv(
            WSLOC_FILE.format(ITER),
            usecols=["HHID", "PersonID", "PersonNum", "PersonType", "WorkLocation"],
        )
        ...
        work_tours_df_with_dest = work_tours_df.loc[
            pandas.notnull(work_tours_df.dest_taz),
        ]
        ...
        # fill in sample rate
        work_tours_df.loc[
            pandas.isnull(work_tours_df.sampleRate), "sampleRate"
        ] = float(SAMPLESHARE)
        ...
        work_tours_df.loc[
            (work_tours_df.tour_mode > 0) & (work_tours_df.tour_mode <= 6),
            "simple_mode",
        ] = "auto"
        work_tours_df.loc[
            (work_tours_df.tour_mode >= 19), "simple_mode"
        ] = "auto"  # tnc
        work_tours_df.loc[
            (work_tours_df.tour_mode >= 7) & (work_tours_df.tour_mode <= 18),
            "simple_mode",
        ] = "non-auto"
        ...
        work_tours_df = work_tours_df.loc[
            (work_tours_df["person_type_str"] == "Full-time worker")
            | (work_tours_df["person_type_str"] == "Part-time worker"),
        ]
        # aggregate to work SD
        work_mode_SD_df = (
            work_tours_df.groupby(["person_type_str", "COUNTY", "SD", "simple_mode"])
            .agg({"num_workers": "sum"})
            .reset_index()
        )
        ...
        work_mode_SD_df = pandas.pivot_table(
            work_mode_SD_df,
            index=["COUNTY", "SD"],
            columns=["person_type_str", "simple_mode"],
            values="num_workers",
        ).reset_index()
        # flatten columns
        work_mode_SD_df.columns = [
            " ".join(col).strip() for col in work_mode_SD_df.columns.values
        ]
        ...
        if int(MODEL_YEAR) <= 2020:
            work_mode_SD_df["Full-time worker not-working"] = (
                P_notworking_if_noworktour_FT
                * work_mode_SD_df["Full-time worker no tour"]
            )
            work_mode_SD_df["Part-time worker not-working"] = (
                P_notworking_if_noworktour_PT
                * work_mode_SD_df["Part-time worker no tour"]
            )
        else:
            work_mode_SD_df["Full-time worker not-working"] = (
                P_notworking_FT * work_tours_df["num_workers"]
            )
            work_mode_SD_df["Part-time worker not-working"] = (
                P_notworking_PT * work_tours_df["num_workers"]
            )

        # they cannot exceed no tour
        work_mode_SD_df["Full-time worker not-working"] = work_mode_SD_df[
            ["Full-time worker not-working", "Full-time worker no tour"]
        ].min(
            axis=1
        )  # min across columns
        work_mode_SD_df["Part-time worker not-working"] = work_mode_SD_df[
            ["Part-time worker not-working", "Part-time worker no tour"]
        ].min(
            axis=1
        )  # min across columns
        # remaining is telecommute
        work_mode_SD_df["Full-time worker telecommute"] = (
            work_mode_SD_df["Full-time worker no tour"]
            - work_mode_SD_df["Full-time worker not-working"]
        )
        work_mode_SD_df["Part-time worker telecommute"] = (
            work_mode_SD_df["Part-time worker no tour"]
            - work_mode_SD_df["Part-time worker not-working"]
        )
        # create total workers not taking time off
        work_mode_SD_df["Full-time worker working"] = (
            work_mode_SD_df["Full-time worker auto"]
            + work_mode_SD_df["Full-time worker non-auto"]
            + work_mode_SD_df["Full-time worker telecommute"]
        )
        work_mode_SD_df["Part-time worker working"] = (
            work_mode_SD_df["Part-time worker auto"]
            + work_mode_SD_df["Part-time worker non-auto"]
            + work_mode_SD_df["Part-time worker telecommute"]
        )

        # generic
        work_mode_SD_df["working"] = (
            work_mode_SD_df["Full-time worker working"]
            + work_mode_SD_df["Part-time worker working"]
        )
        work_mode_SD_df["telecommute"] = (
            work_mode_SD_df["Full-time worker telecommute"]
            + work_mode_SD_df["Part-time worker telecommute"]
        )
        work_mode_SD_df["auto"] = (
            work_mode_SD_df["Full-time worker auto"]
            + work_mode_SD_df["Part-time worker auto"]
        )

        # "mode shares" are now a function of people working
        work_mode_SD_df["telecommute_rate"] = (
            work_mode_SD_df["telecommute"] / work_mode_SD_df["working"]
        )
        work_mode_SD_df["auto_share"] = (
            work_mode_SD_df["auto"] / work_mode_SD_df["working"]
        )
        ...
        telecommute_df = pandas.read_csv(
            TELECOMMUTE_CONSTANTS_FILE.format(int(CALIB_ITER) - 1)
        )

        telecommute_df = telecommute_df[["ZONE", "SD", "COUNTY", "telecommuteConstant"]]
        telecommute_df.rename(
            columns={"telecommuteConstant": "telecommuteConstant_prev"}, inplace=True
        )
        ...
        telecommute_df["telecommuteConstant"] = telecommute_df[
            "telecommuteConstant_prev"
        ]

        # telecommute within threshold of max
        telecommute_df["telecomute_diff"] = (
            telecommute_df["max_telecommute_rate"] - telecommute_df["telecommute_rate"]
        )
        telecommute_df["telecomute_near_max"] = (
            telecommute_df["telecomute_diff"].abs() < TELECOMMUTE_RATE_THRESHHOLD
        )
        ...
        if UPDATE_CONSTANT:
            ...
            telecommute_df.loc[
                ...
                & (
                    telecommute_df["telecommute_rate"]
                    < telecommute_df["max_telecommute_rate"]
                )
                & (telecommute_df["telecomute_near_max"] == False),
                "telecommuteConstant",
            ] = (
                telecommute_df["telecommuteConstant"] - CONSTANT_INCREMENT
            )
            ...
            telecommute_df.loc[
                ...
                telecommute_df["telecommuteConstant"] + CONSTANT_DECREMENT
            )

            # don't go positive
            telecommute_df.loc[
                telecommute_df["telecommuteConstant"] > 0, "telecommuteConstant"
            ] = 0

        # drop person_type_str column and other temp cols
        telecommute_df.drop(
            columns=["telecomute_diff", "telecomute_near_max"], inplace=True
        )
        telecommute_df["CALIB_ITER"] = int(CALIB_ITER)

    # write it with calib iter
    telecommute_df.to_csv(
        TELECOMMUTE_CONSTANTS_FILE.format(int(CALIB_ITER)), header=True, index=False
    )
    print(
        "Wrote {} lines to {}".format(
            len(telecommute_df), TELECOMMUTE_CONSTANTS_FILE.format(int(CALIB_ITER))
        )
    )
