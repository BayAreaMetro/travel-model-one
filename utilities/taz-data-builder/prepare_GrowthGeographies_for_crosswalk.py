...
GGnonPPA_INPUT_FBP_FILE = r'M:\Application\PBA50Plus_Data_Processing\Final_Blueprint_Growth_Geographies\raw_interim_data\FBP_Jan2025_GG_noPPA\FBP_Jan2025_GG_noPPA.shp'
TRA_INPUT_FBP_DIR = r'M:\Application\PBA50Plus_Data_Processing\Final_Blueprint_Growth_Geographies\raw_interim_data\FBP_Discrete_Modeling_GG_v2'
TRA_INPUT_FBP_FILE1 = os.path.join(TRA_INPUT_FBP_DIR, 'FBP_Jan2025_TRA1_discrete.shp')
TRA_INPUT_FBP_FILE2 = os.path.join(TRA_INPUT_FBP_DIR, 'FBP_Jan2025_TRA2_discrete.shp')
TRA_INPUT_FBP_FILE3 = os.path.join(TRA_INPUT_FBP_DIR, 'FBP_Jan2025_TRA3_discrete.shp')
TRA_INPUT_FBP_FILE4 = os.path.join(TRA_INPUT_FBP_DIR, 'FBP_Jan2025_TRA4_discrete.shp')
TRA_INPUT_FBP_FILE5 = os.path.join(TRA_INPUT_FBP_DIR, 'FBP_Jan2025_TRA5__discrete.shp')
...
if __name__ == '__main__':
    ...
    # file handler
    LOG_FILE = os.path.join(QAQC_DIR, 'prepare_GrowthGeographies_for_crosswalk_{}.log'.format(my_args.rtp_v))
    ...
    if my_args.rtp_v == 'RTP2021FBP':
        ...
        GG_nonPPA_TRA_SHAPE_FILE = os.path.join(QAQC_DIR, 'gg_nonPPA_tra_pba50plus.shp')  
    ...
    if my_args.rtp_v == 'RTP2021FBP':   # PBA50
        ...
        gg_nonPPA_gdf.to_file(os.path.join(QAQC_DIR, 'gg_nonPPA_pba50plus.shp'))
        ...
        gg_nonPPA_gdf['gg_FBP'] = 'gg_nonPPA'
        logger.info('crs: {}'.format(gg_nonPPA_gdf.crs))
        gg_nonPPA_gdf_proj = gg_nonPPA_gdf.to_crs(analysis_crs)
        
        # dissolve
        gg_nonPPA_gdf = gg_nonPPA_gdf.dissolve(by='gg_FBP').reset_index()
        assert gg_nonPPA_gdf.shape[0] == 1

        # write out for QAQC
        gg_nonPPA_gdf.to_file(os.path.join(QAQC_DIR, 'gg_nonPPA_pba50plusFBP.shp'))
        ...
        tras_gdf['tra_FBP'] = 'tra'
        tra_gdf_dissol = tras_gdf.dissolve(by='tra_FBP').reset_index()
        # write out for QAQC
        logging.info('write out dissolved TRA data for QAQC')
        tra_gdf_dissol.to_file(os.path.join(QAQC_DIR, 'tra_pba50plusFBP.shp'))

        # Growth Geographies and TRAs for travel model parking policies - need GG & TRA intersection, but without PPA in GG
        logging.info('create intersection of GG_nonPPA and TRA')
        gg_nonPPA_tra_gdf = gpd.overlay(gg_nonPPA_gdf, tra_gdf_dissol, how='intersection')   
