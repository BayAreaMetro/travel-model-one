...
SCHOOLAGE_DISTRIBUTION = os.path.join(
    REFERENCE_DIR, "censusSchoolAgeDistributionsByZone2010.csv"
)
COLLEGEAGE_DISTRIBUTION = os.path.join(
    REFERENCE_DIR, "pumsStudentsByAgeDistributionsByZone2010.csv"
)
...
def calculateTerminalTime(baseyear_tazdata_df, tazdata_df):
    ...
    tazdata_df.loc[tazdata_df["developedAcres"] > 0, "areaTypeDensity"] = (
        tazdata_df["TOTPOP"] + 2.5 * tazdata_df["TOTEMP"]
    ) / tazdata_df["developedAcres"]

    # compute the logarithm of employment density
    tazdata_df["employmentDensity"] = 0.0
    tazdata_df.loc[tazdata_df["CIACRE"] > 0, "employmentDensity"] = (
        tazdata_df["TOTEMP"] / tazdata_df["CIACRE"]
    )
    tazdata_df["logEmploymentDensity"] = numpy.log(
        tazdata_df["employmentDensity"] + 1.0
    )
    ...
    tazdata_df["areaTypeDensityForZeroParkingCostZones"] = tazdata_df[
        "areaTypeDensity"
    ] * (1 - tazdata_df["nonZeroParkingCostDummy"])
    tazdata_df["logEmploymentDensityForNonZeroParkingCostZones"] = (
        tazdata_df["logEmploymentDensity"] * tazdata_df["nonZeroParkingCostDummy"]
    )
    tazdata_df["TERMINAL"] = (
        TERMINAL_TIME_MODEL_CONSTANT
        + (
            TERMINAL_TIME_MODEL_K_AREA_TYPE
            * tazdata_df["areaTypeDensityForZeroParkingCostZones"]
        )
        + (
            TERMINAL_TIME_MODEL_K_LOG_employmentDensity
            * tazdata_df["logEmploymentDensityForNonZeroParkingCostZones"]
        )
    )
    ...
    tazdata_df.drop(
        columns=[
            ...
            "areaTypeDensity",
            "employmentDensity",
            "logEmploymentDensity",
            "nonZeroParkingCostDummy",
            "areaTypeDensityForZeroParkingCostZones",
            "logEmploymentDensityForNonZeroParkingCostZones",
        ],
        inplace=True,
    )
    return tazdata_df
...
def createSchoolEnrollmentDataSet(baseyear_tazdata_df, tazdata_df):
    ...
    baseyear_tazdata_df = pandas.merge(
        left=baseyear_tazdata_df, right=ageShares, how="left"
    )
    baseyear_tazdata_df = pandas.merge(
        left=baseyear_tazdata_df, right=collegeStudentShares, how="left"
    )

    # compute the number of high school students -- assume X percent of kids this age attend high school, as calculated by Chuck
    tazdata_df["highSchoolStudents"] = (
        tazdata_df["AGE0519"]
        * tazdata_df["14to17In05to19"]
        * HIGH_SCHOOL_ENROLLMENT_MODEL_SHARE_OF_HS_AGE_KIDS_ENROLLED_IN_HS
    )
    baseyear_tazdata_df["highSchoolStudents"] = (
        baseyear_tazdata_df["AGE0519"]
        * baseyear_tazdata_df["14to17In05to19"]
        * HIGH_SCHOOL_ENROLLMENT_MODEL_SHARE_OF_HS_AGE_KIDS_ENROLLED_IN_HS
    )

    # compute the number of college students
    tazdata_df["collegeStudents"] = (
        (
            tazdata_df["AGE0519"]
            * tazdata_df["18to19In05to19"]
            * tazdata_df["StudentsAge18to19"]
        )
        + (
            tazdata_df["AGE2044"]
            * tazdata_df["20to24In20to44"]
            * tazdata_df["StudentsAge20to24"]
        )
        + (
            tazdata_df["AGE2044"]
            * tazdata_df["25to44In20to44"]
            * tazdata_df["StudentsAge25to44"]
        )
    )
    baseyear_tazdata_df["collegeStudents"] = (
        (
            baseyear_tazdata_df["AGE0519"]
            * baseyear_tazdata_df["18to19In05to19"]
            * baseyear_tazdata_df["StudentsAge18to19"]
        )
        + (
            baseyear_tazdata_df["AGE2044"]
            * baseyear_tazdata_df["20to24In20to44"]
            * baseyear_tazdata_df["StudentsAge20to24"]
        )
        + (
            baseyear_tazdata_df["AGE2044"]
            * baseyear_tazdata_df["25to44In20to44"]
            * baseyear_tazdata_df["StudentsAge25to44"]
        )
    )

    # sum high school and college students by county
    baseyear_students_df = (
        baseyear_tazdata_df[["COUNTY", "highSchoolStudents", "collegeStudents"]]
        .groupby("COUNTY")
        .agg("sum")
    )
    forecast_students_df = (
        tazdata_df[["COUNTY", "highSchoolStudents", "collegeStudents"]]
        .groupby("COUNTY")
        .agg("sum")
    )

    # rename and join baseyear and forecast county tables
    baseyear_students_df.rename(
        columns={
            "highSchoolStudents": "highSchoolStudents_baseyear",
            "collegeStudents": "collegeStudents_baseyear",
        },
        inplace=True,
    )
    forecast_students_df.rename(
        columns={
            "highSchoolStudents": "highSchoolStudents_forecast",
            "collegeStudents": "collegeStudents_forecast",
        },
        inplace=True,
    )
    students_df = pandas.merge(
        left=baseyear_students_df,
        right=forecast_students_df,
        left_index=True,
        right_index=True,
    )

    students_df["highSchoolGrowthRatio"] = (
        students_df["highSchoolStudents_forecast"]
        / students_df["highSchoolStudents_baseyear"]
    )
    students_df["collegeGrowthRatio"] = (
        students_df["collegeStudents_forecast"]
        / students_df["collegeStudents_baseyear"]
    )
    logging.debug("students_df:\n{}".format(students_df))

    # join growth ratios to baseyear_tazdata_df and apply
    baseyear_tazdata_df = pandas.merge(
        left=baseyear_tazdata_df,
        right=students_df[["highSchoolGrowthRatio", "collegeGrowthRatio"]],
        how="left",
        left_on="COUNTY",
        right_index=True,
    )
    baseyear_tazdata_df["HSENROLL_forecast"] = (
        baseyear_tazdata_df["HSENROLL"] * baseyear_tazdata_df["highSchoolGrowthRatio"]
    )
    baseyear_tazdata_df["COLLFTE_forecast"] = (
        baseyear_tazdata_df["COLLFTE"] * baseyear_tazdata_df["collegeGrowthRatio"]
    )
    baseyear_tazdata_df["COLLPTE_forecast"] = (
        baseyear_tazdata_df["COLLPTE"] * baseyear_tazdata_df["collegeGrowthRatio"]
    )

    # join result to tazdata_df
    tazdata_df = pandas.merge(
        left=tazdata_df,
        right=baseyear_tazdata_df[
            ["ZONE", "HSENROLL_forecast", "COLLFTE_forecast", "COLLPTE_forecast"]
        ],
        how="left",
        on="ZONE",
    )
    tazdata_df.rename(
        columns={
            "HSENROLL_forecast": "HSENROLL",
            "COLLFTE_forecast": "COLLFTE",
            "COLLPTE_forecast": "COLLPTE",
        },
        inplace=True,
    )
...
def createParkingCostDataSet(baseyear_tazdata_df, tazdata_df):
    ...
    baseyear_tazdata_df.loc[
        baseyear_tazdata_df["CIACRE"] > 0, "employmentDensity_baseyear"
    ] = (baseyear_tazdata_df["TOTEMP"] / baseyear_tazdata_df["CIACRE"])
    ...
    tazdata_df = pandas.merge(
        left=tazdata_df,
        right=baseyear_tazdata_df[
            ["ZONE", "employmentDensity_baseyear", "PRKCST", "OPRKCST"]
        ],
        how="left",
        on="ZONE",
    )
    ...
    tazdata_df.loc[
        tazdata_df["employmentDensity_baseyear"] > 0, "employmentDensityRatio"
    ] = (tazdata_df["employmentDensity"] / tazdata_df["employmentDensity_baseyear"])
    # do not allow decreasing parking costs
    tazdata_df.loc[
        tazdata_df["employmentDensityRatio"] < 1.0, "employmentDensityRatio"
    ] = 1.0
    ...
    tazdata_df.drop(
        columns=[
            "employmentDensity",
            "employmentDensity_baseyear",
            "employmentDensityRatio",
        ],
        inplace=True,
    )
    return tazdata_df
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "urbansim_taz_summary",
        metavar="urbansim_taz_summary.csv",
        help="UrbanSim output taz summary file",
    )
    ...
    if ("HSENROLL" in taz_cols) and ("COLLFTE" in taz_cols) and ("COLLPTE" in taz_cols):
        logger.info(
            "Skipping createSchoolEnrollmentDataSet() because HSENROLL, COLLFTE, COLLPTE are already present: {}".format(
                taz_cols
            )
        )
    else:
    ...
    for col in baseyear_copy_cols:
        if col in list(tazdata_df.columns.values):
            tazdata_df.drop(columns=[col], inplace=True)

    tazdata_df = pandas.merge(
        left=tazdata_df,
        right=baseyear_tazdata_df[["ZONE"] + baseyear_copy_cols],
        how="left",
        on="ZONE",
    )
