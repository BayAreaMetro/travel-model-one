...
if os.path.exists(trips_csv):
    ...
    convert_cmd = [
        r"C:\\Program Files\\R\\R-3.5.2\\bin\\x64\\RScript.exe",
        "-e",
        "load('{}'); write.table(trips, file='{}', sep=',', row.names=FALSE)".format(
            trips_rdata.replace("\\", "/"), trips_csv.replace("\\", "/")
        ),
    ]
...
trips_by_orig_tripmode_df = (
    trips_df.groupby(["orig_taz", "trip_mode", "tour_purpose", "incQ"])
    .agg(
        {"num_participants": "sum", "distance": "mean", "cost": "mean", "time": "mean"}
    )
    .reset_index()
)

# this has total cost/time/distance for person trips, but the person trips are a sample -- so scale up
trips_by_orig_tripmode_df["num_participants"] = (
    trips_by_orig_tripmode_df["num_participants"] / SAMPLESHARE
)
print(trips_by_orig_tripmode_df.head())

# write the result
trips_by_orig_tripmode_df.to_csv(
    os.path.join(OUTPUT_FOLDER, "trip-travel-costs-by-orig-TAZ.csv"), index=False
)
