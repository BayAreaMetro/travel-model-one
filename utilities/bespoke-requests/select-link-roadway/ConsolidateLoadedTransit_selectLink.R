# Version of ConsolidateLoadedTransit.R for selectLink output.
# (I would normally make the original version more generic rather than duplicating it but we're at a risk-averse juncture.)
#
...
SELECT_LINK      <- gsub("-","_",Sys.getenv("SELECT_LINK"))
...
for (timeperiod in c("ea","am","md","pm","ev")) {
  ...
  for (submode in c("loc","lrf","exp","hvy","com")) {
    for (acc_egr in list( c("wlk","wlk"), c("drv","wlk"), c("wlk","drv"))) {
        # line file
        filename <- paste0("trnline",timeperiod,"_",acc_egr[1],"_",submode,"_",acc_egr[2],"_",SELECT_LINK,".csv")
        ...
        # link file
        filename <- paste0("trnlink",timeperiod,"_",acc_egr[1],"_",submode,"_",acc_egr[2],"_",SELECT_LINK,".csv")
        fullfile <- file.path(MODEL_DIR, "trn", paste0("TransitAssignment.iter",ITER), filename)
...
# write it
outfile <- file.path(MODEL_DIR, "trn", paste0("trnline_",SELECT_LINK,".csv"))
write.csv(all_trnline_data, file=outfile, row.names=FALSE, quote=FALSE)
print(paste("Wrote",outfile))

outfile <- file.path(MODEL_DIR, "trn", paste0("trnlink_",SELECT_LINK,".csv"))
...
for (my_tp in c("ea","am","md","pm","ev")) {
    ...
    outfile <- file.path(MODEL_DIR, "trn", paste0("trnlink", my_tp,"_withSupport_",SELECT_LINK,".dbf"))
    write.dbf(as.data.frame(trndata_tp), file=outfile)
    print(paste("Wrote", outfile))
}

# read the trnselect_[tp]_[link].csv
all_selectlink_data <- data.frame()

for (timeperiod in c("ea","am","md","pm","ev")) {
  # read the input dbfs
  filename <- paste0("trnselect_",timeperiod,"_",SELECT_LINK,".csv")
  fullfile <- file.path(MODEL_DIR, "trn", "TransitAssignment.iter3", filename)
  trndata  <- read.csv(file=fullfile, as.is=TRUE)
  trndata$timeperiod <- timeperiod

  print(paste("Read",nrow(trndata),"from",fullfile))

  # drop these
  trndata <- select(trndata, -one, -two)

  # fill blanks with zero
  cols <- c("wlk_com_wlk","wlk_hvy_wlk","wlk_exp_wlk","wlk_lrf_wlk","wlk_loc_wlk",
            "drv_com_wlk","drv_hvy_wlk","drv_exp_wlk","drv_lrf_wlk","drv_loc_wlk",
            "wlk_com_drv","wlk_hvy_drv","wlk_exp_drv","wlk_lrf_drv","wlk_loc_drv")
  trndata[cols][is.na(trndata[cols])] <- 0

  # filter rows with zeros
  trndata <- filter_at(trndata, cols, any_vars(. != 0))
  print(paste("Filtered to",nrow(trndata),"non zero rows"))

  all_selectlink_data <- rbind(all_selectlink_data, trndata)
}

# write it
outfile <- file.path(MODEL_DIR, "trn", paste0("trnselect_",SELECT_LINK,".csv"))
write.csv(all_selectlink_data, file=outfile, row.names=FALSE, quote=FALSE)
print(paste("Wrote",outfile))