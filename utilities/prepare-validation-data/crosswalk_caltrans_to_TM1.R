...
route_links   <- readOGR(dsn=file.path(CROSSWALK_DIR, "shapefiles"), layer="avgload5period_nonzeroROUTENUM")
...
route_links_df   <- mutate(as.data.frame(route_links),
                           route_dir = paste(ROUTENUM, DIR)) %>% rowid_to_column("link_rowid")
...
xwalk <- select(xwalk, latitude, longitude, countyCode, postmileVa, routeNumbe, direction, postmileSu, ID, A, B, distance) %>%
