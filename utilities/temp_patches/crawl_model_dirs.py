...
def run_command(workingdir, command, script_env):
    ...
    if retcode != 0:
        raise Exception(
            "Received {}; Failed to run command [{}]".format(retcode, command)
        )
    ...
    for machine in MODEL_MACHINES.keys():
        ...
        for project_dir in project_dirs:
            ...
            if index_project_dir not in model_run_dict.keys():
                ...
                (index_project_dir, subs) = re.subn(
                    r"(\d\d\d\d_TM\d\d\d_PPA_[BCR][FGT])_01_(.+)",
                    r"\1_02_\2",
                    project_dir,
                )
                if subs == 0:
                ...
                logging.debug(
                    " Didn't find project_dir {} in keys; trying {}".format(
                        project_dir, index_project_dir
                    )
                )
                if index_project_dir not in model_run_dict.keys():
                    continue
            ...
            if "model_run_dir" in model_run_dict[index_project_dir]:
                logging.warn(
                    "Found model run dir {} but it's already set: {}".format(
                        model_run_dir,
                        model_run_dict[index_project_dir]["model_run_dir"],
                    )
                )
...
def find_bad_quickboards(model_run_dict):
    ...
    """
    fixed_time = os.path.getmtime(
        "\\\\mainmodel\MainModelShare\\travel-model-one-1.5.1.1\\model-files\\scripts\\core_summaries\\ConsolidateLoadedTransit.R"
    )
    logging.info(
        "Finding bad quickboards (e.g. trnlinkam_withSupport.dbf files older than {}".format(
            time.ctime(fixed_time)
        )
    )
    ...
    for run_id in model_run_dict.keys():
        ...
        if "model_run_dir" not in model_run_dict[run_id]:
            continue

        trn_dbf_file = os.path.join(
            model_run_dict[run_id]["model_run_dir"], "trn", "trnlinkam_withSupport.dbf"
        )

        # if this file doesn't exist then ignore
        if not os.path.exists(trn_dbf_file):
            continue
    ...
    logging.info(
        "Found {:4} runs with bad_quickboards".format(counts["bad_quickboards"])
    )
...
def fix_bad_quickboards(model_run_dict):
    ...
    for run_id in model_run_dict.keys():
        if "bad_quickboards" not in model_run_dict[run_id]:
            continue
        if model_run_dict[run_id]["bad_quickboards"] != True:
            continue

        logging.debug(
            "Fixing {} with model_run_dir {}".format(
                run_id, model_run_dict[run_id]["model_run_dir"]
            )
        )
        ...
        shutil.copy(
            FIXED_SCRIPT,
            os.path.join(
                model_run_dict[run_id]["model_run_dir"],
                "CTRAMP",
                "scripts",
                "core_summaries",
            ),
        ...
        ret_code = run_command(
            command=[
                r"C:\Program Files\R\R-3.5.1\bin\x64\Rscript.exe",
                "--vanilla",
                r".\CTRAMP\scripts\core_summaries\ConsolidateLoadedTransit.R",
            ],
        ...
        shutil.move(
            os.path.join(
                model_run_dict[run_id]["model_run_dir"], "trn", "quickboards.xls"
            ),
            os.path.join(
                model_run_dict[run_id]["model_run_dir"], "trn", "quickboards_bad.xls"
            ),
        )
        # move fixed quickboards into place
        shutil.move(
            os.path.join(model_run_dict[run_id]["model_run_dir"], "quickboards.xls"),
            os.path.join(
                model_run_dict[run_id]["model_run_dir"], "trn", "quickboards.xls"
            ),
        )
        ...
        shutil.move(
            os.path.join(
                model_run_dict[run_id]["ML_dir"], "OUTPUT", "trn", "quickboards.xls"
            ),
            os.path.join(
                model_run_dict[run_id]["ML_dir"], "OUTPUT", "trn", "quickboards_bad.xls"
            ),
        )
        ...
        for timeperiod in ["ea", "am", "md", "pm", "ev"]:
            ...
            os.remove(
                os.path.join(
                    model_run_dict[run_id]["ML_dir"],
                    "OUTPUT",
                    "trn",
                    "trnlink{}_withSupport.dbf".format(timeperiod),
                )
            )
...
def find_bad_ouput(model_run_dict):
    ...
    """
    logging.info(
        "Finding bad output (e.g. runs with mismatching OUTPUT\\metrics\\auto_times.csv and extractor\\metrics\\auto_times.csv"
    )
    ...
    for run_id in model_run_dict.keys():
        ...
        output_file = os.path.join(
            model_run_dict[run_id]["ML_dir"], "OUTPUT", "metrics", "auto_times.csv"
        )
    ...
    logging.info(
        "Found {:4} runs with bad_output == mismatch".format(counts["mismatch"])
    )
...
def find_trnbuild_type(model_run_dict):
    ...
    for run_id in model_run_dict.keys():
        ...
        trn_dir = os.path.join(
            model_run_dict[run_id]["model_run_dir"], "trn", "TransitAssignment.iter3"
        )
        ...
        for line in prn_file:
            ...
            if m.group(1) != "6.4.4":
                logging.warn(
                    "Unexpected TRNBUILD version found for run {} in {}: {}".format(
                        run_id, prn_file, m.group(0)
                    )
                )
...
def find_mismatch_trnbuild(model_run_dict):
    ...
    logging.info(
        "Finding trnbuild_base (e.g. checking if base and build have matching trnbuild_type)"
    )

    for run_id in model_run_dict.keys():
        # only look at those that have a baseline and a trnbuild_type
        if "baseline_id" not in model_run_dict[run_id].keys():
            continue
        baseline_id = model_run_dict[run_id]["baseline_id"]

        if "trnbuild_type" not in model_run_dict[run_id].keys():
            continue
    ...
    logging.info(
        "Found {:4} runs with trnbuild_base == mismatch".format(counts["mismatch"])
    )
    logging.info(
        "Found {:4} runs with trnbuild_base == missing_trnbuild_type".format(
            counts["missing_trnbuild_type"]
        )
    )
    logging.info(
        "Found {:4} runs with trnbuild_base == missing_run".format(
            counts["missing_run"]
        )
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_dirs_path",
        help="Model directory path. e.g. L:\\RTP2021_PPA\\Projects",
        default="L:\\RTP2021_PPA\\Projects",
    )
