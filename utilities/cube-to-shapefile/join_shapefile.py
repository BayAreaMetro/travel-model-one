...
if __name__ == '__main__':
    ...
    arcpy.CopyFeatures_management(in_shapefile1, os.path.join(arcpy.env.workspace, INPUT_SHAPE1))
    ...
    # do the join
    joined_table = arcpy.AddJoin_management(INPUT_SHAPE1, args.join_field, 
                                            INPUT_SHAPE2, args.join_field)
    OUT_FC = "out_fc"
    ...
    for field in fields:
        ...
        if field.name.startswith(INPUT_SHAPE1):
            ...
            if len(new_name) > 9: new_name = new_name[:9]
            new_name = new_name + "1"
        ...
        print("{}: {} => {} is a type of {} with a length of {}".format(OUT_FC, 
               field.name, new_name, field.type, field.length))
