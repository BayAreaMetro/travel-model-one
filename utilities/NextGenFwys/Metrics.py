...
list_dir = [
    ...
    "2035_TM152_NGF_NoProject_01",
    "2035_TM152_NGF_SensDiscount_01",
    "2035_TM152_NGF_SensDiscount_02",
    "2035_TM152_NGF_SensDiscount_04",
    "2035_TM152_NGF_SensDiscount_05",
]
...
# taz_to_city_mapping
citytaz = pd.read_csv(
    "C:/Users/jalatorre/Box/NextGen Freeways Study/07 Tasks/05_GoalsandMetrics/Metrics/taz_with_cities.csv"
)
...
MTB_file_list = [
    os.path.join(
        file_loc, i, "OUTPUT", "core_summaries", "CommuteByIncomeHousehold.csv"
    )
    for i in list_dir
]
MTR_file_list = [
    os.path.join(file_loc, i, "OUTPUT", "metrics", "transit_boards_miles.csv")
    for i in list_dir
]
MVMTM_file_list = [
    os.path.join(file_loc, i, "OUTPUT", "metrics", "vmt_vht_metrics.csv")
    for i in list_dir
]
MR_file_list = [
    os.path.join(file_loc, i, "OUTPUT", "metrics", "auto_times.csv") for i in list_dir
]
MTT_file_list = [
    os.path.join(file_loc, i, "OUTPUT", "avgload5period.csv") for i in list_dir
]
MMS_file_list = [
    os.path.join(file_loc, i, "OUTPUT", "avgload5period_vehclasses.csv")
    for i in list_dir
]
MRDATA_file_list = [
    os.path.join(file_loc, i, "OUTPUT", "updated_output", "trips.rdata")
    for i in list_dir
]
...
def file_list_to_csv(
    file_list, file_name
):  # read through a list of directories for files of the same type (different blueprints), combine the files in one csv and add identifying columns
    df = pd.DataFrame()
    ...
    for file in file_list:
        ...
        try:
            ...
            if (
                ".rdata" in file
            ):  # runtime: 2-3 mins to load and clean each file, 2-3 minutes to write csv. 26 minute runtime for 5 or 6 files
