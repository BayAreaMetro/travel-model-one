...
set M_DIR=L:\Application\Model_One\NextGenFwys_Round2\Scenarios\2035_TM160_NGFr2_NP04_Path2_01
...
:: Yes to add strategies despite this is a "NoProject run". In NGF NoProject, we assume all the PBA50 strategies are on.
:: Except that we have to turn off Vision Zero
set VisionZero=OFF
set VisionZero_NGF=OFF
...
set INPUT_NETWORK=L:\Application\Model_One\NextGenFwys_Round2\INPUT_DEVELOPMENT\Networks\NGF_Networks_NGFround2_P1_14_NoVisionZero\NGF_R2_R2P1_AllLaneTolling_network_2035
...
set PREV_RUN_DIR=L:\Application\Model_One\NextGenFwys_Round2\Scenarios\2035_TM160_NGFr2_NP04_Path1_12
...
set PARAMS=%GITHUB_DIR%\utilities\NextGenFwys\config_NGF_Round2\params_NGFr2_P2_MBUF.properties

:: set the location of the overrides directory (for Blueprint strategies)
set BP_OVERRIDE_DIR=X:\travel-model-one-master\utilities\RTP\strategy_overrides
set NGF_OVERRIDE_DIR=X:\travel-model-one-master\utilities\NextGenFwys\strategy_overrides

:: use special input tolls.csv?
set SwapTollsCsv=Yes
:: if the above is Yes, where is the input tolls.csv?
set TOLLS_CSV=L:\Application\Model_One\NextGenFwys_Round2\Scenarios\2035_TM160_NGFr2_NP04_Path2_01\INPUT_prep\tolls_ALT0.01cent.csv
...
:: NGF-specific scripts
copy /Y "%GITHUB_DIR%\utilities\NextGenFwys\updateUECsToUseTollDist.py"                           CTRAMP\scripts\preprocess
copy /Y "%GITHUB_DIR%\utilities\NextGenFwys\RunNextGenFwysMetrics.bat"                            .
copy /Y "%GITHUB_DIR%\utilities\NextGenFwys\metrics\extract_cost_skims.job"                       CTRAMP\scripts\metrics
copy /Y "%GITHUB_DIR%\utilities\NextGenFwys\metrics\travel-cost-by-income-driving-households.r"   CTRAMP\scripts\metrics
...
:: NGF Vision Zero
:: ------
if %VisionZero_NGF%==ON ( 
   rem Start year (freeways): 2030
   if %MODEL_YEAR_NUM% GEQ 2030 (copy /Y "%NGF_OVERRIDE_DIR%\Vision_Zero_FwyAt65mph\SpeedCapacity_1hour_2030to2050.block"   "CTRAMP\scripts\block\SpeedCapacity_1hour.block")
)
...
:: emfac                              .
copy /y X:\travel-model-one-master\model-files\RunPrepareEmfac.bat                          .
c:\windows\system32\Robocopy.exe /NP /E "X:\travel-model-one-master\model-files\scripts\emfac"   CTRAMP\scripts\emfac
