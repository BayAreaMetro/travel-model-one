...
def find_number_of_active_adults(trips_df):
    ...
    active_adult_trips_df = trips_df.loc[
        trips_df["age"] >= 18,
        [
            "hh_id",
            "person_id",
    ...
    active_adult_trips_df.loc[
        active_adult_trips_df["trip_mode_str"] == "wlk_loc_wlk", "active_mode"
    ] = "wTrnW"
    active_adult_trips_df.loc[
        active_adult_trips_df["trip_mode_str"] == "wlk_lrf_wlk", "active_mode"
    ] = "wTrnW"
    active_adult_trips_df.loc[
        active_adult_trips_df["trip_mode_str"] == "wlk_exp_wlk", "active_mode"
    ] = "wTrnW"
    active_adult_trips_df.loc[
        active_adult_trips_df["trip_mode_str"] == "wlk_hvy_wlk", "active_mode"
    ] = "wTrnW"
    active_adult_trips_df.loc[
        active_adult_trips_df["trip_mode_str"] == "wlk_com_wlk", "active_mode"
    ] = "wTrnW"

    active_adult_trips_df.loc[
        active_adult_trips_df["trip_mode_str"] == "drv_loc_wlk", "active_mode"
    ] = "dTrnW"
    active_adult_trips_df.loc[
        active_adult_trips_df["trip_mode_str"] == "drv_lrf_wlk", "active_mode"
    ] = "dTrnW"
    active_adult_trips_df.loc[
        active_adult_trips_df["trip_mode_str"] == "drv_exp_wlk", "active_mode"
    ] = "dTrnW"
    active_adult_trips_df.loc[
        active_adult_trips_df["trip_mode_str"] == "drv_hvy_wlk", "active_mode"
    ] = "dTrnW"
    active_adult_trips_df.loc[
        active_adult_trips_df["trip_mode_str"] == "drv_com_wlk", "active_mode"
    ] = "dTrnW"

    active_adult_trips_df.loc[
        active_adult_trips_df["trip_mode_str"] == "wlk_loc_drv", "active_mode"
    ] = "wTrnD"
    active_adult_trips_df.loc[
        active_adult_trips_df["trip_mode_str"] == "wlk_lrf_drv", "active_mode"
    ] = "wTrnD"
    active_adult_trips_df.loc[
        active_adult_trips_df["trip_mode_str"] == "wlk_exp_drv", "active_mode"
    ] = "wTrnD"
    active_adult_trips_df.loc[
        active_adult_trips_df["trip_mode_str"] == "wlk_hvy_drv", "active_mode"
    ] = "wTrnD"
    active_adult_trips_df.loc[
        active_adult_trips_df["trip_mode_str"] == "wlk_com_drv", "active_mode"
    ] = "wTrnD"
    ...
    for time_period in ["EA", "AM", "MD", "PM", "EV"]:
        filename = os.path.join(
            "database", "ActiveTimeSkimsDatabase%s.csv" % time_period
        )
        print(
            "{} Reading {}".format(datetime.datetime.now().strftime("%x %X"), filename)
        )
        ...
        for active_mode in ["walk", "bike", "wTrnW", "dTrnW", "wTrnD"]:
            ...
            skim_tp_df.rename(
                columns={"orig": "orig_taz", "dest": "dest_taz"}, inplace=True
            )

            # join it, adding active_mode-named column
            active_adult_trips_df = pandas.merge(
                left=active_adult_trips_df,
                right=skim_tp_df,
            ...
            # set those minutes
            active_adult_trips_df.loc[
                active_adult_trips_df[active_mode].notnull(), "active_minutes"
            ] = active_adult_trips_df[active_mode]
            # drop the new column
    ...
    active_adult_trips_df = active_adult_trips_df.loc[
        active_adult_trips_df["active_minutes"] > 0
    ]
    # see how many trips had failed joins
    percent_fail = 100.0 * len(active_adult_trips_df) / active_adult_trips_df_len
    print(
        "{} Have {} valid active times out of {}, or {:.2f}% join successes".format(
    ...
    active_counts_df = (
        active_adult_trips_df[
            ["hh_id", "person_id", "num_participants", "num_trips", "active_minutes"]
        ]
        .groupby(["hh_id", "person_id"])
        .agg(
            {
                "num_participants": numpy.mean,
                "num_trips": numpy.sum,
                "active_minutes": numpy.sum,
            }
        )
    )
    ...
    active_counts_df = active_counts_df.loc[
        active_counts_df["active_minutes"] >= ACTIVE_MINUTES_THRESHOLD
    ]
    percent_active = 100.0 * len(active_counts_df) / active_counts_len
    print(
        "{} Have {} active (above {} minutes threshold) out of {} total adults with active minutes, or {:.2f}% very active".format(
            datetime.datetime.now().strftime("%x %X"),
            len(active_counts_df),
...
def write_trips_by_od(trips_df, by_income_cat, outsuffix):
    ...
    if by_income_cat:
        ...
        trip_counts = (
            ...
            .groupby(
                ["time_period", "income_cat", "orig_taz", "dest_taz", "trip_mode_str"]
            )
            .sum()
        )
        ...
        trip_counts = (
            ...
            .groupby(["time_period", "orig_taz", "dest_taz", "trip_mode_str"])
            .sum()
        )
        trip_counts = trip_counts.unstack().fillna(0)
        income_list = [0]
    ...
    for timeperiod in ["EA", "AM", "MD", "PM", "EV"]:
        for income_cat in income_list:
            ...
            trip_counts_tpinc = (
                trip_counts.loc[timeperiod, income_cat]
                if by_income_cat
                else trip_counts.loc[timeperiod]
            )
            ...
            for col in cols:
                if col == ("orig_taz", ""):
                    new_cols.append(("orig_taz", "orig_taz"))
                elif col == ("dest_taz", ""):
                    new_cols.append(("dest_taz", "dest_taz"))
                else:
                    new_cols.append(col)
            ...
            if by_income_cat:
                output_filename = os.path.join(
                    "main", "trips%sinc%d%s.dat" % (timeperiod, income_cat, outsuffix)
                )
            else:
                output_filename = os.path.join(
                    "main", "trips%s%s.dat" % (timeperiod, outsuffix)
                )

            trip_counts_tpinc.to_csv(
                output_filename, sep=" ", header=False, index=False
            )
            print(
                "{}  Wrote {}".format(
                    datetime.datetime.now().strftime("%x %X"), output_filename
                )
            )
...
if __name__ == "__main__":
    ...
    for trip_type in ["indiv", "joint"]:
        filename = os.path.join(
            "main", "%sTripDataIncome_%d.csv" % (trip_type, iteration)
        )
        print(
            "{} Reading {}".format(datetime.datetime.now().strftime("%x %X"), filename)
        )
        ...
        print(
            "{} Done reading {} {} trips".format(
                datetime.datetime.now().strftime("%x %X"), len(temp_trips_df), trip_type
            )
        )
    ...
    trips_df.loc[
        (trips_df["depart_hour"] >= 3) & (trips_df["depart_hour"] < 6), "time_period"
    ] = "EA"
    trips_df.loc[
        (trips_df["depart_hour"] >= 6) & (trips_df["depart_hour"] < 10), "time_period"
    ] = "AM"
    trips_df.loc[
        (trips_df["depart_hour"] >= 10) & (trips_df["depart_hour"] < 15), "time_period"
    ] = "MD"
    trips_df.loc[
        (trips_df["depart_hour"] >= 15) & (trips_df["depart_hour"] < 19), "time_period"
    ] = "PM"
    trips_df.loc[
        (trips_df["depart_hour"] >= 19) | (trips_df["depart_hour"] < 3), "time_period"
    ] = "EV"
    ...
    trips_df.loc[
        (trips_df["trip_mode"] == 1) & (trips_df["avAvailable"] == 0), "trip_mode_str"
    ] = "da"
    trips_df.loc[
        (trips_df["trip_mode"] == 2) & (trips_df["avAvailable"] == 0), "trip_mode_str"
    ] = "da_toll"
    trips_df.loc[
        (trips_df["trip_mode"] == 3) & (trips_df["avAvailable"] == 0), "trip_mode_str"
    ] = "sr2"
    trips_df.loc[
        (trips_df["trip_mode"] == 4) & (trips_df["avAvailable"] == 0), "trip_mode_str"
    ] = "sr2_toll"
    trips_df.loc[
        (trips_df["trip_mode"] == 5) & (trips_df["avAvailable"] == 0), "trip_mode_str"
    ] = "sr3"
    trips_df.loc[
        (trips_df["trip_mode"] == 6) & (trips_df["avAvailable"] == 0), "trip_mode_str"
    ] = "sr3_toll"
    ...
    trips_df.loc[
        (trips_df["trip_mode"] == 14) & (trips_df["inbound"] == 0), "trip_mode_str"
    ] = "drv_loc_wlk"
    trips_df.loc[
        (trips_df["trip_mode"] == 15) & (trips_df["inbound"] == 0), "trip_mode_str"
    ] = "drv_lrf_wlk"
    trips_df.loc[
        (trips_df["trip_mode"] == 16) & (trips_df["inbound"] == 0), "trip_mode_str"
    ] = "drv_exp_wlk"
    trips_df.loc[
        (trips_df["trip_mode"] == 17) & (trips_df["inbound"] == 0), "trip_mode_str"
    ] = "drv_hvy_wlk"
    trips_df.loc[
        (trips_df["trip_mode"] == 18) & (trips_df["inbound"] == 0), "trip_mode_str"
    ] = "drv_com_wlk"

    trips_df.loc[
        (trips_df["trip_mode"] == 14) & (trips_df["inbound"] == 1), "trip_mode_str"
    ] = "wlk_loc_drv"
    trips_df.loc[
        (trips_df["trip_mode"] == 15) & (trips_df["inbound"] == 1), "trip_mode_str"
    ] = "wlk_lrf_drv"
    trips_df.loc[
        (trips_df["trip_mode"] == 16) & (trips_df["inbound"] == 1), "trip_mode_str"
    ] = "wlk_exp_drv"
    trips_df.loc[
        (trips_df["trip_mode"] == 17) & (trips_df["inbound"] == 1), "trip_mode_str"
    ] = "wlk_hvy_drv"
    trips_df.loc[
        (trips_df["trip_mode"] == 18) & (trips_df["inbound"] == 1), "trip_mode_str"
    ] = "wlk_com_drv"
    ...
    trips_df.loc[
        (trips_df["trip_mode"] == 1) & (trips_df["avAvailable"] == 1), "trip_mode_str"
    ] = "da_av_notoll"
    trips_df.loc[
        (trips_df["trip_mode"] == 2) & (trips_df["avAvailable"] == 1), "trip_mode_str"
    ] = "da_av_toll"
    trips_df.loc[
        (trips_df["trip_mode"] == 3) & (trips_df["avAvailable"] == 1), "trip_mode_str"
    ] = "sr2_av_notoll"
    trips_df.loc[
        (trips_df["trip_mode"] == 4) & (trips_df["avAvailable"] == 1), "trip_mode_str"
    ] = "sr2_av_toll"
    trips_df.loc[
        (trips_df["trip_mode"] == 5) & (trips_df["avAvailable"] == 1), "trip_mode_str"
    ] = "sr3_av_notoll"
    trips_df.loc[
        (trips_df["trip_mode"] == 6) & (trips_df["avAvailable"] == 1), "trip_mode_str"
    ] = "sr3_av_toll"
    ...
    trips_df.loc[
        (trips_df["income"] >= 30000) & (trips_df["income"] < 60000), "income_cat"
    ] = 2
    trips_df.loc[
        (trips_df["income"] >= 60000) & (trips_df["income"] < 100000), "income_cat"
    ] = 3
    ...
    print(
        "{} Filtered to non-auto trips, of which there are {}".format(
            datetime.datetime.now().strftime("%x %X"), len(trips_df)
        )
    )
    ...
    print(
        "{} => {} indiv trips, {} joint trip rows making {} joint trips".format(
            datetime.datetime.now().strftime("%x %X"),
            len(trips_df),
            len(joint_trips_df),
            num_joint_trips,
    ...
    joint_tours = pandas.read_table(
        os.path.join("main", "jointTourData_%d.csv" % iteration),
        sep=",",
        index_col=False,
    )
    joint_tours = joint_tours[["hh_id", "tour_id", "tour_participants"]]
    joint_tours["num_participants"] = (
        joint_tours.tour_participants.str.count(" ") + 1.0
    ) / sampleshare
    ...
    trips_df.drop(
        "person_id", axis=1, inplace=True
    )  # this will come from hh_id, person_num and persons table
    ...
    print(
        "{} Done reading {} persons".format(
            datetime.datetime.now().strftime("%x %X"), len(persons_df)
        )
    )
    ...
    print(
        "{} Filtered to {} trips between 20-74 year olds".format(
            datetime.datetime.now().strftime("%x %X"), len(trips_df)
        )
    )
    ...
    print(
        "{} => made by {} unique individuals walking".format(
            datetime.datetime.now().strftime("%x %X"),
            travelers_dict["unique_walkers_2074"],
        )
    )
    ...
    print(
        "{} => made by {} unique individuals taking transit".format(
            datetime.datetime.now().strftime("%x %X"),
            travelers_dict["unique_transiters_2074"],
        )
    )
    ...
    print(
        "{} Filtered to {} trips between 20-64 year olds".format(
            datetime.datetime.now().strftime("%x %X"), len(trips_df)
        )
    )
    ...
    print(
        "{} => made by {} unique individuals biking".format(
            datetime.datetime.now().strftime("%x %X"),
            travelers_dict["unique_cyclists_2064"],
        )
    )
