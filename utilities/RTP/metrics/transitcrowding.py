...
PSEUDO_LINE_MAPPING = {
    ...
    "120_OR_YEL": (
        "120_ORANGE[A]?-$",
        "120_YELLOW[-9W]$",
    ),  # Orange/Richmond - MacArthur - Yellow/SFO
    "120_OR_YER": (
        "120_YELLOW[E]?$",
        "120_ORANGE[A]?$",
    ),  # Yellow/SFO - MacArthur - Orange/Richmond
    # 2015 IPA
    ...
    "120_BL_PS": (
        "120_BLUE-",
        "120_GREEN-",
    ),  # Blue/DublinPleasanton - San Antonio - Green/Millbrae
    "120_BL_PSR": (
        "120_GREEN$",
        "120_BLUE$",
    ),  # Green/Millbrae - San Antonio - Blue/DublinPleasanton
    "120_GREEN_BL": (
        "120_GREEN-",
        "120_BLUE-",
    ),  # Green/BerryessaFremont - SanAntonio - Blue/Millbrae
    "120_GREEN_BR": (
        "120_BLUE$",
        "120_GREEN$",
    ),  # Blue/MillBrae - San Antonio - Green/BerryessaFremont
    "120_GRN": (
        "120_ORANGE$",
        "120_NEW-",
    ),  # Orange/BerryessaFremont - San Antonio - New/Millbrae
    "120_GRN_R": (
        "120_NEW$",
        "120_ORANGE-",
    ),  # New/Millbrae - San Antonio - Orange/BerryessaFremont
    "120_NEW_PS": (
        "120_NEW$",
        "120_YELLOWW",
    ),  # New/Pittsburg - MacArthur - Yellow/Millbrae (*)
    "120_NEW_PSR": (
        "120_YELLOWE",
        "120_NEW-",
    ),  # Yellow/Millbrae - MacArthur - New/Pittsburg (*)
    "120_RED_YEL": (
        "120_ORANGE-",
        "120_YELLOWW",
    ),  # Orange/Richmond - MacArthur - Yellow/Millbrae
    "120_RED_YER": (
        "120_YELLOWE",
        "120_ORANGE$",
    ),  # Yellow/Millbrae - MacArthur - Orange/Richmond
    # crossings 4
    "130_RR_PSEU": (
        "130_RR_SACR$",
        "130_RR_F_EXP-",
        14648,
    ),  # RR_Sacr/Martinez - Jack London - Fremont (along easy bay)
...
def find_pseudo_lines(trn_link_df):
    ...
    for group_id, line in line_group:
        if group_id[0] not in [120]:
            continue  #  ie do this only for BART and Reg Rail
        if len(line) < 3:
            continue  # pseudo lines must have board + transfer + exit
        ...
        if seq_last_board < seq_first_exit:
            ...
            pseudo_lines.append(
                (group_id[0], group_id[1], group_id[2], seq_last_board, seq_first_exit)
            )
...
def move_pseudo_line_ridership(trn_link_df, pseudo_lines):
    ...
    for pseudo_line in pseudo_lines:
        ...
        if pseudo_line_name not in PSEUDO_LINE_MAPPING:
            logging.fatal(
                "{} is not configured in PSEUDO_LINE_MAPPING".format(pseudo_line_name)
            )
            sys.exit(2)
        ...
        pseudo_line_df = trn_link_df.loc[
            (trn_link_df["NAME"] == pseudo_line_name)
            & (trn_link_df["period"] == period),
            [
                "NAME",
                "SEQ",
                "period",
        ...
        logging.info(
            "Moving ridership for pseudo line {} period {} run_per_hr {} last board {}/first exit {}".format(
                pseudo_line_name,
                period,
                pseudo_run_per_hr,
                seq_last_board,
        ...
        if seq_last_board < seq_first_exit - 1:
            if len(PSEUDO_LINE_MAPPING[pseudo_line_name]) > 2:
                ...
                seq_last_board = pseudo_line_df.loc[
                    pseudo_line_df["B"] == transfer_stop
                ].iloc[0]["SEQ"]
                seq_first_exit = pseudo_line_df.loc[
                    pseudo_line_df["A"] == transfer_stop
                ].iloc[0]["SEQ"]
                logging.debug(
                    "Multiple possible transfer stops; PSEUDO_LINE_MAPPING lookup used to determine last board {}/first exit {}".format(
                        seq_last_board, seq_first_exit
                    )
                )
            else:
                logging.fatal(
                    "Multiple possible transfer stops; specify the transfer stop in PSEUDO_LINE_MAPPING"
                )

        # preparing for join -- everybody gets off at transfer point and on at transfer point
        pseudo_line_df.loc[
            pseudo_line_df["SEQ"] == seq_last_board, "AB_XITB"
        ] = pseudo_line_df["AB_VOL"]
        pseudo_line_df.loc[
            pseudo_line_df["SEQ"] == seq_first_exit, "AB_BRDA"
        ] = pseudo_line_df["AB_VOL"]

        # create column that contains start_line_re for board stops, end_line_re for exit stops
        pseudo_line_df["name_re"] = ""
        pseudo_line_df.loc[
            pseudo_line_df["SEQ"] <= seq_last_board, "name_re"
        ] = start_line_re
        pseudo_line_df.loc[
            pseudo_line_df["SEQ"] >= seq_first_exit, "name_re"
        ] = end_line_re
        ...
        trn_link_df["name_re"] = ""
        trn_link_df.loc[
            trn_link_df["NAME"].str.match(start_line_re) == True, "name_re"
        ] = start_line_re
        trn_link_df.loc[
            trn_link_df["NAME"].str.match(end_line_re) == True, "name_re"
        ] = end_line_re

        # join it on the pseudo line
        trn_link_df = pd.merge(
            left=trn_link_df,
            right=pseudo_line_df[
                [
                    "period",
                    "run_per_hr",
                    "name_re",
        ...
        debug_df = trn_link_df.loc[
            trn_link_df["_merge"] != "left_only",
            [
                "NAME",
                "period",
                "SEQ",
        ...
        ].sort_values(by="SEQ_pseudo")
        logging.debug("BEFORE\n{}".format(debug_df))

        # count the matches for each link
        match_group = trn_link_df.loc[trn_link_df["_merge"] != "left_only"].groupby(
            ["A", "B"]
        )
        ...
        pseudo_link_with_missing_actual = match_agg_df.loc[
            match_agg_df["NAME"] < 1
        ].reset_index()
        if len(pseudo_link_with_missing_actual) > 0:
            ...
            if len(pseudo_link_with_missing_actual) <= 5:
                logging.warning(
                    "{} Pseudo links found with no corresponding actual link:\n{}".format(
                        len(pseudo_link_with_missing_actual),
                        pseudo_link_with_missing_actual,
                    )
                )
            else:
        ...
        match_agg_df.loc[
            abs(match_agg_df["run_per_hr_diff"]) < 0.01, "run_per_hr_diff"
        ] = 0
        logging.debug("match_agg_df:\n{}".format(match_agg_df))

        mismatch_run_per_hr = match_agg_df.loc[
            match_agg_df["run_per_hr_diff"] != 0
        ].reset_index()
        if len(mismatch_run_per_hr) > 0:
            # let a few links slide
            if len(mismatch_run_per_hr) <= 5:
                logging.warning(
                    "{} Pseudo links found with mismatching run per hour:\n{}".format(
            ...
            # more in EA, EV
                ...
                logging.fatal(
                    "Mismatch run_per_hr between pseudo line and matching line\n{}".format(
                        mismatch_run_per_hr
                    )
                )
                sys.exit()
        ...
        trn_link_df.loc[trn_link_df["_merge"] == "both", "AB_BRDA"] = (
            trn_link_df["AB_BRDA"] + trn_link_df["AB_BRDA_pseudo"]
        )
        trn_link_df.loc[trn_link_df["_merge"] == "both", "AB_XITB"] = (
            trn_link_df["AB_XITB"] + trn_link_df["AB_XITB_pseudo"]
        )
        trn_link_df.loc[trn_link_df["_merge"] == "both", "AB_VOL"] = (
            trn_link_df["AB_VOL"] + trn_link_df["AB_VOL_pseudo"]
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no_pseudo_move",
        action="store_true",
        help="Don't move pseudo line ridership into real lines",
    )
    ...
    if os.path.exists(metrics_dir):
        ...
        print(
            "metrics dir [{}] doesn't exist; trying {}".format(
    ...
    transit_seatcap_df.rename(
        columns={"VEHTYPE": "veh_type_updated", "100pctCapacity": "standcap"},
        inplace=True,
    )
    ...
    if my_args.no_pseudo_move:
        logging.info(
            "Moving pseudo line ridership into actual lines suppressed by argument"
        )
    elif len(pseudo_lines) > 0:

        # custom override for project
        if "7000_Resilience_BARTCaldecott" in my_args.project_dir:
            PSEUDO_LINE_MAPPING["120_OR_YEL"] = (
                "120_ORANGE[A]?-$",
                "120_YELLOWE-$",
            )  # Orange/Richmond - MacArthur - Yellow/SFO
            PSEUDO_LINE_MAPPING["120_OR_YER"] = (
                "120_YELLOWE$",
                "120_ORANGE[A]?$",
            )  # Yellow/SFO - MacArthur - Orange/Richmond
    ...
    # AC Transit Transbay
    all_trn_df.loc[
        all_trn_df["SYSTEM"] == "AC Transit", "veh_type_updated"
    ] = "AC Plus Bus"  # note this is transbay buses
    all_trn_df.loc[
        all_trn_df["SYSTEM"] == "AC Transit Transbay", "veh_type_updated"
    ] = "AC Plus Bus"  # note this is transbay buses

    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("84_LA"), "veh_type_updated"
    ] = "Motor Articulated Bus"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("84_NL"), "veh_type_updated"
    ] = "Motor Articulated Bus"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("84_O"), "veh_type_updated"
    ] = "Motor Articulated Bus"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("84_NX"), "veh_type_updated"
    ] = "Motor Articulated Bus"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("84_SB"), "veh_type_updated"
    ] = "Motor Articulated Bus"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("84_F"), "veh_type_updated"
    ] = "Motor Articulated Bus"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("84_J"), "veh_type_updated"
    ] = "Motor Articulated Bus"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("84_CFLA"), "veh_type_updated"
    ] = "Motor Articulated Bus"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("84_CFNL"), "veh_type_updated"
    ] = "Motor Articulated Bus"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("84_CFO"), "veh_type_updated"
    ] = "Motor Articulated Bus"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("84_CFNX"), "veh_type_updated"
    ] = "Motor Articulated Bus"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("84_CFSB"), "veh_type_updated"
    ] = "Motor Articulated Bus"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("84_CFF"), "veh_type_updated"
    ] = "Motor Articulated Bus"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("84_CFJ"), "veh_type_updated"
    ] = "Motor Articulated Bus"

    # Other express buses
    all_trn_df.loc[
        all_trn_df["SYSTEM"] == "Golden Gate Transit Express", "veh_type_updated"
    ] = "AC Plus Bus"  # note this is transbay buses
    all_trn_df.loc[
        all_trn_df["SYSTEM"] == "WestCAT Express", "veh_type_updated"
    ] = "AC Plus Bus"  # note this is transbay buses
    all_trn_df.loc[
        all_trn_df["SYSTEM"] == "Golden Gate Transit Express", "veh_type_updated"
    ] = "AC Plus Bus"  # note this is transbay buses
    ...
    # AC Transit Local
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("30_1AC"), "veh_type_updated"
    ] = "Motor Articulated Bus"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("30_BRT"), "veh_type_updated"
    ] = "Motor Articulated Bus"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("30_6AC"), "veh_type_updated"
    ] = "Motor Articulated Bus"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("30_72R"), "veh_type_updated"
    ] = "Motor Articulated Bus"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("30_97R"), "veh_type_updated"
    ] = "Motor Articulated Bus"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("30_40"), "veh_type_updated"
    ] = "Motor Bus Mix of Standard and Artics"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("30_52"), "veh_type_updated"
    ] = "Motor Bus Mix of Standard and Artics"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("30_57"), "veh_type_updated"
    ] = "Motor Bus Mix of Standard and Artics"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("30_60"), "veh_type_updated"
    ] = "Motor Bus Mix of Standard and Artics"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("30_97"), "veh_type_updated"
    ] = "Motor Bus Mix of Standard and Artics"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("30_99"), "veh_type_updated"
    ] = "Motor Bus Mix of Standard and Artics"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("30_217"), "veh_type_updated"
    ] = "Motor Bus Mix of Standard and Artics"

    # VTA bus
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("522VTA"), "veh_type_updated"
    ] = "Motor Articulated Bus"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("523VTA"), "veh_type_updated"
    ] = "Motor Articulated Bus"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("22VTA"), "veh_type_updated"
    ] = "Motor Articulated Bus"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("40VTA"), "veh_type_updated"
    ] = "Motor Bus Mix of Standard and Artics"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("55VTA"), "veh_type_updated"
    ] = "Motor Bus Mix of Standard and Artics"

    # Samtrans
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("24_ECR"), "veh_type_updated"
    ] = "Motor Articulated Bus"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("24_292"), "veh_type_updated"
    ] = "Motor Articulated Bus"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("24_398"), "veh_type_updated"
    ] = "Motor Articulated Bus"

    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("120_EBART"), "veh_type_updated"
    ] = "eBart 1 car"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("120_EBART") & (all_trn_df["period"] == "AM"),
        "veh_type_updated",
    ] = "eBart 2 car"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("120_EBART") & (all_trn_df["period"] == "PM"),
        "veh_type_updated",
    ] = "eBart 2 car"

    # for crossings projects
    all_trn_df.loc[
        (all_trn_df["veh_type_updated"] == "8 Car BART")
        & (all_trn_df["period"] == "EA"),
        "veh_type_updated",
    ] = "5 Car BART RENOVATED"
    all_trn_df.loc[
        (all_trn_df["veh_type_updated"] == "8 Car BART")
        & (all_trn_df["period"] == "MD"),
        "veh_type_updated",
    ] = "5 Car BART RENOVATED"
    all_trn_df.loc[
        (all_trn_df["veh_type_updated"] == "8 Car BART")
        & (all_trn_df["period"] == "EV"),
        "veh_type_updated",
    ] = "5 Car BART RENOVATED"
    all_trn_df.loc[
        (all_trn_df["veh_type_updated"] == "8 Car BART")
        & (all_trn_df["period"] == "AM"),
        "veh_type_updated",
    ] = "10 Car BART RENOVATED"
    all_trn_df.loc[
        (all_trn_df["veh_type_updated"] == "8 Car BART")
        & (all_trn_df["period"] == "PM"),
        "veh_type_updated",
    ] = "10 Car BART RENOVATED"
    all_trn_df.loc[
        all_trn_df["NAME"].str.contains("130_RR"), "veh_type_updated"
    ] = "Caltrain PCBB 10 car"

    # print(all_trn_df.loc[ all_trn_df["VEHTYPE"] != all_trn_df["veh_type_updated"]])
    logging.info(
        "Updated vehicle type for {} links".format(
            len(all_trn_df.loc[all_trn_df["VEHTYPE"] != all_trn_df["veh_type_updated"]])
        )
    )
    ...
    all_trn_df["period_seatcap"] = (
        all_trn_df["PERIODCAP"] / all_trn_df["VEHCAP"] * all_trn_df["seatcap"]
    )  # total seated capacity in time period
    all_trn_df["period_standcap"] = (
        all_trn_df["PERIODCAP"] / all_trn_df["VEHCAP"] * all_trn_df["standcap"]
    )  # total seated capacity in time period
    all_trn_df["load_seatcap"] = (
        all_trn_df["AB_VOL"] / all_trn_df["period_seatcap"]
    )  # seated load over time period
    all_trn_df["load_standcap"] = (
        all_trn_df["AB_VOL"] / all_trn_df["period_standcap"]
    )  # standing load over time period
    all_trn_df["ivtt_hours"] = (
        all_trn_df["AB_VOL"] * (all_trn_df["TIME"] / 100) / 60
    )  # number of trips * time per trip
    ...
    all_trn_df["vol_minus_seatcap"] = (
        all_trn_df["AB_VOL"] - all_trn_df["period_seatcap"]
    )
    all_trn_df.loc[all_trn_df["vol_minus_seatcap"] < 0, "vol_minus_seatcap"] = 0

    all_trn_df.loc[
        all_trn_df["AB_VOL"] > all_trn_df["period_seatcap"], "crowdingfactor_metrolinx"
    ] = (
        (
            (1.0 + (0.1 * all_trn_df["load_seatcap"].pow(1.4)))
            * all_trn_df[["period_seatcap", "AB_VOL"]].min(axis=1)
        )
        + (
            (1.4 + (0.2 * all_trn_df["load_seatcap"].pow(3.4)))
            * all_trn_df["vol_minus_seatcap"]
        )
    ) / all_trn_df[
        "AB_VOL"
    ...
    all_trn_df["crowdingfactor_metrolinx_max2pt5"] = all_trn_df[
        "crowdingfactor_metrolinx"
    ]
    all_trn_df.loc[
        all_trn_df["crowdingfactor_metrolinx"] > 2.5, "crowdingfactor_metrolinx_max2pt5"
    ] = 2.5
    ...
    all_trn_df.loc[
        all_trn_df["AB_VOL"] > all_trn_df["period_seatcap"], "crowdingfactor_ukdft"
    ] = (
        (all_trn_df["period_seatcap"] * all_trn_df["sit"])
        + ((all_trn_df["AB_VOL"] - all_trn_df["period_seatcap"]) * all_trn_df["stand"])
    ) / all_trn_df[
        "AB_VOL"
    ]

    # calculating effective ivtt = ivtt * crowding factor
    all_trn_df["effective_ivtt_ukdft"] = (
        all_trn_df["ivtt_hours"] * all_trn_df["crowdingfactor_ukdft"]
    )
    all_trn_df["effective_ivtt_metrolinx"] = (
        all_trn_df["ivtt_hours"] * all_trn_df["crowdingfactor_metrolinx"]
    )
    all_trn_df["effective_ivtt_metrolinx_max2pt5"] = (
        all_trn_df["ivtt_hours"] * all_trn_df["crowdingfactor_metrolinx_max2pt5"]
    )

    all_trn_df["crowding_penalty_hrs_ukdft"] = (
        all_trn_df["effective_ivtt_ukdft"] - all_trn_df["ivtt_hours"]
    )
    all_trn_df["crowding_penalty_hrs_metrolinx"] = (
        all_trn_df["effective_ivtt_metrolinx"] - all_trn_df["ivtt_hours"]
    )
    all_trn_df["crowding_penalty_hrs_metrolinx_max2pt5"] = (
        all_trn_df["effective_ivtt_metrolinx_max2pt5"] - all_trn_df["ivtt_hours"]
    )
    ...
    transit_crowding_filename = os.path.join(
        metrics_dir, "transit_crowding_complete.csv"
    )
    all_trn_df.to_csv(transit_crowding_filename, header=True, index=False)
    logging.info(
        "Wrote {} lines to {}".format(len(all_trn_df), transit_crowding_filename)
    )
    ...
    all_trn_df[
        [
            ...
            "period",
            "ivtt_hours",
            "effective_ivtt_ukdft",
            "effective_ivtt_metrolinx",
            "effective_ivtt_metrolinx_max2pt5",
            "crowding_penalty_hrs_ukdft",
            "crowding_penalty_hrs_metrolinx",
            "crowding_penalty_hrs_metrolinx_max2pt5",
        ]
