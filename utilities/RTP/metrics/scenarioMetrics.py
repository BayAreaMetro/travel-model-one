...
def tally_travel_cost(iteration, sampleshare, metrics_dict):
    ...
    transit_df = pandas.read_csv(
        os.path.join("metrics", "transit_times_by_mode_income.csv"),
        sep=",",
        index_col=[0, 1],
    )
    ...
    for inc_level in range(1, 5):
        metrics_dict["total_transit_fares_inc%d" % inc_level] = transit_df.loc[
            "_no_zpv_inc%d" % inc_level, "Total Cost"
        ]
        metrics_dict["total_transit_trips_inc%d" % inc_level] = transit_df.loc[
            "_no_zpv_inc%d" % inc_level, "Daily Trips"
        ]

    auto_df = pandas.read_csv(
        os.path.join("metrics", "auto_times.csv"), sep=",", index_col=[0, 1]
    )
    auto_df = auto_df.groupby("Income").agg("sum")
    for inc_level in range(1, 5):
        metrics_dict["total_auto_cost_inc%d" % inc_level] = (
            auto_df.loc[
                "inc%d" % inc_level, ["Total Cost", "Bridge Tolls", "Value Tolls"]
            ].sum()
            / 100
        )  # cents -> dollars
        metrics_dict["total_auto_trips_inc%d" % inc_level] = auto_df.loc[
            "inc%d" % inc_level, "Daily Person Trips"
        ]

    # Count households from disaggregate output
    household_df = pandas.read_csv(
        os.path.join("main", "householdData_%d.csv" % iteration), sep=","
    )
    household_df["income_cat"] = 0
    household_df.loc[(household_df["income"] < 30000), "income_cat"] = 1
    household_df.loc[
        (household_df["income"] >= 30000) & (household_df["income"] < 60000),
        "income_cat",
    ] = 2
    household_df.loc[
        (household_df["income"] >= 60000) & (household_df["income"] < 100000),
        "income_cat",
    ] = 3
    ...
    for inc_level in range(1, 5):
        metrics_dict["total_households_inc%d" % inc_level] = household_df.loc[
            household_df.income_cat == inc_level, "num_hhs"
        ].sum()
        metrics_dict["total_hh_inc_inc%d" % inc_level] = household_df.loc[
            household_df.income_cat == inc_level, "income"
        ].sum()
...
def tally_access_to_jobs(iteration, sampleshare, metrics_dict):
    ...
    * jobacc_trn_drv_acc_accessible_job_share  :   accessible job share for transit *and drv
    ...
    traveltime_df = pandas.read_csv(
        os.path.join("database", "TimeSkimsDatabaseAM.csv"), sep=","
    )
    ...
    traveltime_df = traveltime_df.loc[
        (traveltime_df.wTrnW <= 45) | (traveltime_df.da <= 30)
    ]
    print(
        "  Out of {} O/D pairs, {} are accessible within 45 min wTrnW or 30 min da".format(
            len_traveltime_df, len(traveltime_df)
        )
    )
    ...
    traveltime_df.loc[
        (traveltime_df.wTrnW <= 45)
        & ((traveltime_df.da > 30) | pandas.isnull(traveltime_df.da)),
        "trn_only",
    ] = 1
    traveltime_df.loc[
        ((traveltime_df.wTrnW > 45) | pandas.isnull(traveltime_df.wTrnW))
        & (traveltime_df.da <= 30),
        "drv_only",
    ] = 1
    traveltime_df.loc[
        (traveltime_df.wTrnW <= 45) & (traveltime_df.da <= 30), "trn_drv"
    ] = 1
    assert (
        traveltime_df.trn_only.sum()
        + traveltime_df.drv_only.sum()
        + traveltime_df.trn_drv.sum()
        == len(traveltime_df)
    )
    ...
    traveltime_df = pandas.merge(
        left=traveltime_df,
        right=tazdata_df[["ZONE", "TOTEMP"]],
        left_on="dest",
        right_on="ZONE",
        how="left",
    )
    ...
    accessiblejobs_df = traveltime_df_grouped.agg(
        {
            "da": numpy.mean,
            "wTrnW": numpy.mean,
            "TOTEMP": numpy.sum,
    ...
    coc_df = pandas.read_csv(
        os.path.join("metrics", "CommunitiesOfConcern.csv"), sep=","
    )
    tazdata_df = pandas.merge(
        left=tazdata_df, right=coc_df, left_on="ZONE", right_on="taz"
    )
    ...
    urban_suburban_df = pandas.read_csv(
        os.path.join("INPUT", "metrics", "taz_urban_suburban.csv")
    )
    urban_suburban_df.rename(
        columns={"area_type": "U_S_R"}, inplace=True
    )  # Urban Suburban Rural
    print(
        "  Read urban_suburban_df:\n{}".format(
            urban_suburban_df["U_S_R"].value_counts()
        )
    )
    tazdata_df = pandas.merge(
        left=tazdata_df, right=urban_suburban_df, left_on="ZONE", right_on="TAZ1454"
    )
    ...
    accessiblejobs_df = pandas.merge(
        ...
        right=tazdata_df[["ZONE", "TOTPOP", "in_coc", "in_hra", "U_S_R"]],
        left_index=True,
        right_on="ZONE",
        how="left",
    )
    accessiblejobs_df["TOTEMP_weighted"] = (
        accessiblejobs_df["TOTEMP"] * accessiblejobs_df["TOTPOP"]
    )
    accessiblejobs_df["trn_only_weighted"] = (
        accessiblejobs_df["trn_only"] * accessiblejobs_df["TOTPOP"]
    )
    accessiblejobs_df["drv_only_weighted"] = (
        accessiblejobs_df["drv_only"] * accessiblejobs_df["TOTPOP"]
    )
    accessiblejobs_df["trn_drv_weighted"] = (
        accessiblejobs_df["trn_drv"] * accessiblejobs_df["TOTPOP"]
    )
    # print(accessiblejobs_df.head())
    ...
    for suffix in [
        ...
        if suffix == "_coc":
            ...
            accjob_subset_df = accessiblejobs_df.loc[
                accessiblejobs_df["U_S_R"] == "urban"
            ]
            totalpop_subset = tazdata_df.loc[
                tazdata_df["U_S_R"] == "urban", "TOTPOP"
            ].sum()
        elif suffix == "_suburban":
            accjob_subset_df = accessiblejobs_df.loc[
                accessiblejobs_df["U_S_R"] == "suburban"
            ]
            totalpop_subset = tazdata_df.loc[
                tazdata_df["U_S_R"] == "suburban", "TOTPOP"
            ].sum()
        elif suffix == "_rural":
            accjob_subset_df = accessiblejobs_df.loc[
                accessiblejobs_df["U_S_R"] == "rural"
            ]
            totalpop_subset = tazdata_df.loc[
                tazdata_df["U_S_R"] == "rural", "TOTPOP"
            ].sum()
        ...
        metrics_dict["jobacc_acc_jobs_weighted_persons%s" % suffix] = accjob_subset_df[
            "TOTEMP_weighted"
        ].sum()
        metrics_dict[
            "jobacc_trn_only_acc_jobs_weighted_persons%s" % suffix
        ] = accjob_subset_df["trn_only_weighted"].sum()
        metrics_dict[
            "jobacc_drv_only_acc_jobs_weighted_persons%s" % suffix
        ] = accjob_subset_df["drv_only_weighted"].sum()
        metrics_dict[
            "jobacc_trn_drv_acc_jobs_weighted_persons%s" % suffix
        ] = accjob_subset_df["trn_drv_weighted"].sum()
        metrics_dict["jobacc_total_jobs_weighted_persons%s" % suffix] = (
            total_emp * totalpop_subset
        )
        metrics_dict["jobacc_accessible_job_share%s" % suffix] = float(
            metrics_dict["jobacc_acc_jobs_weighted_persons%s" % suffix]
        ) / float(metrics_dict["jobacc_total_jobs_weighted_persons%s" % suffix])
        metrics_dict["jobacc_trn_only_acc_accessible_job_share%s" % suffix] = float(
            metrics_dict["jobacc_trn_only_acc_jobs_weighted_persons%s" % suffix]
        ) / float(metrics_dict["jobacc_total_jobs_weighted_persons%s" % suffix])
        metrics_dict["jobacc_drv_only_acc_accessible_job_share%s" % suffix] = float(
            metrics_dict["jobacc_drv_only_acc_jobs_weighted_persons%s" % suffix]
        ) / float(metrics_dict["jobacc_total_jobs_weighted_persons%s" % suffix])
        metrics_dict["jobacc_trn_drv_acc_accessible_job_share%s" % suffix] = float(
            metrics_dict["jobacc_trn_drv_acc_jobs_weighted_persons%s" % suffix]
        ) / float(metrics_dict["jobacc_total_jobs_weighted_persons%s" % suffix])
...
def tally_access_to_jobs_v2(iteration, sampleshare, metrics_dict):
    ...
    * jobacc2_walk_20_accessible_job_share       : accessible job share = jobacc2_walk_20_acc_jobs_weighted_persons/jobacc2_total_jobs_weighted_persons
    ...
    traveltime_df = traveltime_df[
        ["orig", "dest", "da", "daToll", "wTrnW", "bike", "walk"]
    ]
    # -999 is really no-access
    traveltime_df.replace(to_replace=[-999.0], value=[None], inplace=True)
    len_traveltime_df = len(traveltime_df)
    ...
    accessiblejobs_df["wtrn_45_weighted"] = (
        accessiblejobs_df["wtrn_45"] * accessiblejobs_df["TOTPOP"]
    )
    accessiblejobs_df["wtrn_30_weighted"] = (
        accessiblejobs_df["wtrn_30"] * accessiblejobs_df["TOTPOP"]
    )
    accessiblejobs_df["da_30_weighted"] = (
        accessiblejobs_df["da_30"] * accessiblejobs_df["TOTPOP"]
    )
    accessiblejobs_df["dat_30_weighted"] = (
        accessiblejobs_df["dat_30"] * accessiblejobs_df["TOTPOP"]
    )
    accessiblejobs_df["bike_20_weighted"] = (
        accessiblejobs_df["bike_20"] * accessiblejobs_df["TOTPOP"]
    )
    accessiblejobs_df["walk_20_weighted"] = (
        accessiblejobs_df["walk_20"] * accessiblejobs_df["TOTPOP"]
    )
    ...
    for suffix in [
        ...
        #  e.g. sum over TAZs of (totpop at TAZ x totemp jobs accessible)
        # denominator = total jobs weighted by persons
        metrics_dict["jobacc2_acc_jobs_weighted_persons%s" % suffix] = accjob_subset_df[
            "TOTEMP_weighted"
        ].sum()
        metrics_dict[
            "jobacc2_wtrn_45_acc_jobs_weighted_persons%s" % suffix
        ] = accjob_subset_df["wtrn_45_weighted"].sum()
        metrics_dict[
            "jobacc2_wtrn_30_acc_jobs_weighted_persons%s" % suffix
        ] = accjob_subset_df["wtrn_30_weighted"].sum()
        metrics_dict[
            "jobacc2_da_30_acc_jobs_weighted_persons%s" % suffix
        ] = accjob_subset_df["da_30_weighted"].sum()
        metrics_dict[
            "jobacc2_dat_30_acc_jobs_weighted_persons%s" % suffix
        ] = accjob_subset_df["dat_30_weighted"].sum()
        metrics_dict[
            "jobacc2_bike_20_acc_jobs_weighted_persons%s" % suffix
        ] = accjob_subset_df["bike_20_weighted"].sum()
        metrics_dict[
            "jobacc2_walk_20_acc_jobs_weighted_persons%s" % suffix
        ] = accjob_subset_df["walk_20_weighted"].sum()

        metrics_dict["jobacc2_total_jobs_weighted_persons%s" % suffix] = (
            total_emp * totalpop_subset
        )

        metrics_dict["jobacc2_wtrn_45_acc_accessible_job_share%s" % suffix] = float(
            metrics_dict["jobacc2_wtrn_45_acc_jobs_weighted_persons%s" % suffix]
        ) / float(metrics_dict["jobacc2_total_jobs_weighted_persons%s" % suffix])
        metrics_dict["jobacc2_wtrn_30_acc_accessible_job_share%s" % suffix] = float(
            metrics_dict["jobacc2_wtrn_30_acc_jobs_weighted_persons%s" % suffix]
        ) / float(metrics_dict["jobacc2_total_jobs_weighted_persons%s" % suffix])
        metrics_dict["jobacc2_da_30_acc_accessible_job_share%s" % suffix] = float(
            metrics_dict["jobacc2_da_30_acc_jobs_weighted_persons%s" % suffix]
        ) / float(metrics_dict["jobacc2_total_jobs_weighted_persons%s" % suffix])
        metrics_dict["jobacc2_dat_30_acc_accessible_job_share%s" % suffix] = float(
            metrics_dict["jobacc2_dat_30_acc_jobs_weighted_persons%s" % suffix]
        ) / float(metrics_dict["jobacc2_total_jobs_weighted_persons%s" % suffix])
        metrics_dict["jobacc2_bike_20_acc_accessible_job_share%s" % suffix] = float(
            metrics_dict["jobacc2_bike_20_acc_jobs_weighted_persons%s" % suffix]
        ) / float(metrics_dict["jobacc2_total_jobs_weighted_persons%s" % suffix])
        metrics_dict["jobacc2_walk_20_acc_accessible_job_share%s" % suffix] = float(
            metrics_dict["jobacc2_walk_20_acc_jobs_weighted_persons%s" % suffix]
        ) / float(metrics_dict["jobacc2_total_jobs_weighted_persons%s" % suffix])
    ...
    # household version
    accessiblejobs_df["TOTEMP_weightedhh"] = (
        accessiblejobs_df["TOTEMP"] * accessiblejobs_df["TOTHH"]
    )
    accessiblejobs_df["wtrn_45_weightedhh"] = (
        accessiblejobs_df["wtrn_45"] * accessiblejobs_df["TOTHH"]
    )
    accessiblejobs_df["wtrn_30_weightedhh"] = (
        accessiblejobs_df["wtrn_30"] * accessiblejobs_df["TOTHH"]
    )
    accessiblejobs_df["da_30_weightedhh"] = (
        accessiblejobs_df["da_30"] * accessiblejobs_df["TOTHH"]
    )
    accessiblejobs_df["dat_30_weightedhh"] = (
        accessiblejobs_df["dat_30"] * accessiblejobs_df["TOTHH"]
    )
    accessiblejobs_df["bike_20_weightedhh"] = (
        accessiblejobs_df["bike_20"] * accessiblejobs_df["TOTHH"]
    )
    accessiblejobs_df["walk_20_weightedhh"] = (
        accessiblejobs_df["walk_20"] * accessiblejobs_df["TOTHH"]
    )

    accessiblejobs_df["TOTEMP_weightedhhq1q2"] = accessiblejobs_df["TOTEMP"] * (
        accessiblejobs_df["HHINCQ1"] + accessiblejobs_df["HHINCQ2"]
    )
    accessiblejobs_df["wtrn_45_weightedhhq1q2"] = accessiblejobs_df["wtrn_45"] * (
        accessiblejobs_df["HHINCQ1"] + accessiblejobs_df["HHINCQ2"]
    )
    accessiblejobs_df["wtrn_30_weightedhhq1q2"] = accessiblejobs_df["wtrn_30"] * (
        accessiblejobs_df["HHINCQ1"] + accessiblejobs_df["HHINCQ2"]
    )
    accessiblejobs_df["da_30_weightedhhq1q2"] = accessiblejobs_df["da_30"] * (
        accessiblejobs_df["HHINCQ1"] + accessiblejobs_df["HHINCQ2"]
    )
    accessiblejobs_df["dat_30_weightedhhq1q2"] = accessiblejobs_df["dat_30"] * (
        accessiblejobs_df["HHINCQ1"] + accessiblejobs_df["HHINCQ2"]
    )
    accessiblejobs_df["bike_20_weightedhhq1q2"] = accessiblejobs_df["bike_20"] * (
        accessiblejobs_df["HHINCQ1"] + accessiblejobs_df["HHINCQ2"]
    )
    accessiblejobs_df["walk_20_weightedhhq1q2"] = accessiblejobs_df["walk_20"] * (
        accessiblejobs_df["HHINCQ1"] + accessiblejobs_df["HHINCQ2"]
    )

    accessiblejobs_df["TOTEMP_weightedhhq3q4"] = accessiblejobs_df["TOTEMP"] * (
        accessiblejobs_df["HHINCQ3"] + accessiblejobs_df["HHINCQ4"]
    )
    accessiblejobs_df["wtrn_45_weightedhhq3q4"] = accessiblejobs_df["wtrn_45"] * (
        accessiblejobs_df["HHINCQ3"] + accessiblejobs_df["HHINCQ4"]
    )
    accessiblejobs_df["wtrn_30_weightedhhq3q4"] = accessiblejobs_df["wtrn_30"] * (
        accessiblejobs_df["HHINCQ3"] + accessiblejobs_df["HHINCQ4"]
    )
    accessiblejobs_df["da_30_weightedhhq3q4"] = accessiblejobs_df["da_30"] * (
        accessiblejobs_df["HHINCQ3"] + accessiblejobs_df["HHINCQ4"]
    )
    accessiblejobs_df["dat_30_weightedhhq3q4"] = accessiblejobs_df["dat_30"] * (
        accessiblejobs_df["HHINCQ3"] + accessiblejobs_df["HHINCQ4"]
    )
    accessiblejobs_df["bike_20_weightedhhq3q4"] = accessiblejobs_df["bike_20"] * (
        accessiblejobs_df["HHINCQ3"] + accessiblejobs_df["HHINCQ4"]
    )
    accessiblejobs_df["walk_20_weightedhhq3q4"] = accessiblejobs_df["walk_20"] * (
        accessiblejobs_df["HHINCQ3"] + accessiblejobs_df["HHINCQ4"]
    )

    for hhsuffix in ["", "q1q2", "q3q4"]:
        metrics_dict[
            "jobacc2_acc_jobs_weighted_hh{}".format(hhsuffix)
        ] = accessiblejobs_df["TOTEMP_weightedhh{}".format(hhsuffix)].sum()
        metrics_dict[
            "jobacc2_wtrn_45_acc_jobs_weighted_hh{}".format(hhsuffix)
        ] = accessiblejobs_df["wtrn_45_weightedhh{}".format(hhsuffix)].sum()
        metrics_dict[
            "jobacc2_wtrn_30_acc_jobs_weighted_hh{}".format(hhsuffix)
        ] = accessiblejobs_df["wtrn_30_weightedhh{}".format(hhsuffix)].sum()
        metrics_dict[
            "jobacc2_da_30_acc_jobs_weighted_hh{}".format(hhsuffix)
        ] = accessiblejobs_df["da_30_weightedhh{}".format(hhsuffix)].sum()
        metrics_dict[
            "jobacc2_dat_30_acc_jobs_weighted_hh{}".format(hhsuffix)
        ] = accessiblejobs_df["dat_30_weightedhh{}".format(hhsuffix)].sum()
        metrics_dict[
            "jobacc2_bike_20_acc_jobs_weighted_hh{}".format(hhsuffix)
        ] = accessiblejobs_df["bike_20_weightedhh{}".format(hhsuffix)].sum()
        metrics_dict[
            "jobacc2_walk_20_acc_jobs_weighted_hh{}".format(hhsuffix)
        ] = accessiblejobs_df["walk_20_weightedhh{}".format(hhsuffix)].sum()

        metrics_dict[
            "jobacc2_total_jobs_weighted_hh{}".format(hhsuffix)
        ] = metrics_dict["jobacc2_acc_jobs_weighted_hh{}".format(hhsuffix)]

        metrics_dict[
            "jobacc2_wtrn_45_acc_accessible_job_share_hh{}".format(hhsuffix)
        ] = float(
            metrics_dict["jobacc2_wtrn_45_acc_jobs_weighted_hh{}".format(hhsuffix)]
        ) / float(
            metrics_dict["jobacc2_total_jobs_weighted_hh{}".format(hhsuffix)]
        )
        metrics_dict[
            "jobacc2_wtrn_30_acc_accessible_job_share_hh{}".format(hhsuffix)
        ] = float(
            metrics_dict["jobacc2_wtrn_30_acc_jobs_weighted_hh{}".format(hhsuffix)]
        ) / float(
            metrics_dict["jobacc2_total_jobs_weighted_hh{}".format(hhsuffix)]
        )
        metrics_dict[
            "jobacc2_da_30_acc_accessible_job_share_hh{}".format(hhsuffix)
        ] = float(
            metrics_dict["jobacc2_da_30_acc_jobs_weighted_hh{}".format(hhsuffix)]
        ) / float(
            metrics_dict["jobacc2_total_jobs_weighted_hh{}".format(hhsuffix)]
        )
        metrics_dict[
            "jobacc2_dat_30_acc_accessible_job_share_hh{}".format(hhsuffix)
        ] = float(
            metrics_dict["jobacc2_dat_30_acc_jobs_weighted_hh{}".format(hhsuffix)]
        ) / float(
            metrics_dict["jobacc2_total_jobs_weighted_hh{}".format(hhsuffix)]
        )
        metrics_dict[
            "jobacc2_bike_20_acc_accessible_job_share_hh{}".format(hhsuffix)
        ] = float(
            metrics_dict["jobacc2_bike_20_acc_jobs_weighted_hh{}".format(hhsuffix)]
        ) / float(
            metrics_dict["jobacc2_total_jobs_weighted_hh{}".format(hhsuffix)]
        )
        metrics_dict[
            "jobacc2_walk_20_acc_accessible_job_share_hh{}".format(hhsuffix)
        ] = float(
            metrics_dict["jobacc2_walk_20_acc_jobs_weighted_hh{}".format(hhsuffix)]
        ) / float(
            metrics_dict["jobacc2_total_jobs_weighted_hh{}".format(hhsuffix)]
        )
...
def tally_goods_movement_delay(iteration, sampleshare, metrics_dict):
    ...
    * goods_delay_vhd_per_person: goods_delay_vehicle_hours/goods_delay_total_pop
    ...
    roadvols_df = pandas.read_csv(
        os.path.join("hwy", "iter%d" % iteration, "avgload5period_vehclasses.csv"),
        sep=",",
    )
    tazdata_df = pandas.read_csv(os.path.join("landuse", "tazData.csv"), sep=",")
    ...
    for timeperiod in ["EA", "AM", "MD", "PM", "EV"]:
        ...
        roadvols_df["vhd_%s" % timeperiod] = (
            roadvols_df["vol%s_tot" % timeperiod]
            * (roadvols_df["ctim%s" % timeperiod] - roadvols_df["fft"])
            / 60.0
        )
    ...
    metrics_dict["goods_delay_vhd_per_person"] = total_vehicle_hours_delay / float(
        tazdata_df["TOTPOP"].sum()
    )
    ...
    * nonauto_mode_share               : total nonauto trips / total trips
    ...
    for trip_type in ["indiv", "joint"]:
        ...
        if trip_type == "indiv":
            ...
            temp_trips_df["num_participants"] = (
                temp_trips_df["num_participants"] / sampleshare
            )
            trips_df = pandas.concat([trips_df, temp_trips_df], axis=0, sort=True)

    metrics_dict["nonauto_mode_share_walk_trips"] = trips_df.loc[
        trips_df["trip_mode"] == 7
    ].num_participants.sum()
    metrics_dict["nonauto_mode_share_bike_trips"] = trips_df.loc[
        trips_df["trip_mode"] == 8
    ].num_participants.sum()
    metrics_dict["nonauto_mode_share_transit_trips"] = trips_df.loc[
        trips_df["trip_mode"] >= 9
    ].num_participants.sum()
    metrics_dict["nonauto_mode_share_nonauto_trips"] = trips_df.loc[
        trips_df["trip_mode"] >= 7
    ].num_participants.sum()
    metrics_dict["nonauto_mode_share_total_trips"] = trips_df.num_participants.sum()

    metrics_dict["nonauto_mode_share_walk"] = float(
        metrics_dict["nonauto_mode_share_walk_trips"]
    ) / float(metrics_dict["nonauto_mode_share_total_trips"])
    metrics_dict["nonauto_mode_share_bike"] = float(
        metrics_dict["nonauto_mode_share_bike_trips"]
    ) / float(metrics_dict["nonauto_mode_share_total_trips"])
    metrics_dict["nonauto_mode_share_transit"] = float(
        metrics_dict["nonauto_mode_share_transit_trips"]
    ) / float(metrics_dict["nonauto_mode_share_total_trips"])
    metrics_dict["nonauto_mode_share"] = float(
        metrics_dict["nonauto_mode_share_nonauto_trips"]
    ) / float(metrics_dict["nonauto_mode_share_total_trips"])
...
def tally_road_cost_vmt(iteration, sampleshare, metrics_dict):
    ...
    * road_vmt_lrtr                  : VMT by large trucks
    ...
    for timeperiod in ["EA", "AM", "MD", "PM", "EV"]:
        ...
        roadvols_df["vol%s_auto" % timeperiod] = (
            roadvols_df["vol%s_da" % timeperiod]
            + roadvols_df["vol%s_s2" % timeperiod]
            + roadvols_df["vol%s_s3" % timeperiod]
            + roadvols_df["vol%s_dat" % timeperiod]
            + roadvols_df["vol%s_s2t" % timeperiod]
            + roadvols_df["vol%s_s3t" % timeperiod]
        )
        # vmt
        roadvols_df["vmt%s_auto" % timeperiod] = (
            roadvols_df["vol%s_auto" % timeperiod] * roadvols_df["distance"]
        )
        roadvols_df["vmt%s_smtr" % timeperiod] = roadvols_df["distance"] * (
            roadvols_df["vol%s_sm" % timeperiod] + roadvols_df["vol%s_smt" % timeperiod]
        )
        roadvols_df["vmt%s_lrtr" % timeperiod] = roadvols_df["distance"] * (
            roadvols_df["vol%s_hv" % timeperiod] + roadvols_df["vol%s_hvt" % timeperiod]
        )

        # auto opcost in $2000
        roadvols_df["autoopc_2000$_%s" % timeperiod] = (
            0.01 * roadvols_df["autoopc"] * roadvols_df["vmt%s_auto" % timeperiod]
        )
        # small truck opcost in $2000
        roadvols_df["smtropc_2000$_%s" % timeperiod] = (
            0.01 * roadvols_df["smtropc"] * roadvols_df["vmt%s_smtr" % timeperiod]
        )
        # large truck opcost in $2000
        roadvols_df["lrtropc_2000$_%s" % timeperiod] = (
            0.01 * roadvols_df["lrtropc"] * roadvols_df["vmt%s_lrtr" % timeperiod]
        )
