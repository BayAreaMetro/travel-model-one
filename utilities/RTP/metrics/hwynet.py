...
parser.add_argument(
    "--filter",
    metavar="lookup_filter",
    help="Filter keyword for lookup files",
    required=True,
)
parser.add_argument(
    "--year",
...
if args.link_mapping:
    ...
    link_mapping_df = link_mapping_df[
        ["A", "B", args.link_mapping[1], args.link_mapping[2]]
    ]
    # rename A,B to a,b
    link_mapping_df.rename(columns={"A": "a", "B": "b"}, inplace=True)
    print(
        "link_mapping_df read from {}; head:\n{}".format(
            args.link_mapping[0], link_mapping_df.head()
        )
    )
    print(
        "link_mapping_df.{}.sum()={}".format(
            args.link_mapping[2], link_mapping_df[args.link_mapping[2]].sum()
        )
    )
...
link_tp_df = pandas.merge(
    how="left", left=link_tp_df, right=link_df[id_fields + fields], on=id_fields
)
...
tp_vc_field_df = pandas.DataFrame(
    {"timeperiod": timeperiod_list, "vclass": vclass_list, "variable": tp_vc_field_list}
)

# do the melt and join with our decoder
link_tp_vehclass_df = pandas.melt(
    link_df, id_vars=id_fields, value_vars=tp_vc_field_list
)
link_tp_vehclass_df = pandas.merge(
    how="left", left=link_tp_vehclass_df, right=tp_vc_field_df
)
link_tp_vehclass_df = link_tp_vehclass_df.rename(columns={"value": "vol"}).drop(
    columns="variable"
)
print(
    "Melted to timeperiods x vehclasses to createlink_tp_vehclass_df: {} rows".format(
        len(link_tp_vehclass_df)
    )
)
...
nrclookup_df = nrclookup_df.loc[
    (nrclookup_df["filter"] == args.filter) & (nrclookup_df["year"] == args.year)
].copy()
nrclookup_df["vcratio"] = nrclookup_df["vcratio"].map(
    "{:,.2f}".format
...
nrclookup_df = (
    nrclookup_df.stack()
    .reset_index()
    .rename(columns={"level_1": "lanes_24", 0: "nrcdelay"})
)
print(nrclookup_df.head())
print(
    "Read {} and filtered by {} and year {} to create nrclookup_df:\n{}".format(
        nrc_file, args.filter, args.year, nrclookup_df.head()
    )
)
...
# filter by given filter
collisionlookup_df = collisionlookup_df.loc[
    (collisionlookup_df["filter"] == args.filter)
    & (collisionlookup_df["year"] == args.year)
].copy()
...
print(
    "Read {} and filtered by {} and year {} to create collisionlookup_df:\n{}".format(
        collision_file, args.filter, args.year, collisionlookup_df.head()
    )
)
...
if not os.path.exists(emission_file):
    ...
    # filter by given filter
    emissionslookup_df = emissionslookup_df.loc[
        (emissionslookup_df["filter"] == args.filter)
        & (emissionslookup_df["year"] == args.year)
    ].copy()
    if len(emissionslookup_df) == 0:
        ...
        print(
            "Read {} and filtered by {} and year {} to create emissionslookup_df:\n{}".format(
                emission_file, args.filter, args.year, emissionslookup_df.head()
            )
        )
...
link_tp_df["vcratio"] = link_tp_df["vcratio"].map(
    "{:,.2f}".format
)  # convert to string with two decimals

# join to get non recurring delay per vmt lookup
link_tp_df = pandas.merge(
    left=link_tp_df, right=nrclookup_df, on=["vcratio", "lanes_24"]
).rename(columns={"nrcdelay": "nrcdelay_pervmt"})
...
link_tp_df.loc[
    link_tp_df.ft == 1, "collision_ft"
] = 2  # Freeway-to-freeway connector is like a freeway
link_tp_df.loc[
    link_tp_df.ft == 8, "collision_ft"
] = 2  # managed freeway is like a freeway
...
link_tp_df = pandas.merge(
    how="left",
    left=link_tp_df,
    right=collisionlookup_df.rename(
        columns={"ft": "collision_ft", "at": "collision_at"}
    ),
...
link_tp_vehclass_df = pandas.merge(
    left=link_tp_vehclass_df,
    right=link_tp_df,
    how="left",
    on=id_fields + ["timeperiod"],
)

# calculate VMT, VHT and Hypothetical FreeFlow Time
link_tp_vehclass_df["vmt"] = (
    link_tp_vehclass_df["vol"] * link_tp_vehclass_df["distance"]
)
link_tp_vehclass_df["vht"] = (
    link_tp_vehclass_df["vol"] * link_tp_vehclass_df["ctim"] / 60.0
)
link_tp_vehclass_df["hypfft"] = (
    link_tp_vehclass_df["vol"] * link_tp_vehclass_df["fft"] / 60.0
)
...
link_tp_vehclass_df["nrcdelay"] = (
    link_tp_vehclass_df["nrcdelay_pervmt"] * link_tp_vehclass_df["vmt"]
)
# so zero out non-freeway
link_tp_vehclass_df.loc[~link_tp_vehclass_df.ft.isin([1, 2, 8]), "nrcdelay"] = 0.0
print(
    "c link_tp_vehclass_df len {}:\n{}".format(
        len(link_tp_vehclass_df), link_tp_vehclass_df.head()
    )
)

# collisionlookup in collisions per 1000000 VMT
for collision_type in collision_types:
    link_tp_vehclass_df[collision_type] = (
        link_tp_vehclass_df[collision_type] * link_tp_vehclass_df["vmt"]
    )

# join to vehicle class group and then emission lookup
vclassgroup_df = pandas.DataFrame.from_dict(
    vclassgroup, orient="index", columns=["vclassgroup"]
)
vclassgroup_df = vclassgroup_df.reset_index(drop=False).rename(
    columns={"index": "vclass"}
)
vclassgroup_df["vclass"] = vclassgroup_df["vclass"].str.lower()

link_tp_vehclass_df = pandas.merge(
    how="left", left=link_tp_vehclass_df, right=vclassgroup_df
)
...
if len(emissionslookup_df) > 0:
    ...
    for emission_type in emission_types:
        link_tp_vehclass_df[emission_type] = (
            link_tp_vehclass_df[emission_type] * link_tp_vehclass_df["vmt"]
        )

print(
    "d link_tp_vehclass_df len {}:\n{}".format(
        len(link_tp_vehclass_df), link_tp_vehclass_df.head()
    )
)

aggregate_dict = {"vmt": "sum", "vht": "sum", "hypfft": "sum", "nrcdelay": "sum"}
for collision_type in collision_types:
    aggregate_dict[collision_type] = "sum"
if len(emissionslookup_df) > 0:
    for emission_type in emission_types:
        aggregate_dict[emission_type] = "sum"

# aggregate to timeperiod x vclass
metrics_df = (
    link_tp_vehclass_df.groupby(["timeperiod", "vclass"])
...
if args.link_mapping:
    link_tp_vehclass_mapped_df = link_tp_vehclass_df.merge(
        link_mapping_df, on=["a", "b"], how="outer", indicator=True
    )
    print(
        "Merged with link_mapping_df to create link_tp_vehclass_mapped_df with {} rows; head:\n{}".format(
    ...
    for metric_name in aggregate_dict.keys():
        ...
        link_tp_vehclass_mapped_df[metric_name] = (
            link_tp_vehclass_mapped_df[metric_name]
            * link_tp_vehclass_mapped_df[share_col]
        )
    ...
    link_tp_vehclass_mapped_df.loc[
        link_tp_vehclass_mapped_df["ft"].isin([1, 2, 3, 8]), "road_type"
    ] = "freeway"
    ...
    link_tp_vehclass_mapped_df.loc[
        link_tp_vehclass_mapped_df["at"] <= 3, "area_type"
    ] = "urban"
    link_tp_vehclass_mapped_df.loc[
        link_tp_vehclass_mapped_df["at"] == 4, "area_type"
    ] = "suburban"
    link_tp_vehclass_mapped_df.loc[
        link_tp_vehclass_mapped_df["at"] == 5, "area_type"
    ] = "rural"

    # and aggregate to timeperiod x vclass x road_type x area_type x indexcol
    metrics_mapped_df = (
        link_tp_vehclass_mapped_df.groupby(
            ["timeperiod", "vclass", "road_type", "area_type", index_col]
        )
        .aggregate(aggregate_dict)
        .reset_index()
    )
    print(
        "  - aggregated to timeperiod, vclass, road_type, area_type, {}; head:\n{}".format(
            index_col, metrics_mapped_df.head(10)
        )
    )
...
for collision_type in collision_types:
    ...
    if len(metrics_mapped_df) > 0:
        metrics_mapped_df[collision_type] = (
            metrics_mapped_df[collision_type] / 1000000.0
        )
...
if len(emissionslookup_df) > 0:
    for emission_type in emission_types:
        ...
        # ditto for mapped
        if len(metrics_mapped_df) > 0:
            metrics_mapped_df[emission_type] = (
                metrics_mapped_df[emission_type] / 1000000.0
            )
...
if len(metrics_mapped_df) > 0:
    ...
    vmt_vht_outputfile = vmt_vht_outputfile.replace(
        ".csv", "{}.csv".format(output_suffix)
    )
