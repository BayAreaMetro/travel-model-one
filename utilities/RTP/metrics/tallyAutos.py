...
if __name__ == "__main__":
    ...
    households = pd.read_table(
        os.path.join("main", "householdData_%d.csv" % iteration), sep=",", index_col=[0]
    )
    ...
    households.loc[
        (households["income"] >= 30000) & (households["income"] < 60000), "incQ"
    ] = 2
    households.loc[
        (households["income"] >= 60000) & (households["income"] < 100000), "incQ"
    ] = 3
    ...
    autos_by_inc.to_csv(
        os.path.join("metrics", "autos_owned.csv"), header=True, index=True
    )
