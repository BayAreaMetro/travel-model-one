...
def calculate_urbansim_highlevelmetrics(
    ...
    # all households
    metrics_dict[runid, metric_id, "TotHH_region", y2, dbp] = parcel_sum_df[
        "tothh_2050"
    ].sum()
    metrics_dict[runid, metric_id, "TotHH_region", y1, dbp] = parcel_sum_df[
        "tothh_2015"
    ].sum()
    ...
    for index, row in county_sum_df.iterrows():
        metrics_dict[
            runid, metric_id, "TotHH_county_growth_%s" % row["county"], y_diff, dbp
        ] = row["tothh_growth"]
        metrics_dict[
            runid,
            metric_id,
            "TotHH_county_shareofgrowth_%s" % row["county"],
            y_diff,
            dbp,
        ] = (
            row["tothh_growth"]
    ...
    metrics_dict[runid, metric_id, "TotHH_GG", y2, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("GG", na=False), "tothh_2050"
    ].sum()
    metrics_dict[runid, metric_id, "TotHH_GG", y1, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("GG", na=False), "tothh_2015"
    ].sum()
    ...
    # HH Growth in PDAs
    metrics_dict[runid, metric_id, "TotHH_PDA", y2, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pda_id"].str.contains("", na=False), "tothh_2050"
    ].sum()
    metrics_dict[runid, metric_id, "TotHH_PDA", y1, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pda_id"].str.contains("", na=False), "tothh_2015"
    ].sum()
    ...
    metrics_dict[runid, metric_id, "TotHH_GG_notPDA", y2, dbp] = parcel_sum_df.loc[
        (parcel_sum_df["pba50chcat"].str.contains("GG", na=False))
        & (parcel_sum_df["pda_id"].str.contains("", na=False) == 0),
        "tothh_2050",
    ].sum()
    metrics_dict[runid, metric_id, "TotHH_GG_notPDA", y1, dbp] = parcel_sum_df.loc[
        (parcel_sum_df["pba50chcat"].str.contains("GG", na=False))
        & (parcel_sum_df["pda_id"].str.contains("", na=False) == 0),
        "tothh_2015",
    ].sum()
    ...
    # HH Growth in HRAs
    metrics_dict[runid, metric_id, "TotHH_HRA", y2, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("HRA", na=False), "tothh_2050"
    ].sum()
    metrics_dict[runid, metric_id, "TotHH_HRA", y1, dbp] = parcel_sum_df.loc[
    ...
    # HH Growth in TRAs
    metrics_dict[runid, metric_id, "TotHH_TRA", y2, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("tra", na=False), "tothh_2050"
    ].sum()
    metrics_dict[runid, metric_id, "TotHH_TRA", y1, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("tra", na=False), "tothh_2015"
    ].sum()
    ...
    metrics_dict[runid, metric_id, "TotHH_HRAandTRA", y2, dbp] = parcel_sum_df.loc[
        (parcel_sum_df["pba50chcat"].str.contains("HRA", na=False))
        & (parcel_sum_df["pba50chcat"].str.contains("tra", na=False)),
        "tothh_2050",
    ].sum()
    metrics_dict[runid, metric_id, "TotHH_HRAandTRA", y1, dbp] = parcel_sum_df.loc[
        (parcel_sum_df["pba50chcat"].str.contains("HRA", na=False))
        & (parcel_sum_df["pba50chcat"].str.contains("tra", na=False)),
        "tothh_2015",
    ].sum()
    ...
    # all jobs
    metrics_dict[runid, metric_id, "TotJobs_region", y2, dbp] = parcel_sum_df[
        "totemp_2050"
    ].sum()
    metrics_dict[runid, metric_id, "TotJobs_region", y1, dbp] = parcel_sum_df[
        "totemp_2015"
    ].sum()
    ...
    for index, row in county_sum_df.iterrows():
        metrics_dict[
            runid, metric_id, "TotJobs_growth_%s" % row["county"], y_diff, dbp
        ] = row["totemp_growth"]
        metrics_dict[
            runid,
            metric_id,
            "TotJobs_county_shareofgrowth_%s" % row["county"],
            y_diff,
            dbp,
        ] = (
            row["totemp_growth"]
            / metrics_dict[
                runid, metric_id, "TotJobs_growth_region_number", y_diff, dbp
            ]
        )

    # Job Growth in all GGs
    metrics_dict[runid, metric_id, "TotJobs_GG", y2, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("GG", na=False), "totemp_2050"
    ].sum()
    metrics_dict[runid, metric_id, "TotJobs_GG", y1, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("GG", na=False), "totemp_2015"
    ].sum()
    ...
    # Job Growth in PDAs
    metrics_dict[runid, metric_id, "TotJobs_PDA", y2, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pda_id"].str.contains("", na=False), "totemp_2050"
    ].sum()
    metrics_dict[runid, metric_id, "TotJobs_PDA", y1, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pda_id"].str.contains("", na=False), "totemp_2015"
    ].sum()
    ...
    metrics_dict[runid, metric_id, "TotJobs_GG_notPDA", y2, dbp] = parcel_sum_df.loc[
        (parcel_sum_df["pba50chcat"].str.contains("GG", na=False))
        & (parcel_sum_df["pda_id"].str.contains("", na=False) == 0),
        "totemp_2050",
    ].sum()
    metrics_dict[runid, metric_id, "TotJobs_GG_notPDA", y1, dbp] = parcel_sum_df.loc[
        (parcel_sum_df["pba50chcat"].str.contains("GG", na=False))
        & (parcel_sum_df["pda_id"].str.contains("", na=False) == 0),
        "totemp_2015",
    ].sum()
    ...
    # Job Growth in HRAs
    metrics_dict[runid, metric_id, "TotJobs_HRA", y2, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("HRA", na=False), "totemp_2050"
    ].sum()
    metrics_dict[runid, metric_id, "TotJobs_HRA", y1, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("HRA", na=False), "totemp_2015"
    ].sum()
    ...
    # Job Growth in TRAs
    metrics_dict[runid, metric_id, "TotJobs_TRA", y2, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("tra", na=False), "totemp_2050"
    ].sum()
    metrics_dict[runid, metric_id, "TotJobs_TRA", y1, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("tra", na=False), "totemp_2015"
    ].sum()
    ...
    metrics_dict[runid, metric_id, "TotJobs_HRAandTRA", y2, dbp] = parcel_sum_df.loc[
        (parcel_sum_df["pba50chcat"].str.contains("HRA", na=False))
        & (parcel_sum_df["pba50chcat"].str.contains("tra", na=False)),
        "totemp_2050",
    ].sum()
    metrics_dict[runid, metric_id, "TotJobs_HRAandTRA", y1, dbp] = parcel_sum_df.loc[
        (parcel_sum_df["pba50chcat"].str.contains("HRA", na=False))
        & (parcel_sum_df["pba50chcat"].str.contains("tra", na=False)),
        "totemp_2015",
    ].sum()
...
def calculate_tm_highlevelmetrics(
    runid, dbp, parcel_sum_df, county_sum_df, metrics_dict
):
...
def calculate_Affordable1_transportation_costs(
    runid,
    year,
    dbp,
    tm_scen_metrics_df,
    ...
    tm_tot_hh = tm_scen_metrics_df.loc[
        (
            tm_scen_metrics_df["metric_name"].str.contains("total_households_inc")
            == True
        ),
        "value",
    ].sum()
    tm_tot_hh_inc1 = tm_scen_metrics_df.loc[
        (tm_scen_metrics_df["metric_name"] == "total_households_inc1"), "value"
    ].item()
    tm_tot_hh_inc2 = tm_scen_metrics_df.loc[
        (tm_scen_metrics_df["metric_name"] == "total_households_inc2"), "value"
    ].item()

    # Total household income (model outputs are in 2000$, annual)
    tm_total_hh_inc = tm_scen_metrics_df.loc[
        (tm_scen_metrics_df["metric_name"].str.contains("total_hh_inc") == True),
        "value",
    ].sum()
    tm_total_hh_inc_inc1 = tm_scen_metrics_df.loc[
        (tm_scen_metrics_df["metric_name"] == "total_hh_inc_inc1"), "value"
    ].item()
    tm_total_hh_inc_inc2 = tm_scen_metrics_df.loc[
        (tm_scen_metrics_df["metric_name"] == "total_hh_inc_inc2"), "value"
    ].item()

    # Total transit fares (model outputs are in 2000$, per day)
    tm_tot_transit_fares = (
        tm_scen_metrics_df.loc[
            (
                tm_scen_metrics_df["metric_name"].str.contains("total_transit_fares")
                == True
            ),
            "value",
        ].sum()
    ...
    tm_tot_transit_fares_inc1 = (
        tm_scen_metrics_df.loc[
            (tm_scen_metrics_df["metric_name"] == "total_transit_fares_inc1"), "value"
        ].item()
        * days_per_year
    )
    tm_tot_transit_fares_inc2 = (
        tm_scen_metrics_df.loc[
            (tm_scen_metrics_df["metric_name"] == "total_transit_fares_inc2"), "value"
        ].item()
        * days_per_year
    )
    ...
    tm_tot_auto_op_cost = (
        tm_scen_metrics_df.loc[
            (
                tm_scen_metrics_df["metric_name"].str.contains("total_auto_cost_inc")
                == True
            ),
            "value",
        ].sum()
    ...
    tm_tot_auto_op_cost_inc1 = (
        tm_scen_metrics_df.loc[
            (tm_scen_metrics_df["metric_name"] == "total_auto_cost_inc1"), "value"
        ].item()
        * days_per_year
    )
    tm_tot_auto_op_cost_inc2 = (
        tm_scen_metrics_df.loc[
            (tm_scen_metrics_df["metric_name"] == "total_auto_cost_inc2"), "value"
        ].item()
        * days_per_year
    )
    ...
    tm_tot_auto_park_cost = (
        (tm_travel_cost_df.pcost_indiv.sum() + tm_travel_cost_df.pcost_joint.sum())
        * days_per_year
        / 100
    )
    tm_tot_auto_park_cost_inc1 = (
        (
            tm_travel_cost_df.loc[(tm_travel_cost_df["incQ"] == 1), "pcost_indiv"].sum()
            + tm_travel_cost_df.loc[
                (tm_travel_cost_df["incQ"] == 1), "pcost_joint"
            ].sum()
        )
        * days_per_year
        / 100
    )
    tm_tot_auto_park_cost_inc2 = (
        (
            tm_travel_cost_df.loc[(tm_travel_cost_df["incQ"] == 2), "pcost_indiv"].sum()
            + tm_travel_cost_df.loc[
                (tm_travel_cost_df["incQ"] == 2), "pcost_joint"
            ].sum()
        )
        * days_per_year
        / 100
    ...
    tm_auto_owned_df["tot_autos"] = (
        tm_auto_owned_df["autos"] * tm_auto_owned_df["households"]
    )
    tm_tot_autos_owned = tm_auto_owned_df["tot_autos"].sum()
    tm_tot_autos_owned_inc1 = tm_auto_owned_df.loc[
        (tm_auto_owned_df["incQ"] == 1), "tot_autos"
    ].sum()
    tm_tot_autos_owned_inc2 = tm_auto_owned_df.loc[
        (tm_auto_owned_df["incQ"] == 2), "tot_autos"
    ].sum()

    # Total auto ownership cost in 2000$
    tm_tot_auto_owner_cost = (
        tm_tot_autos_owned * auto_ownership_cost * inflation_18_20 / inflation_00_20
    )
    tm_tot_auto_owner_cost_inc1 = (
        tm_tot_autos_owned_inc1
        * auto_ownership_cost_inc1
        * inflation_18_20
        / inflation_00_20
    )
    tm_tot_auto_owner_cost_inc2 = (
        tm_tot_autos_owned_inc2
        * auto_ownership_cost_inc2
        * inflation_18_20
        / inflation_00_20
    )
    ...
    metrics_dict[
        runid, metric_id, "mean_transportation_cost_2020$", year, dbp
    ] = tp_cost_mean
    metrics_dict[
        runid, metric_id, "mean_transportation_cost_2020$_inc1", year, dbp
    ] = tp_cost_mean_inc1
    metrics_dict[
        runid, metric_id, "mean_transportation_cost_2020$_inc2", year, dbp
    ] = tp_cost_mean_inc2
    ...
    tp_cost_pct_inc_inc1and2 = (tp_cost_inc1 + tp_cost_inc2) / (
        tm_total_hh_inc_inc1 + tm_total_hh_inc_inc2
    )

    # Transportation cost % of income metrics
    metrics_dict[
        runid, metric_id, "transportation_cost_pct_income", year, dbp
    ] = tp_cost_pct_inc
    metrics_dict[
        runid, metric_id, "transportation_cost_pct_income_inc1", year, dbp
    ] = tp_cost_pct_inc_inc1
    metrics_dict[
        runid, metric_id, "transportation_cost_pct_income_inc2", year, dbp
    ] = tp_cost_pct_inc_inc2
    metrics_dict[
        runid, metric_id, "transportation_cost_pct_income_inc1and2", year, dbp
    ] = tp_cost_pct_inc_inc1and2

    # Transportation cost % of income metrics; split by cost bucket
    metrics_dict[
        runid, metric_id, "transportation_cost_pct_income_autoop", year, dbp
    ] = (tm_tot_auto_op_cost / tm_total_hh_inc)
    metrics_dict[
        runid, metric_id, "transportation_cost_pct_income_autopark", year, dbp
    ] = (tm_tot_auto_park_cost / tm_total_hh_inc)
    metrics_dict[
        runid, metric_id, "transportation_cost_pct_income_transitfare", year, dbp
    ] = (tm_tot_transit_fares / tm_total_hh_inc)
    metrics_dict[
        runid, metric_id, "transportation_cost_pct_income_autoown", year, dbp
    ] = (tm_tot_auto_owner_cost / tm_total_hh_inc)

    # Add housing costs from Shimon's outputs
    housing_costs_2050_df = pd.read_csv(
        "C:/Users/ATapase/Box/Horizon and Plan Bay Area 2050/Equity and Performance/7_Analysis/Metrics/metrics_files/2050 Share of Income Spent on Housing.csv"
    )
    housing_costs_2015_df = pd.read_csv(
        "C:/Users/ATapase/Box/Horizon and Plan Bay Area 2050/Equity and Performance/7_Analysis/Metrics/metrics_files/2015 Share of Income Spent on Housing.csv"
    )
    housing_costs_2015_df["totcosts"] = (
        housing_costs_2015_df["share_income"] * housing_costs_2015_df["households"]
    )

    if year == "2050":
        metrics_dict[
            runid, metric_id, "housing_cost_pct_income", year, dbp
        ] = housing_costs_2050_df["w_all"].sum()
        metrics_dict[
            runid, metric_id, "housing_cost_pct_income_inc1", year, dbp
        ] = housing_costs_2050_df["w_q1"].sum()
        metrics_dict[
            runid, metric_id, "housing_cost_pct_income_inc2", year, dbp
        ] = housing_costs_2050_df["w_q2"].sum()
        metrics_dict[
            runid, metric_id, "housing_cost_pct_income_inc1and2", year, dbp
        ] = housing_costs_2050_df["w_q1_q2"].sum()
    elif year == "2015":
        metrics_dict[runid, metric_id, "housing_cost_pct_income", year, dbp] = (
            housing_costs_2015_df.loc[
                (housing_costs_2015_df["tenure"].str.contains("Total")), "totcosts"
            ].sum()
            / housing_costs_2015_df.loc[
                (housing_costs_2015_df["tenure"].str.contains("Total")), "households"
            ].sum()
        )
        metrics_dict[
            runid, metric_id, "housing_cost_pct_income_inc1", year, dbp
        ] = housing_costs_2015_df.loc[
            (housing_costs_2015_df["short_name"].str.contains("q1t")), "share_income"
        ].sum()
        metrics_dict[
            runid, metric_id, "housing_cost_pct_income_inc2", year, dbp
        ] = housing_costs_2015_df.loc[
            (housing_costs_2015_df["short_name"].str.contains("q2t")), "share_income"
        ].sum()
        metrics_dict[
            runid, metric_id, "housing_cost_pct_income_inc1and2", year, dbp
        ] = (
            housing_costs_2015_df.loc[
                (housing_costs_2015_df["short_name"].str.contains("q1t")), "totcosts"
            ].sum()
            + housing_costs_2015_df.loc[
                (housing_costs_2015_df["short_name"].str.contains("q2t")), "totcosts"
            ].sum()
        ) / (
            housing_costs_2015_df.loc[
                (housing_costs_2015_df["short_name"].str.contains("q1t")), "households"
            ].sum()
            + housing_costs_2015_df.loc[
                (housing_costs_2015_df["short_name"].str.contains("q2t")), "households"
            ].sum()
        )

    # Total H+T Costs pct of income
    metrics_dict[runid, metric_id, "HplusT_cost_pct_income", year, dbp] = (
        metrics_dict[runid, metric_id, "transportation_cost_pct_income", year, dbp]
        + metrics_dict[runid, metric_id, "housing_cost_pct_income", year, dbp]
    )
    metrics_dict[runid, metric_id, "HplusT_cost_pct_income_inc1", year, dbp] = (
        metrics_dict[runid, metric_id, "transportation_cost_pct_income_inc1", year, dbp]
        + metrics_dict[runid, metric_id, "housing_cost_pct_income_inc1", year, dbp]
    )
    metrics_dict[runid, metric_id, "HplusT_cost_pct_income_inc2", year, dbp] = (
        metrics_dict[runid, metric_id, "transportation_cost_pct_income_inc2", year, dbp]
        + metrics_dict[runid, metric_id, "housing_cost_pct_income_inc2", year, dbp]
    )
    metrics_dict[runid, metric_id, "HplusT_cost_pct_income_inc1and2", year, dbp] = (
        metrics_dict[
            runid, metric_id, "transportation_cost_pct_income_inc1and2", year, dbp
        ]
    ...
    for inc_level in range(1, 5):
        total_tolls["inc%d" % inc_level] = (
            tm_auto_times_df.loc[
                "inc%d" % inc_level, ["Bridge Tolls", "Value Tolls"]
            ].sum()
            / 100
        )  # cents -> dollars
    ...
    metrics_dict[runid, metric_id, "tolls_per_HH", year, dbp] = (
        total_tolls_allHH / tm_tot_hh * inflation_00_20
    )
    metrics_dict[runid, metric_id, "tolls_per_LIHH", year, dbp] = (
        total_tolls_LIHH / (tm_tot_hh_inc1 + tm_tot_hh_inc2) * inflation_00_20
    )
    metrics_dict[runid, metric_id, "tolls_per_inc1HH", year, dbp] = (
        total_tolls["inc1"] / tm_tot_hh_inc1 * inflation_00_20
    )
    ...
    metrics_dict[runid, metric_id, "fares_per_HH", year, dbp] = (
        tm_tot_transit_fares / tm_tot_hh * inflation_00_20 / days_per_year
    )
    metrics_dict[runid, metric_id, "fares_per_LIHH", year, dbp] = (
        (tm_tot_transit_fares_inc1 + tm_tot_transit_fares_inc2)
        / (tm_tot_hh_inc1 + tm_tot_hh_inc2)
        * inflation_00_20
        / days_per_year
    )
    metrics_dict[runid, metric_id, "fares_per_inc1HH", year, dbp] = (
        tm_tot_transit_fares_inc1 / tm_tot_hh_inc1 * inflation_00_20 / days_per_year
    )

    # per trip

    # Total auto trips per day (model outputs are in trips, per day)
    tm_tot_auto_trips = tm_scen_metrics_df.loc[
        (tm_scen_metrics_df["metric_name"].str.contains("total_auto_trips") == True),
        "value",
    ].sum()
    tm_tot_auto_trips_inc1 = tm_scen_metrics_df.loc[
        (tm_scen_metrics_df["metric_name"] == "total_auto_trips_inc1"), "value"
    ].item()
    tm_tot_auto_trips_inc2 = tm_scen_metrics_df.loc[
        (tm_scen_metrics_df["metric_name"] == "total_auto_trips_inc2"), "value"
    ].item()
    # Total transit trips per day (model outputs are in trips, per day)
    tm_tot_transit_trips = tm_scen_metrics_df.loc[
        (tm_scen_metrics_df["metric_name"].str.contains("total_transit_trips") == True),
        "value",
    ].sum()
    tm_tot_transit_trips_inc1 = tm_scen_metrics_df.loc[
        (tm_scen_metrics_df["metric_name"] == "total_transit_trips_inc1"), "value"
    ].item()
    tm_tot_transit_trips_inc2 = tm_scen_metrics_df.loc[
        (tm_scen_metrics_df["metric_name"] == "total_transit_trips_inc2"), "value"
    ].item()

    # Average Tolls per trip  (total_tolls_xx is calculated above as per day tolls in 2000 dollars)
    metrics_dict[runid, metric_id, "tolls_per_trip", year, dbp] = (
        total_tolls_allHH / tm_tot_auto_trips * inflation_00_20
    )
    metrics_dict[runid, metric_id, "tolls_per_trip_inc1and2", year, dbp] = (
        total_tolls_LIHH
        / (tm_tot_auto_trips_inc1 + tm_tot_auto_trips_inc2)
        * inflation_00_20
    )
    metrics_dict[runid, metric_id, "tolls_per_trip_inc1", year, dbp] = (
        total_tolls["inc1"] / tm_tot_auto_trips_inc1 * inflation_00_20
    )
    ...
    metrics_dict[runid, metric_id, "autocost_per_trip", year, dbp] = (
        (tm_tot_auto_op_cost + tm_tot_auto_park_cost)
        / tm_tot_auto_trips
        * inflation_00_20
        / days_per_year
    )
    metrics_dict[runid, metric_id, "autocost_per_trip_inc1and2", year, dbp] = (
        (
            tm_tot_auto_op_cost_inc1
            + tm_tot_auto_op_cost_inc2
            + tm_tot_auto_park_cost_inc1
    ...
    metrics_dict[runid, metric_id, "autocost_per_trip_inc1", year, dbp] = (
        (tm_tot_auto_op_cost_inc1 + tm_tot_auto_park_cost_inc1)
        / tm_tot_auto_trips_inc1
        * inflation_00_20
        / days_per_year
    )

    # Average Fares per trip   (note: transit fares totals calculated above are annual and need to be divided by days_per_year)
    metrics_dict[runid, metric_id, "fares_per_trip", year, dbp] = (
        tm_tot_transit_fares / tm_tot_transit_trips * inflation_00_20 / days_per_year
    )
    metrics_dict[runid, metric_id, "fares_per_trip_inc1and2", year, dbp] = (
        (tm_tot_transit_fares_inc1 + tm_tot_transit_fares_inc2)
        / (tm_tot_transit_trips_inc1 + tm_tot_transit_trips_inc2)
        * inflation_00_20
        / days_per_year
    )
    metrics_dict[runid, metric_id, "fares_per_trip_inc1", year, dbp] = (
        tm_tot_transit_fares_inc1
        / tm_tot_transit_trips_inc1
        * inflation_00_20
        / days_per_year
    )
...
def calculate_Affordable2_deed_restricted_housing(
    ...
    metrics_dict[runid, metric_id, "deed_restricted_TRA", y2, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("tra", na=False),
        "deed_restricted_units_2050",
    ].sum()
    metrics_dict[runid, metric_id, "deed_restricted_TRA", y1, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("tra", na=False),
        "deed_restricted_units_2015",
    ].sum()
    metrics_dict[
        runid, metric_id, "residential_units_TRA", y2, dbp
    ] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("tra", na=False),
        "residential_units_2050",
    ].sum()
    metrics_dict[
        runid, metric_id, "residential_units_TRA", y1, dbp
    ] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("tra", na=False),
        "residential_units_2015",
    ].sum()
    metrics_dict[runid, metric_id, "deed_restricted_CoC", y2, dbp] = parcel_sum_df.loc[
        parcel_sum_df["coc_flag_pba2050"] == 1, "deed_restricted_units_2050"
    ].sum()
    metrics_dict[runid, metric_id, "deed_restricted_CoC", y1, dbp] = parcel_sum_df.loc[
        parcel_sum_df["coc_flag_pba2050"] == 1, "deed_restricted_units_2015"
    ].sum()
    metrics_dict[
        runid, metric_id, "residential_units_CoC", y2, dbp
    ] = parcel_sum_df.loc[
        parcel_sum_df["coc_flag_pba2050"] == 1, "residential_units_2050"
    ].sum()
    metrics_dict[
        runid, metric_id, "residential_units_CoC", y1, dbp
    ] = parcel_sum_df.loc[
        parcel_sum_df["coc_flag_pba2050"] == 1, "residential_units_2015"
    ].sum()
    ...
    metrics_dict[runid, metric_id, "deed_restricted_CoC_diff", y_diff, dbp] = (
        metrics_dict[runid, metric_id, "deed_restricted_CoC", y2, dbp]
        - metrics_dict[runid, metric_id, "deed_restricted_CoC", y1, dbp]
    )
    metrics_dict[runid, metric_id, "residential_units_CoC_diff", y_diff, dbp] = (
        metrics_dict[runid, metric_id, "residential_units_CoC", y2, dbp]
        - metrics_dict[runid, metric_id, "residential_units_CoC", y1, dbp]
    )
    ...
    metrics_dict[runid, metric_id, "deed_restricted_pct_new_units_CoC", y_diff, dbp] = (
        metrics_dict[runid, metric_id, "deed_restricted_CoC_diff", y_diff, dbp]
        / metrics_dict[runid, metric_id, "residential_units_CoC_diff", y_diff, dbp]
    )
    ...
    print(
        "DR pct of new units in TRAs %s" % dbp,
        metrics_dict[
            runid, metric_id, "deed_restricted_pct_new_units_TRA", y_diff, dbp
        ],
    )
    print(
...
def calculate_Connected1_accessibility(
    runid, year, dbp, tm_scen_metrics_df, metrics_dict
):
    ...
    metrics_dict[
        runid, metric_id, "pct_jobs_acc_by_allmodes", year, dbp
    ] = tm_scen_metrics_df.loc[
        (tm_scen_metrics_df["metric_name"] == "jobacc_accessible_job_share"), "value"
    ].item()
    metrics_dict[
        runid, metric_id, "pct_jobs_acc_by_allmodes_coc", year, dbp
    ] = tm_scen_metrics_df.loc[
        (tm_scen_metrics_df["metric_name"] == "jobacc_accessible_job_share_coc"),
        "value",
    ].item()
    metrics_dict[
        runid, metric_id, "pct_jobs_acc_by_allmodes_noncoc", year, dbp
    ] = tm_scen_metrics_df.loc[
        (tm_scen_metrics_df["metric_name"] == "jobacc_accessible_job_share_noncoc"),
        "value",
    ].item()
    ...
    metrics_dict[runid, metric_id, "pct_jobs_acc_by_drv_only", year, dbp] = (
        tm_scen_metrics_df.loc[
            (
                tm_scen_metrics_df["metric_name"]
                == "jobacc_drv_only_acc_accessible_job_share"
            ),
            "value",
        ].item()
        + tm_scen_metrics_df.loc[
            (
                tm_scen_metrics_df["metric_name"]
                == "jobacc_trn_drv_acc_accessible_job_share"
            ),
            "value",
        ].item()
    )
    ...
    metrics_dict[runid, metric_id, "pct_jobs_acc_by_drv_only_coc", year, dbp] = (
        tm_scen_metrics_df.loc[
            (
                tm_scen_metrics_df["metric_name"]
                == "jobacc_drv_only_acc_accessible_job_share_coc"
            ),
            "value",
        ].item()
        + tm_scen_metrics_df.loc[
            (
                tm_scen_metrics_df["metric_name"]
                == "jobacc_trn_drv_acc_accessible_job_share_coc"
            ),
            "value",
        ].item()
    )
    metrics_dict[runid, metric_id, "pct_jobs_acc_by_drv_only_noncoc", year, dbp] = (
        tm_scen_metrics_df.loc[
            (
                tm_scen_metrics_df["metric_name"]
                == "jobacc_drv_only_acc_accessible_job_share_noncoc"
            ),
            "value",
        ].item()
        + tm_scen_metrics_df.loc[
            (
                tm_scen_metrics_df["metric_name"]
                == "jobacc_trn_drv_acc_accessible_job_share_noncoc"
            ),
            "value",
        ].item()
    )
    ...
    metrics_dict[runid, metric_id, "pct_jobs_acc_by_trn_only", year, dbp] = (
        tm_scen_metrics_df.loc[
            (
                tm_scen_metrics_df["metric_name"]
                == "jobacc_trn_only_acc_accessible_job_share"
            ),
            "value",
        ].item()
        + tm_scen_metrics_df.loc[
    ...
    metrics_dict[runid, metric_id, "pct_jobs_acc_by_trn_only_coc", year, dbp] = (
        tm_scen_metrics_df.loc[
            (
                tm_scen_metrics_df["metric_name"]
                == "jobacc_trn_only_acc_accessible_job_share_coc"
            ),
            "value",
        ].item()
        + tm_scen_metrics_df.loc[
    ...
    metrics_dict[runid, metric_id, "pct_jobs_acc_by_trn_only_noncoc", year, dbp] = (
        tm_scen_metrics_df.loc[
            (
                tm_scen_metrics_df["metric_name"]
                == "jobacc_trn_only_acc_accessible_job_share_noncoc"
            ),
            "value",
        ].item()
        + tm_scen_metrics_df.loc[
...
def calculate_Connected2_crowding(runid, year, dbp, transit_operator_df, metrics_dict):
    ...
    if "2015" in runid:
        tm_run_location = tm_run_location_ipa
    else:
        tm_run_location = tm_run_location_bp
    tm_crowding_df = pd.read_csv(
        tm_run_location + runid + "/OUTPUT/metrics/transit_crowding_complete.csv"
    )

    tm_crowding_df = tm_crowding_df[
        ["TIME", "SYSTEM", "ABNAMESEQ", "period", "load_standcap", "AB_VOL"]
    ]
    tm_crowding_df = tm_crowding_df.loc[tm_crowding_df["period"] == "AM"]
    tm_crowding_df["time_overcapacity"] = tm_crowding_df.apply(
        lambda row: row["TIME"] if (row["load_standcap"] > 1) else 0, axis=1
    )
    tm_crowding_df["time_crowded"] = tm_crowding_df.apply(
        lambda row: row["TIME"] if (row["load_standcap"] > 0.85) else 0, axis=1
    )
    tm_crowding_df["person_hrs_total"] = (
        tm_crowding_df["TIME"] * tm_crowding_df["AB_VOL"]
    )
    tm_crowding_df["person_hrs_overcap"] = (
        tm_crowding_df["time_overcapacity"] * tm_crowding_df["AB_VOL"]
    )
    tm_crowding_df["person_hrs_crowded"] = (
        tm_crowding_df["time_crowded"] * tm_crowding_df["AB_VOL"]
    )

    tm_crowding_df = pd.merge(
        left=tm_crowding_df,
        right=transit_operator_df,
        left_on="SYSTEM",
        right_on="SYSTEM",
        how="left",
    )
    ...
    system_crowding_df = (
        tm_crowding_df[["person_hrs_total", "person_hrs_overcap", "person_hrs_crowded"]]
        .groupby(tm_crowding_df["operator"])
        .sum()
        .reset_index()
    )
    system_crowding_df["pct_overcapacity"] = (
        system_crowding_df["person_hrs_overcap"]
        / system_crowding_df["person_hrs_total"]
    )
    system_crowding_df["pct_crowded"] = (
        system_crowding_df["person_hrs_crowded"]
        / system_crowding_df["person_hrs_total"]
    )
    ...
    for index, row in system_crowding_df.iterrows():
        if row["operator"] in [
            ...
            metrics_dict[
                runid,
                metric_id,
                "crowded_pct_personhrs_AM_%s" % row["operator"],
                year,
                dbp,
            ] = row["pct_crowded"]


def calculate_Connected2_hwy_traveltimes(
    runid, year, dbp, hwy_corridor_links_df, metrics_dict
):
    ...
    if "2015" in runid:
        ...
        tm_run_location = tm_run_location_bp
    tm_loaded_network_df = pd.read_csv(
        tm_run_location + runid + "/OUTPUT/avgload5period.csv"
    )
    ...
    tm_loaded_network_df["link"] = (
        tm_loaded_network_df["a"].astype(str)
        + "_"
        + tm_loaded_network_df["b"].astype(str)
    )

    # merging df that has the list of all
    hwy_corridor_links_df = pd.merge(
        left=hwy_corridor_links_df,
        right=tm_loaded_network_df,
        left_on="link",
        right_on="link",
        how="left",
    )
    corridor_travel_times_df = (
        hwy_corridor_links_df[["distance", "fft", "ctimAM"]]
        .groupby(hwy_corridor_links_df["route"])
        .sum()
        .reset_index()
    )
    ...
    for index, row in corridor_travel_times_df.iterrows():
        metrics_dict[
            runid, metric_id, "travel_time_AM_%s" % row["route"], year, dbp
        ] = row["ctimAM"]


def calculate_Connected2_trn_traveltimes(
    runid, year, dbp, transit_operator_df, metrics_dict
):
    ...
    trn_operator_travel_times_df = (
        tm_trn_line_df[["line time", "line dist"]]
        .groupby(tm_trn_line_df["operator"])
        .sum()
        .reset_index()
    )
    trn_operator_travel_times_df["time_per_dist_AM"] = (
        trn_operator_travel_times_df["line time"]
        / trn_operator_travel_times_df["line dist"]
    )

    # grouping by mode, and summing all line times and distances, to get metric of "time per unit distance", in minutes/mile
    trn_mode_travel_times_df = (
        tm_trn_line_df[["line time", "line dist"]]
        .groupby(tm_trn_line_df["mode_name"])
        .sum()
        .reset_index()
    )
    trn_mode_travel_times_df["time_per_dist_AM"] = (
        trn_mode_travel_times_df["line time"] / trn_mode_travel_times_df["line dist"]
    )
    ...
    for index, row in trn_operator_travel_times_df.iterrows():
        if row["operator"] in [
            ...
            metrics_dict[
                runid, metric_id, "time_per_dist_AM_%s" % row["operator"], year, dbp
            ] = row["time_per_dist_AM"]

    for index, row in trn_mode_travel_times_df.iterrows():
        metrics_dict[
            runid, metric_id, "time_per_dist_AM_%s" % row["mode_name"], year, dbp
        ] = row["time_per_dist_AM"]
...
def calculate_Diverse1_LIHHinHRAs(
    ...
    metrics_dict[runid, metric_id, "TotHH_inTRA", y1, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("tra", na=False), "tothh_2015"
    ].sum()
    metrics_dict[runid, metric_id, "TotHH_inTRA", y2, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("tra", na=False), "tothh_2050"
    ].sum()
    # Total HHs in HRAs only, in 2015 and 2050
    metrics_dict[runid, metric_id, "TotHH_inHRAonly", y1, dbp] = parcel_sum_df.loc[
        (parcel_sum_df["pba50chcat"].str.contains("HRA", na=False))
        & (parcel_sum_df["pba50chcat"].str.contains("tra", na=False) == False),
        "tothh_2015",
    ].sum()
    metrics_dict[runid, metric_id, "TotHH_inHRAonly", y2, dbp] = parcel_sum_df.loc[
        (parcel_sum_df["pba50chcat"].str.contains("HRA", na=False))
        & (parcel_sum_df["pba50chcat"].str.contains("tra", na=False) == False),
        "tothh_2050",
    ].sum()
    # Total HHs in TRAs only, in 2015 and 2050
    metrics_dict[runid, metric_id, "TotHH_inTRAonly", y1, dbp] = parcel_sum_df.loc[
        (parcel_sum_df["pba50chcat"].str.contains("tra", na=False))
        & (parcel_sum_df["pba50chcat"].str.contains("HRA", na=False) == False),
        "tothh_2015",
    ].sum()
    metrics_dict[runid, metric_id, "TotHH_inTRAonly", y2, dbp] = parcel_sum_df.loc[
        (parcel_sum_df["pba50chcat"].str.contains("tra", na=False))
        & (parcel_sum_df["pba50chcat"].str.contains("HRA", na=False) == False),
        "tothh_2050",
    ].sum()
    # Total HHs in HRA/TRAs, in 2015 and 2050
    metrics_dict[runid, metric_id, "TotHH_inHRATRA", y1, dbp] = parcel_sum_df.loc[
        (parcel_sum_df["pba50chcat"].str.contains("tra", na=False))
        & (parcel_sum_df["pba50chcat"].str.contains("HRA", na=False)),
        "tothh_2015",
    ].sum()
    metrics_dict[runid, metric_id, "TotHH_inHRATRA", y2, dbp] = parcel_sum_df.loc[
        (parcel_sum_df["pba50chcat"].str.contains("tra", na=False))
        & (parcel_sum_df["pba50chcat"].str.contains("HRA", na=False)),
        "tothh_2050",
    ].sum()
    ...
    metrics_dict[runid, metric_id, "TotHH_inRoR", y1, dbp] = parcel_sum_df.loc[
        (parcel_sum_df["pba50chcat"].str.contains("HRA", na=False) == False)
        & (parcel_sum_df["pba50chcat"].str.contains("tra", na=False) == False)
        & (parcel_sum_df["pba50chcat"].str.contains("DR", na=False) == False)
        & (parcel_sum_df["coc_flag_pba2050"] == 0),
        "tothh_2015",
    ].sum()
    metrics_dict[runid, metric_id, "TotHH_inRoR", y2, dbp] = parcel_sum_df.loc[
        (parcel_sum_df["pba50chcat"].str.contains("HRA", na=False) == False)
        & (parcel_sum_df["pba50chcat"].str.contains("tra", na=False) == False)
        & (parcel_sum_df["pba50chcat"].str.contains("DR", na=False) == False)
        & (parcel_sum_df["coc_flag_pba2050"] == 0),
    ...
    metrics_dict[runid, metric_id, "Q1HH_shareofTRA", y1, dbp] = (
        parcel_sum_df.loc[
            parcel_sum_df["pba50chcat"].str.contains("tra", na=False), "hhq1_2015"
        ].sum()
    ...
    metrics_dict[runid, metric_id, "Q1HH_shareofTRA", y2, dbp] = (
        parcel_sum_df.loc[
            parcel_sum_df["pba50chcat"].str.contains("tra", na=False), "hhq1_2050"
        ].sum()
...
def calculate_Diverse2_LIHH_Displacement(
    ...
    metrics_dict[runid, metric_id, "Num_HRAtracts_total", y1, dbp] = tract_sum_df.loc[
        (tract_sum_df["hra"] == 1), "tract_id"
    ].nunique()
    ...
    tract_sum_df["hhq1_pct_2015"] = (
        tract_sum_df["hhq1_2015"] / tract_sum_df["tothh_2015"]
    )
    ...
    def check_losthhq1_share(row, j):
        if row["hhq1_pct_2015"] == 0:
            return 0
        elif (row["hhq1_pct_2050"] / row["hhq1_pct_2015"]) < j:
            return 1
        else:
            return 0
    ...
    def check_losthhq1_abs(row, j):
        if row["hhq1_2015"] == 0:
            return 0
        elif (row["hhq1_2050"] / row["hhq1_2015"]) < j:
            return 1
        else:
            return 0
    ...
    for i in [0, 10]:
        ...
        tract_sum_df["lost_hhq1_%dpct" % i] = tract_sum_df.apply(
            lambda row: check_losthhq1_share(row, j), axis=1
        )
        ...
        tract_sum_df["lost_hhq1_abs_%dpct" % i] = tract_sum_df.apply(
            lambda row: check_losthhq1_abs(row, j), axis=1
        )
        ...
        print(
            "Number of DR Tracts that lost LIHH (as a share) from 2015 to 2050: ",
            metrics_dict[
                runid, metric_id, "Num_DRtracts_lostLIHH_%dpct" % i, y_diff, dbp
            ],
        )
        print(
            "Pct of DR Tracts that lost LIHH (as a share) from 2015 to 2050: ",
            metrics_dict[
                runid, metric_id, "Pct_DRtracts_lostLIHH_%dpct" % i, y_diff, dbp
            ],
        )
        ...
        print(
            "Number of CoC Tracts that lost LIHH (as a share) from 2015 to 2050: ",
            metrics_dict[
                runid, metric_id, "Num_CoCtracts_lostLIHH_%dpct" % i, y_diff, dbp
            ],
        )
        print(
            "Pct of CoC Tracts that lost LIHH (as a share) from 2015 to 2050: ",
            metrics_dict[
                runid, metric_id, "Pct_CoCtracts_lostLIHH_%dpct" % i, y_diff, dbp
            ],
        )
        ...
        metrics_dict[
            runid, metric_id, "Num_HRAtracts_lostLIHH_%dpct" % i, y_diff, dbp
        ] = tract_sum_df.loc[
            ((tract_sum_df["hra"] == 1) & (tract_sum_df["lost_hhq1_%dpct" % i] == 1)),
            "tract_id",
        ].nunique()
        metrics_dict[
            runid, metric_id, "Pct_HRAtracts_lostLIHH_%dpct" % i, y_diff, dbp
        ] = float(
            metrics_dict[
                runid, metric_id, "Num_HRAtracts_lostLIHH_%dpct" % i, y_diff, dbp
            ]
        ...
        print(
            "Number of HRA Tracts that lost LIHH (as a share) from 2015 to 2050: ",
            metrics_dict[
                runid, metric_id, "Num_HRAtracts_lostLIHH_%dpct" % i, y_diff, dbp
            ],
        )
        print(
            "Pct of HRA Tracts that lost LIHH (as a share) from 2015 to 2050: ",
            metrics_dict[
                runid, metric_id, "Pct_HRAtracts_lostLIHH_%dpct" % i, y_diff, dbp
            ],
        )
        ...
        metrics_dict[
            runid, metric_id, "Num_DRtracts_lostLIHH_abs_%dpct" % i, y_diff, dbp
        ] = tract_sum_df.loc[
            (
                (tract_sum_df["DispRisk"] == 1)
                & (tract_sum_df["lost_hhq1_abs_%dpct" % i] == 1)
            ),
            "tract_id",
        ].nunique()
        metrics_dict[
            runid, metric_id, "Pct_DRtracts_lostLIHH_abs_%dpct" % i, y_diff, dbp
        ] = float(
            metrics_dict[
                runid, metric_id, "Num_DRtracts_lostLIHH_abs_%dpct" % i, y_diff, dbp
            ]
        ...
        print(
            "Number of DR Tracts that lost LIHH from (in absolute numbers) 2015 to 2050: ",
            metrics_dict[
                runid, metric_id, "Num_DRtracts_lostLIHH_abs_%dpct" % i, y_diff, dbp
            ],
        )
        print(
            "Pct of DR Tracts that lost LIHH from (in absolute numbers) 2015 to 2050: ",
            metrics_dict[
                runid, metric_id, "Pct_DRtracts_lostLIHH_abs_%dpct" % i, y_diff, dbp
            ],
        )
        ...
        metrics_dict[
            runid, metric_id, "Num_CoCtracts_lostLIHH_abs_%dpct" % i, y_diff, dbp
        ] = tract_sum_df.loc[
            (
                (tract_sum_df["coc_flag_pba2050"] == 1)
                & (tract_sum_df["lost_hhq1_abs_%dpct" % i] == 1)
        ...
        metrics_dict[
            runid, metric_id, "Pct_CoCtracts_lostLIHH_abs_%dpct" % i, y_diff, dbp
        ] = float(
            metrics_dict[
                runid, metric_id, "Num_CoCtracts_lostLIHH_abs_%dpct" % i, y_diff, dbp
            ]
        ...
        print(
            "Number of CoC Tracts that lost LIHH (in absolute numbers) from 2015 to 2050: ",
            metrics_dict[
                runid, metric_id, "Num_CoCtracts_lostLIHH_abs_%dpct" % i, y_diff, dbp
            ],
        )
        print(
            "Pct of CoC Tracts that lost LIHH (in absolute numbers) from 2015 to 2050: ",
            metrics_dict[
                runid, metric_id, "Pct_CoCtracts_lostLIHH_abs_%dpct" % i, y_diff, dbp
            ],
        )
        ...
        metrics_dict[
            runid, metric_id, "Num_HRAtracts_lostLIHH_abs_%dpct" % i, y_diff, dbp
        ] = tract_sum_df.loc[
            (
                (tract_sum_df["hra"] == 1)
                & (tract_sum_df["lost_hhq1_abs_%dpct" % i] == 1)
        ...
        metrics_dict[
            runid, metric_id, "Pct_HRAtracts_lostLIHH_abs_%dpct" % i, y_diff, dbp
        ] = float(
            metrics_dict[
                runid, metric_id, "Num_HRAtracts_lostLIHH_abs_%dpct" % i, y_diff, dbp
            ]
        ...
        print(
            "Number of HRA Tracts that lost LIHH (in absolute numbers) from 2015 to 2050: ",
            metrics_dict[
                runid, metric_id, "Num_HRAtracts_lostLIHH_abs_%dpct" % i, y_diff, dbp
            ],
        )
        print(
            "Pct of HRA Tracts that lost LIHH (in absolute numbers) from 2015 to 2050: ",
            metrics_dict[
                runid, metric_id, "Pct_HRAtracts_lostLIHH_abs_%dpct" % i, y_diff, dbp
            ],
        )
    ...
    metrics_dict[
        runid, metric_id, "Num_LIHH_inDRCoCHRAtracts", y2, dbp
    ] = tract_sum_df.loc[
        (
            (tract_sum_df["DispRisk"] == 1)
            | (tract_sum_df["coc_flag_pba2050"] == 1)
    ...
    metrics_dict[
        runid, metric_id, "Num_LIHH_inDRCoCHRAtracts_disp", y_diff, dbp
    ] = tract_sum_df.loc[
        (
            (
                (tract_sum_df["DispRisk"] == 1)
            ...
            & (tract_sum_df["lost_hhq1_abs_0pct"] == 1)
        ),
        "hhq1_2050",
    ].sum()
    ...
    metrics_dict[
        runid, metric_id, "DispRisk_PBA40_Q1Q2_DRCoCHRAtracts", y_diff, dbp
    ] = (
        metrics_dict[runid, metric_id, "Num_Q1Q2HH_inDRCoCHRAtracts_disp", y_diff, dbp]
        / metrics_dict[runid, metric_id, "Num_Q1Q2HH_inDRCoCHRAtracts", y2, dbp]
    )
    ...
    metrics_dict[runid, metric_id, "Num_TRAs_total", y1, dbp] = TRA_sum_df[
        "juris_tra"
    ].nunique()
    ...
    for i in [0, 10]:
        ...
        TRA_sum_df["lost_hhq1_%dpct" % i] = TRA_sum_df.apply(
            lambda row: check_losthhq1_share(row, j), axis=1
        )

        # Calculating absolute change in LIHH at TRA level to check true displacement
        TRA_sum_df["lost_hhq1_abs_%dpct" % i] = TRA_sum_df.apply(
            lambda row: check_losthhq1_abs(row, j), axis=1
        )
        ...
        metrics_dict[
            runid, metric_id, "Num_TRAs_lostLIHH_%dpct" % i, y_diff, dbp
        ] = TRA_sum_df.loc[
            (TRA_sum_df["lost_hhq1_%dpct" % i] == 1), "juris_tra"
        ].nunique()
        metrics_dict[
            runid, metric_id, "Pct_TRAs_lostLIHH_%dpct" % i, y_diff, dbp
        ] = float(
            metrics_dict[runid, metric_id, "Num_TRAs_lostLIHH_%dpct" % i, y_diff, dbp]
        ) / float(
            metrics_dict[runid, metric_id, "Num_TRAs_total", y1, dbp]
        )
        print(
            "Number of TRAs that lost LIHH (as a share) from 2015 to 2050: ",
            metrics_dict[runid, metric_id, "Num_TRAs_lostLIHH_%dpct" % i, y_diff, dbp],
        )
        print(
            "Pct of TRAs that lost LIHH (as a share) from 2015 to 2050: ",
        ...
        # Number or percent of DR tracts that lost Q1 households in absolute numbers
        metrics_dict[
            runid, metric_id, "Num_TRAs_lostLIHH_abs_%dpct" % i, y_diff, dbp
        ] = TRA_sum_df.loc[
            (TRA_sum_df["lost_hhq1_abs_%dpct" % i] == 1), "juris_tra"
        ].nunique()
        metrics_dict[
            runid, metric_id, "Pct_TRAs_lostLIHH_abs_%dpct" % i, y_diff, dbp
        ] = float(
            metrics_dict[
                runid, metric_id, "Num_TRAs_lostLIHH_abs_%dpct" % i, y_diff, dbp
            ]
        ...
        print(
            "Number of TRAs that lost LIHH (in absolute numbers) from 2015 to 2050: ",
            metrics_dict[
                runid, metric_id, "Num_TRAs_lostLIHH_abs_%dpct" % i, y_diff, dbp
            ],
        )
        print(
            "Pct of TRAs that lost LIHH (in absolute numbers) from 2015 to 2050: ",
            metrics_dict[
                runid, metric_id, "Pct_TRAs_lostLIHH_abs_%dpct" % i, y_diff, dbp
            ],
        )
    ...
    metrics_dict[runid, metric_id, "Num_LIHH_inTRAs", y2, dbp] = TRA_sum_df[
        "hhq1_2050"
    ].sum()
    metrics_dict[
        runid, metric_id, "Num_LIHH_inTRAs_disp", y_diff, dbp
    ...
    tract_sum_filename = "C:/Users/{}/Box/Horizon and Plan Bay Area 2050/Equity and Performance/7_Analysis/Metrics/tract_summary_output.csv".format(
        os.getenv("USERNAME")
    )
...
def calculate_Healthy1_HHs_SLRprotected(runid, dbp, parcel_sum_df, metrics_dict):
    ...
    def label_SLR(row):
        if (row["SLR"] == 12) or (row["SLR"] == 24) or (row["SLR"] == 36):
            return "Unprotected"
        elif row["SLR"] == 100:
            return "Protected"
        else:
...
def calculate_Healthy1_safety(
    runid, year, dbp, tm_taz_input_df, safety_df, metrics_dict
):
    ...
    fatalities = safety_df.loc[
        (safety_df["index"] == "N_total_fatalities")
        & (safety_df["modelrunID"].str.contains(dbp)),
        "value",
    ].sum()
    fatalities_m = safety_df.loc[
        (safety_df["index"] == "N_motorist_fatalities")
        & (safety_df["modelrunID"].str.contains(dbp)),
        "value",
    ].sum()
    fatalities_b = safety_df.loc[
        (safety_df["index"] == "N_bike_fatalities")
        & (safety_df["modelrunID"].str.contains(dbp)),
        "value",
    ].sum()
    fatalities_p = safety_df.loc[
        (safety_df["index"] == "N_ped_fatalities")
        & (safety_df["modelrunID"].str.contains(dbp)),
        "value",
    ].sum()
    injuries = safety_df.loc[
        (safety_df["index"] == "N_injuries")
        & (safety_df["modelrunID"].str.contains(dbp)),
        "value",
    ].sum()

    metrics_dict[runid, metric_id, "fatalities_annual_per_MNppl", year, dbp] = (
        fatalities / population * per_x_people
    )
    metrics_dict[runid, metric_id, "fatalities_auto_annual_per_MNppl", year, dbp] = (
        fatalities_m / population * per_x_people
    )
    metrics_dict[runid, metric_id, "fatalities_bike_annual_per_MNppl", year, dbp] = (
        fatalities_b / population * per_x_people
    )
    metrics_dict[runid, metric_id, "fatalities_ped_annual_per_MNppl", year, dbp] = (
        fatalities_p / population * per_x_people
    )
    metrics_dict[runid, metric_id, "injuries_annual_per_MNppl", year, dbp] = (
        injuries / population * per_x_people
    )

    metrics_dict[
        runid, metric_id, "fatalities_annual_per_100MVMT", year, dbp
    ] = safety_df.loc[
        (safety_df["index"] == "N_total_fatalities_per_100M_VMT")
        & (safety_df["modelrunID"].str.contains(dbp)),
        "value",
    ].sum()
    metrics_dict[
        runid, metric_id, "fatalities_auto_annual_per_100MVMT", year, dbp
    ] = safety_df.loc[
        (safety_df["index"] == "N_motorist_fatalities_per_100M_VMT")
        & (safety_df["modelrunID"].str.contains(dbp)),
        "value",
    ].sum()
    metrics_dict[
        runid, metric_id, "fatalities_bike_annual_per_100MVMT", year, dbp
    ] = safety_df.loc[
        (safety_df["index"] == "N_bike_fatalities_per_100M_VMT")
        & (safety_df["modelrunID"].str.contains(dbp)),
        "value",
    ].sum()
    metrics_dict[
        runid, metric_id, "fatalities_ped_annual_per_100MVMT", year, dbp
    ] = safety_df.loc[
        (safety_df["index"] == "N_ped_fatalities_per_100M_VMT")
        & (safety_df["modelrunID"].str.contains(dbp)),
        "value",
    ].sum()
    metrics_dict[
        runid, metric_id, "injuries_annual_per_100MVMT", year, dbp
    ] = safety_df.loc[
        (safety_df["index"] == "N_injuries_per_100M_VMT")
        & (safety_df["modelrunID"].str.contains(dbp)),
        "value",
    ].sum()


def calculate_Healthy2_emissions(
    runid, year, dbp, tm_taz_input_df, tm_auto_times_df, emfac_df, metrics_dict
):
    ...
    dailyVMT = (
        tm_auto_times_df["Vehicle Miles"].sum()
        - tm_auto_times_df.loc["truck", ["Vehicle Miles"]].sum()
    )

    metrics_dict[runid, metric_id, "daily_vmt_per_capita", year, dbp] = (
        dailyVMT / population
    )

    metrics_dict[
        runid, metric_id, "daily_vmt_per_capita", "2005", "2005"
    ] = emfac_df.loc[(emfac_df["dbp"] == 2005), "VMT per capita"].sum()
    metrics_dict[
        runid, metric_id, "daily_vmt_per_capita", "2035", "2035"
    ] = emfac_df.loc[(emfac_df["dbp"] == 2035), "VMT per capita"].sum()

    metrics_dict[
        "emfac_hardcode", metric_id, "ghg_emissions_lbs_per_capita", "2005", "2005"
    ] = emfac_df.loc[
        (emfac_df["dbp"] == 2005), "Total CO2 Emissions Per Capita (lbs)"
    ].sum()
    metrics_dict[
        "emfac_hardcode", metric_id, "ghg_emissions_lbs_per_capita", "2015", "2015"
    ] = emfac_df.loc[
        (emfac_df["dbp"] == 2015), "Total CO2 Emissions Per Capita (lbs)"
    ].sum()
    metrics_dict[
        "emfac_hardcode", metric_id, "ghg_emissions_lbs_per_capita", "2035", "2035"
    ] = emfac_df.loc[
        (emfac_df["dbp"] == 2035), "Total CO2 Emissions Per Capita (lbs)"
    ].sum()
    metrics_dict[
        "emfac_hardcode", metric_id, "ghg_emissions_lbs_per_capita", "2050", "Plus"
    ] = 0

    metrics_dict[
        "emfac_hardcode",
        metric_id,
        "ghg_emissions_nonSB375_lbs_per_capita",
        "2005",
        "2005",
    ] = emfac_df.loc[
        (emfac_df["dbp"] == 2005), "Total CO2 Emissions Per Capita (lbs)"
...
def calculate_Vibrant1_JobsHousing(runid, dbp, county_sum_df, metrics_dict):
    ...
    for index, row in county_sum_df.iterrows():
        metrics_dict[
            runid, metric_id, "jobs_housing_ratio_%s" % row["county"], y1, dbp
        ] = (row["totemp_2015"] / row["tothh_2015"])
        metrics_dict[
            runid, metric_id, "jobs_housing_ratio_%s" % row["county"], y2, dbp
        ] = (row["totemp_2050"] / row["tothh_2050"])
...
def calculate_Vibrant1_median_commute(runid, year, dbp, tm_commute_df, metrics_dict):
    ...
    tm_commute_df["total_commute_miles"] = (
        tm_commute_df["freq"] * tm_commute_df["distance"]
    )

    commute_dist_df = (
        tm_commute_df[["incQ", "freq", "total_commute_miles"]].groupby(["incQ"]).sum()
    )
...
def calculate_travelmodel_metrics_change(list_tm_runid_blueprintonly, metrics_dict):
    ...
    for tm_runid in list_tm_runid_blueprintonly:
        ...
        # Tolls
        metrics_dict[tm_runid, metric_id, "tolls_per_HH_change_2015", year, dbp] = (
            metrics_dict[tm_runid, metric_id, "tolls_per_HH", year, dbp]
            / metrics_dict[tm_2015_runid, metric_id, "tolls_per_HH", y1, "2015"]
            - 1
        )
        metrics_dict[
            tm_runid, metric_id, "tolls_per_HH_change_2050noproject", year, dbp
        ] = (
            metrics_dict[tm_runid, metric_id, "tolls_per_HH", year, dbp]
            / metrics_dict[
                tm_2050_DBP_NoProject_runid, metric_id, "tolls_per_HH", y2, "NoProject"
            ]
            - 1
        )
        metrics_dict[tm_runid, metric_id, "tolls_per_LIHH_change_2015", year, dbp] = (
            metrics_dict[tm_runid, metric_id, "tolls_per_LIHH", year, dbp]
            / metrics_dict[tm_2015_runid, metric_id, "tolls_per_LIHH", y1, "2015"]
            - 1
        )
        metrics_dict[
            tm_runid, metric_id, "tolls_per_LIHH_change_2050noproject", year, dbp
        ] = (
            metrics_dict[tm_runid, metric_id, "tolls_per_LIHH", year, dbp]
            / metrics_dict[
                tm_2050_DBP_NoProject_runid,
                metric_id,
                "tolls_per_LIHH",
                y2,
                "NoProject",
        ...
        metrics_dict[tm_runid, metric_id, "tolls_per_inc1HH_change_2015", year, dbp] = (
            metrics_dict[tm_runid, metric_id, "tolls_per_inc1HH", year, dbp]
            / metrics_dict[tm_2015_runid, metric_id, "tolls_per_inc1HH", y1, "2015"]
            - 1
        )
        metrics_dict[
            tm_runid, metric_id, "tolls_per_inc1HH_change_2050noproject", year, dbp
        ] = (
            metrics_dict[tm_runid, metric_id, "tolls_per_inc1HH", year, dbp]
            / metrics_dict[
                tm_2050_DBP_NoProject_runid,
        ...
        # Transit Fares
        metrics_dict[tm_runid, metric_id, "fares_per_HH_change_2015", year, dbp] = (
            metrics_dict[tm_runid, metric_id, "fares_per_HH", year, dbp]
            / metrics_dict[tm_2015_runid, metric_id, "fares_per_HH", y1, "2015"]
            - 1
        )
        metrics_dict[
            tm_runid, metric_id, "fares_per_HH_change_2050noproject", year, dbp
        ] = (
            metrics_dict[tm_runid, metric_id, "fares_per_HH", year, dbp]
            / metrics_dict[
                tm_2050_DBP_NoProject_runid, metric_id, "fares_per_HH", y2, "NoProject"
            ]
            - 1
        )
        metrics_dict[tm_runid, metric_id, "fares_per_LIHH_change_2015", year, dbp] = (
            metrics_dict[tm_runid, metric_id, "fares_per_LIHH", year, dbp]
            / metrics_dict[tm_2015_runid, metric_id, "fares_per_LIHH", y1, "2015"]
            - 1
        )
        metrics_dict[
            tm_runid, metric_id, "fares_per_LIHH_change_2050noproject", year, dbp
        ] = (
            metrics_dict[tm_runid, metric_id, "fares_per_LIHH", year, dbp]
            / metrics_dict[
                tm_2050_DBP_NoProject_runid,
        ...
        metrics_dict[tm_runid, metric_id, "fares_per_inc1HH_change_2015", year, dbp] = (
            metrics_dict[tm_runid, metric_id, "fares_per_inc1HH", year, dbp]
            / metrics_dict[tm_2015_runid, metric_id, "fares_per_inc1HH", y1, "2015"]
            - 1
        )
        metrics_dict[
            tm_runid, metric_id, "fares_per_inc1HH_change_2050noproject", year, dbp
        ] = (
            metrics_dict[tm_runid, metric_id, "fares_per_inc1HH", year, dbp]
            / metrics_dict[
                tm_2050_DBP_NoProject_runid,
        ...
        for route in [
            ...
            metrics_dict[
                tm_runid, metric_id, "travel_time_AM_change_2015_%s" % route, year, dbp
            ] = (
                metrics_dict[
                    tm_runid, metric_id, "travel_time_AM_%s" % route, year, dbp
                ]
                / metrics_dict[
                    tm_2015_runid, metric_id, "travel_time_AM_%s" % route, y1, "2015"
                ]
                - 1
            )
            metrics_dict[
                tm_runid,
                metric_id,
                "travel_time_AM_change_2050noproject_%s" % route,
                year,
                dbp,
            ] = (
                metrics_dict[
        ...
        for operator in [
            ...
            try:
                metrics_dict[
                    tm_runid,
                    metric_id,
                    "crowded_pct_personhrs_AM_change_2015_%s" % operator,
                    year,
                    dbp,
                ] = (
                    metrics_dict[
                        tm_runid,
                        metric_id,
                        "crowded_pct_personhrs_AM_%s" % operator,
                        year,
                        dbp,
                    ]
                    / metrics_dict[
            ...
            try:
                metrics_dict[
                    tm_runid,
                    metric_id,
                    "crowded_pct_personhrs_AM_change_2050noproject_%s" % operator,
                    year,
                    dbp,
                ] = (
                    metrics_dict[
        ...
        for operator in [
            ...
            metrics_dict[
                tm_runid,
                metric_id,
                "time_per_dist_AM_change_2015_%s" % operator,
                year,
                dbp,
            ] = (
                metrics_dict[
                    tm_runid, metric_id, "time_per_dist_AM_%s" % operator, year, dbp
                ]
                / metrics_dict[
                    tm_2015_runid,
                    metric_id,
                    "time_per_dist_AM_%s" % operator,
                    y1,
                    "2015",
                ]
                - 1
            ...
            metrics_dict[
                tm_runid,
                metric_id,
                "time_per_dist_AM_change_2050noproject_%s" % operator,
                year,
                dbp,
            ] = (
                metrics_dict[
        ...
        for mode_name in [
            ...
            metrics_dict[
                tm_runid,
                metric_id,
                "time_per_dist_AM_change_2015_%s" % mode_name,
                year,
                dbp,
            ] = (
                metrics_dict[
                    tm_runid, metric_id, "time_per_dist_AM_%s" % mode_name, year, dbp
                ]
                / metrics_dict[
                    tm_2015_runid,
                    metric_id,
                    "time_per_dist_AM_%s" % mode_name,
                    y1,
                    "2015",
                ]
                - 1
            ...
            metrics_dict[
                tm_runid,
                metric_id,
                "time_per_dist_AM_change_2050noproject_%s" % mode_name,
                year,
                dbp,
            ] = (
                metrics_dict[
...
def calc_pba40urbansim():
    ...
    urbansim_runid = "C:/Users/{}/Box/Modeling and Surveys/Share Data/plan-bay-area-2040/RTP17 UrbanSim Output/r7224c/run7224".format(
        os.getenv("USERNAME")
    )
    ...
    parcel_output_2040_df["tothh"] = parcel_output_2040_df[hhq_list].sum(
        axis=1, skipna=True
    )
    parcel_output_2040_df["totemp"] = parcel_output_2040_df[emp_list].sum(
        axis=1, skipna=True
    )

    parcel_output_2010_df = pd.read_csv((urbansim_runid + "_parcel_data_2010.csv"))
    parcel_output_2010_df["tothh"] = parcel_output_2010_df[hhq_list].sum(
        axis=1, skipna=True
    )
    parcel_output_2010_df["totemp"] = parcel_output_2010_df[emp_list].sum(
        axis=1, skipna=True
    )

    # keeping essential columns / renaming columns
    parcel_output_2040_df.drop(
        [
            "x",
            "y",
            "zoned_du",
    ...
    # all households
    metrics_dict[runid, metric_id, "TotHH_region", year2, dbp] = parcel_sum_df[
        "tothh_2040"
    ].sum()
    metrics_dict[runid, metric_id, "TotHH_region", year1, dbp] = parcel_sum_df[
        "tothh_2010"
    ].sum()
    metrics_dict[runid, metric_id, "TotHH_growth_region", yeardiff, dbp] = (
        metrics_dict[runid, metric_id, "TotHH_region", year2, dbp]
        / metrics_dict[runid, metric_id, "TotHH_region", year1, dbp]
        - 1
    )
    ...
    # HH Growth in HRAs
    metrics_dict[runid, metric_id, "TotHH_HRA", year2, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("HRA", na=False), "tothh_2040"
    ].sum()
    metrics_dict[runid, metric_id, "TotHH_HRA", year1, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("HRA", na=False), "tothh_2010"
    ].sum()
    ...
    # HH Growth in TRAs
    metrics_dict[runid, metric_id, "TotHH_TRA", year2, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("tra", na=False), "tothh_2040"
    ].sum()
    metrics_dict[runid, metric_id, "TotHH_TRA", year1, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("tra", na=False), "tothh_2010"
    ].sum()
    ...
    # all jobs
    metrics_dict[runid, metric_id, "TotJobs_region", year2, dbp] = parcel_sum_df[
        "totemp_2040"
    ].sum()
    metrics_dict[runid, metric_id, "TotJobs_region", year1, dbp] = parcel_sum_df[
        "totemp_2010"
    ].sum()
    metrics_dict[runid, metric_id, "TotJobs_growth_region", yeardiff, dbp] = (
        metrics_dict[runid, metric_id, "TotJobs_region", year2, dbp]
        / metrics_dict[runid, metric_id, "TotJobs_region", year1, dbp]
        - 1
    )
    ...
    # Job Growth in HRAs
    metrics_dict[runid, metric_id, "TotJobs_HRA", year2, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("HRA", na=False), "totemp_2040"
    ].sum()
    metrics_dict[runid, metric_id, "TotJobs_HRA", year1, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("HRA", na=False), "totemp_2010"
    ].sum()
    ...
    # Job Growth in TRAs
    metrics_dict[runid, metric_id, "TotJobs_TRA", year2, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("tra", na=False), "totemp_2040"
    ].sum()
    metrics_dict[runid, metric_id, "TotJobs_TRA", year1, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("tra", na=False), "totemp_2010"
    ].sum()
...
def calc_urbansim_metrics():
    ...
    for us_runid in list_us_runid:
        ...
        county_sum_df["tothh_growth"] = (
            county_sum_df["tothh_2050"] / county_sum_df["tothh_2015"] - 1
        )
        county_sum_df["totemp_growth"] = (
            county_sum_df["totemp_2050"] / county_sum_df["totemp_2015"] - 1
        )
        ...
        county_sum_df["LIHH_growth"] = (
            county_sum_df["hhq1_2050"] + county_sum_df["hhq2_2050"]
        ) / (county_sum_df["hhq1_2015"] + county_sum_df["hhq2_2015"]) - 1
        ...
        TRA_sum_df = (
            parcel_sum_df.groupby(["juris_tra"])[
                "tothh_2050",
                "tothh_2015",
                "hhq1_2050",
                "hhq1_2015",
        ...
        parcel_filename = "C:/Users/{}/Box/Horizon and Plan Bay Area 2050/Equity and Performance/7_Analysis/Metrics/parcel_summary_output.csv".format(
            os.getenv("USERNAME")
        )
...
def calc_travelmodel_metrics():
    ...
    for tm_runid in list_tm_runid:
        ...
        if "2015" in tm_runid:
            ...
            tm_run_location = tm_run_location_bp
        tm_scen_metrics_df = pd.read_csv(
            tm_run_location + tm_runid + "/OUTPUT/metrics/scenario_metrics.csv",
            names=["runid", "metric_name", "value"],
        )
        tm_auto_owned_df = pd.read_csv(
            tm_run_location + tm_runid + "/OUTPUT/metrics/autos_owned.csv"
        )
        tm_auto_times_df = pd.read_csv(
            tm_run_location + tm_runid + "/OUTPUT/metrics/auto_times.csv",
            sep=",",
            index_col=[0, 1],
        )
        tm_travel_cost_df = pd.read_csv(
            tm_run_location + tm_runid + "/OUTPUT/core_summaries/TravelCost.csv"
        )
        tm_commute_df = pd.read_csv(
            tm_run_location
            + tm_runid
            + "/OUTPUT/core_summaries/CommuteByIncomeHousehold.csv"
        )
        tm_taz_input_df = pd.read_csv(
            tm_run_location + tm_runid + "/INPUT/landuse/tazData.csv"
        )

        calculate_Affordable1_transportation_costs(
            tm_runid,
            year,
            dbp,
            tm_scen_metrics_df,
        ...
        print("@@@@@@@@@@@@@ A1 Done")
        calculate_Connected1_accessibility(
            tm_runid, year, dbp, tm_scen_metrics_df, metrics_dict
        )
        print("@@@@@@@@@@@@@ C1 Done")
        calculate_Connected2_hwy_traveltimes(
            tm_runid, year, dbp, hwy_corridor_links_df, metrics_dict
        )
        print("@@@@@@@@@@@@@ C2hwy Done")
        calculate_Connected2_trn_traveltimes(
            tm_runid, year, dbp, transit_operator_df, metrics_dict
        )
        print("@@@@@@@@@@@@@ C2trn Done")
        calculate_Connected2_crowding(
            tm_runid, year, dbp, transit_operator_df, metrics_dict
        )
        print("@@@@@@@@@@@@@ C2crowding Done")
        calculate_Healthy1_safety(
            tm_runid, year, dbp, tm_taz_input_df, safety_df, metrics_dict
        )
        print("@@@@@@@@@@@@@ H1 Done")
        calculate_Healthy2_emissions(
            tm_runid,
            year,
            dbp,
            tm_taz_input_df,
        ...
        print("@@@@@@@@@@@@@ H2 Done")
        calculate_Vibrant1_median_commute(
            tm_runid, year, dbp, tm_commute_df, metrics_dict
        )
        print("@@@@@@@@@@@@@ V1 Done")
...
if __name__ == "__main__":
    ...
    us_2050_DBP_Plus_runid = (
        "Blueprint Plus Crossing (s23)/v1.7.1- FINAL DRAFT BLUEPRINT/run98"
    )
    ...
    list_tm_runid = [
        tm_2015_runid,
        tm_2050_DBP_NoProject_runid,
        tm_2050_DBP_PlusCrossing_runid,
    ]
    ...
    parcel_PDA_xwalk_file = (
        "M:/Data/GIS layers/Blueprint Land Use Strategies/ID_idx/pda_id_2020.csv"
    )
    parcel_TRA_xwalk_file = (
        "M:/Data/GIS layers/Blueprint Land Use Strategies/ID_idx/tra_id_2020_s23.csv"
    )
    parcel_HRA_xwalk_file = (
        "M:/Data/GIS layers/Blueprint Land Use Strategies/ID_idx/hra_id_2020.csv"
    )
    ...
    print(
        "*****************#####################Completed calc_travelmodel_metrics#####################*******************"
    )
    ...
    metrics.name = "value"
    out_filename = "C:/Users/{}/Box/Horizon and Plan Bay Area 2050/Equity and Performance/7_Analysis/Metrics/metrics.csv".format(
        os.getenv("USERNAME")
    )
