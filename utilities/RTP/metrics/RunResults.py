...
class RunResults:
    ...
    ALREADY_ANNUAL = {
        ...
        (
            "Other Travel Metrics (for reference only)",
    ...
    ALREADY_DIFF = {
        (
            "Accessibility Benefits (household-based) (with CEM)",
            "Logsum Hours - Mandatory Tours - Workers & Students",
        ): True,
        (
            "Accessibility Benefits (household-based) (with CEM)",
            "Logsum Hours - NonMandatory Tours - All people",
        ): True,
        (
            "Accessibility Benefits (household-based) (no CEM)",
            "Logsum Hours - Mandatory Tours - Workers & Students",
        ): True,
        (
            "Accessibility Benefits (household-based) (no CEM)",
    ...
    def __init__(self, rundir, overwrite_config=None):
        ...
        if len(rundir) > 22:
            configs_df = pd.read_excel(
                self.ppa_master_input, sheet_name="configs_projects", header=0
            )
            configs_df = configs_df.drop(
                ["Base Model", "Base ID", "Future Run", "Iteration", "Full Name"],
                axis=1,
            )
            configs_df.insert(0, "Folder", "")
            configs_df["Folder"] = configs_df[
                ["Foldername - Project", "Foldername - Future"]
            ].apply(lambda x: "\\".join(x), axis=1)
            configs_df.drop(["Foldername - Project", "Foldername - Future"], axis=1)
        else:
            configs_df = pd.read_excel(
                self.ppa_master_input, sheet_name="configs_base", header=0
            )
        ...
        valuations_df = pd.read_excel(
            self.ppa_master_input, sheet_name="valuations", header=0
        )
        self.BENEFIT_VALUATION = valuations_df.set_index(
            ["Benefit Category 1", "Benefit Category 2", "Benefit Category 3"]
        ).T.to_dict("list")
        ...
        for dict_key in old_keys:
            ...
            try:
                if math.isnan(float(dict_key[2])):
                    self.BENEFIT_VALUATION[
                        (dict_key[0], dict_key[1])
                    ] = self.BENEFIT_VALUATION.get(dict_key)
        ...
        if overwrite_config:
            ...
            for key in overwrite_config.keys():
        ...
        print("")
        # read the csvs
        self.auto_times = pd.read_table(
            os.path.join(self.rundir, "auto_times.csv"), sep=",", index_col=[0, 1]
        )
        # print(self.auto_times)

        self.autos_owned = pd.read_table(
            os.path.join(self.rundir, "autos_owned.csv"), sep=","
        )
        self.autos_owned["total autos"] = (
            self.autos_owned["households"] * self.autos_owned["autos"]
        )
        self.autos_owned.set_index(["incQ", "autos"], inplace=True)
        # print(self.autos_owned)

        self.parking_costs = pd.read_table(
            os.path.join(self.rundir, "parking_costs.csv"), sep=","
        )
        # print(self.parking_costs.head())

        self.vmt_vht_metrics = pd.read_table(
            os.path.join(self.rundir, "vmt_vht_metrics.csv"), sep=",", index_col=[0, 1]
        )
        # print(self.vmt_vht_metrics)

        self.nonmot_times = pd.read_table(
            os.path.join(self.rundir, "nonmot_times.csv"), sep=",", index_col=[0, 1, 2]
        )
        # print(self.nonmot_times)

        self.transit_boards_miles = pd.read_table(
            os.path.join(self.rundir, "transit_boards_miles.csv"), sep=",", index_col=0
        )
        # print(self.transit_boards_miles)

        self.transit_times_by_acc_mode_egr = pd.read_table(
            os.path.join(self.rundir, "transit_times_by_acc_mode_egr.csv"),
            sep=",",
            index_col=[0, 1, 2, 3],
        )
        # print(self.transit_times_by_acc_mode_egr)

        self.transit_times_by_mode_income = pd.read_table(
            os.path.join(self.rundir, "transit_times_by_mode_income.csv"),
            sep=",",
            index_col=[0, 1],
        )
        # print(self.transit_times_by_mode_income)

        self.unique_active_travelers = pd.read_csv(
            os.path.join(self.rundir, "unique_active_travelers.csv"),
            index_col=0,
            header=None,
            squeeze=True,
        )
        ...
        self.crowding_df = pd.read_csv(
            os.path.join(self.rundir, "transit_crowding.csv")
        )
        self.crowding_complete_df = pd.read_csv(
            os.path.join(self.rundir, "transit_crowding_complete.csv")
        )
        ...
        if "base_dir" in self.config.keys():
            ...
            # Project costs
            df_costs = pd.read_excel(
                self.ppa_master_input, sheet_name="project_costs", header=0
            )
            df_costs = df_costs.drop(["Project Name"], axis=1)
            df_costs = df_costs[
                df_costs["project_id"]
                == int(self.config.loc["Project ID"].split("_")[0])
            ]
            self.proj_costs = df_costs.to_dict("records", into=OrderedDict)[0]

            # asset life
            df_asset_life = pd.read_excel(
                self.ppa_master_input, sheet_name="asset_life", header=0
            )
            self.asset_life = df_asset_life.set_index(["asset_class"]).T.to_dict(
                "records", into=OrderedDict
            )[0]

            # CRF-related collisions
            df_collisions_SWITRS = pd.read_excel(
                self.ppa_master_input, sheet_name="collisions_switrs", header=0
            )
            new_header = df_collisions_SWITRS.iloc[
                0
            ]  # grab the first row for the header
            df_collisions_SWITRS = df_collisions_SWITRS[
                1:
            ]  # take the data less the header row
            df_collisions_SWITRS.columns = new_header
            df_collisions_SWITRS = df_collisions_SWITRS.drop(["Project Name"], axis=1)

            df_collisions_SWITRS2 = df_collisions_SWITRS[
                df_collisions_SWITRS["project_id"]
                == int(self.config.loc["Project ID"].split("_")[0])
            ]
            if df_collisions_SWITRS2.shape[0] == 0:
                df_collisions_SWITRS2 = df_collisions_SWITRS[
                    df_collisions_SWITRS["project_id"] == 0
                ]
            self.collisions_switrs = df_collisions_SWITRS2.to_dict(
                "records", into=OrderedDict
            )[0]

            # Natural Land
            df_natural_land = pd.read_excel(
                self.ppa_master_input, sheet_name="natural_land", header=0
            )
            df_natural_land = df_natural_land.drop(["Project Name"], axis=1)
            df_natural_land2 = df_natural_land[
                df_natural_land["project_id"]
                == int(self.config.loc["Project ID"].split("_")[0])
            ]
            ...
            # Guiding Principles
            df_gp = pd.read_excel(
                self.ppa_master_input, sheet_name="guiding_principles", header=0
            )
            df_gp = df_gp.drop(["Project Name"], axis=1)
            df_gp2 = df_gp[
                df_gp["project_id"] == int(self.config.loc["Project ID"].split("_")[0])
            ]
            if df_gp2.shape[0] == 0:
        ...
        # on M
        roadway_netfile = os.path.abspath(
            os.path.join(self.rundir, "..", "avgload5period_vehclasses.csv")
        )
        ...
        if not roadway_read:
            roadway_netfile = os.path.abspath(
                os.path.join(
                    self.rundir, "..", "extractor", "avgload5period_vehclasses.csv"
                )
            )
            if os.path.exists(roadway_netfile):
                self.roadways_df = pd.read_table(roadway_netfile, sep=",")
        ...
        if not roadway_read:
            ...
            roadway_netfile = os.path.abspath(
                os.path.join(
                    ...
                    "..",
                    "hwy",
                    "iter%s" % os.environ["ITER"],
                    "avgload5period_vehclasses.csv",
                )
            )
            self.roadways_df = pd.read_table(roadway_netfile, sep=",")
            print("Read roadways from {}".format(roadway_netfile))
        ...
        self.roadways_df["small truck volume"] = (
            self.roadways_df.volEA_sm
            + self.roadways_df.volEA_smt
            + self.roadways_df.volAM_sm
            + self.roadways_df.volAM_smt
            + self.roadways_df.volMD_sm
            + self.roadways_df.volMD_smt
            + self.roadways_df.volPM_sm
            + self.roadways_df.volPM_smt
            + self.roadways_df.volEV_sm
            + self.roadways_df.volEV_smt
        )
        self.roadways_df["large truck volume"] = (
            self.roadways_df.volEA_hv
            + self.roadways_df.volEA_hvt
            + self.roadways_df.volAM_hv
            + self.roadways_df.volAM_hvt
            + self.roadways_df.volMD_hv
            + self.roadways_df.volMD_hvt
            + self.roadways_df.volPM_hv
            + self.roadways_df.volPM_hvt
            + self.roadways_df.volEV_hv
            + self.roadways_df.volEV_hvt
        )

        for filename in ["mandatoryAccessibilities", "nonMandatoryAccessibilities"]:
            accessibilities = pd.read_table(
                os.path.join(self.rundir, "..", "logsums", "%s.csv" % filename), sep=","
            )
            ...
            accessibilities["incQ_label"] = (
                accessibilities["level_2"].str.split("_", n=1).str.get(0)
            )
            accessibilities["autoSuff_label"] = (
                accessibilities["level_2"].str.split("_", n=1).str.get(1)
            )
            accessibilities["autoSuff_label"] = (
                accessibilities["autoSuff_label"].str.rsplit("_", n=1).str.get(0)
            )
            accessibilities["hasAV"] = accessibilities.apply(
                lambda row: 0 if "noAV" in row["level_2"] else 1, axis=1
            )
            ...
            accessibilities.rename(
                columns={0: "%s_dclogsum" % col_prefix, "subzone": "walk_subzone"},
                inplace=True,
            )
        ...
        self.accessibilityMarkets = pd.read_table(
            os.path.join(
                self.rundir, "..", "core_summaries", "AccessibilityMarkets.csv"
            ),
            sep=",",
        )

        self.accessibilityMarkets.rename(
            columns={
                "num_persons": "%s_num_persons" % col_prefix,
    ...
    def createBaseRunResults(self):
        ...
        if self.base_dir:
            ...
            self.base_results = RunResults(
                rundir=self.base_dir, overwrite_config=base_overwrite_config
            )
    ...
    def updateDailyMetrics(self):
        ...
        base_avg_auto_speed = self.base_results.daily_results[
            (cat1, cat2, "Average Auto Speed")
        ]
        pct_change_speed = (avg_auto_speed - base_avg_auto_speed) / base_avg_auto_speed

        base_avg_truck_speed = self.base_results.daily_results[
            (cat1, cat2, "Average Truck Speed")
        ]
        ...
        cat2 = "Truck Hours"
        base_truck_vmt = (
            self.base_results.daily_results[(cat1, cat2, "Truck (Computed VHT)")]
            * base_avg_truck_speed
        )

        self.daily_results[cat1, cat2, "Truck (Computed VHT)"] = (
            base_truck_vmt / new_truck_speed
        )
        self.daily_results[
            "Accessibility Benefits (other)",
            "Non-Household",
        ...
        base_auto_nrfd = self.base_results.daily_results[
            cat1, cat2, "Auto (Person Hours)"
        ]
        base_truck_nrfd = self.base_results.daily_results[
            cat1, cat2, "Truck (Computed VH)"
        ]

        pct_change_nrfd = (auto_nrfd - base_auto_nrfd) / base_auto_nrfd
        self.daily_results[cat1, cat2, "Truck (Computed VH)"] = (
            1.0 + pct_change_nrfd
        ) * base_truck_nrfd
        self.daily_results[
            "Other Transportation Benefits",
            "Travel Time Reliability (Non-Recurring Freeway Delay) (Hours)",
            "Truck (Computed VH)",
        ] = (1.0 + pct_change_nrfd) * base_truck_nrfd
        ...
        cat2 = "VMT by class"
        auto_vmt = (
            self.daily_results[cat1, cat2, "Auto non-AV"]
            + self.daily_results[(cat1, cat2, "Auto AV")]
        )
        base_auto_vmt = (
            self.base_results.daily_results[cat1, cat2, "Auto non-AV"]
            + self.base_results.daily_results[cat1, cat2, "Auto AV"]
        )
        ...
        self.daily_results[cat1, cat2, "Truck - Computed"] = (
            1.0 + pct_change_vmt
        ) * base_truck_vmt
        ...
        self.quick_summary["VMT -- all vehicles"] = (
            vmt_total - vmt_trucks + ((1.0 + pct_change_vmt) * base_truck_vmt)
        )
        ...
        self.daily_results[cat1, cat2, "PM2.5 Road Dust"] = (
            ...
            * RunResults.PM25_ROADDUST
            * RunResults.GRAMS_TO_US_TONS
        )
        ...
        self.roadways_df = pd.merge(
            left=self.roadways_df,
            right=self.base_results.roadways_df[
                ["a", "b", "small truck volume", "large truck volume"]
            ],
            how="left",
            on=["a", "b"],
            suffixes=(" scenario", " baseline"),
        )
        self.roadways_df["small truck volume"] = self.roadways_df[
            "small truck volume baseline"
        ] * (1.0 + pct_change_vmt)
        self.roadways_df["large truck volume"] = self.roadways_df[
            "large truck volume baseline"
        ] * (1.0 + pct_change_vmt)
        # calculate the new truck cost with these assumed truck VMTs
        self.roadways_df["total truck cost"] = (
            self.roadways_df["small truck volume"]
            * self.roadways_df["smtropc"]
            * self.roadways_df["distance"]
            * 0.01
        ) + (
            self.roadways_df["large truck volume"]
            * self.roadways_df["lrtropc"]
            * self.roadways_df["distance"]
            * 0.01
        )

        # apply change in average auto cost per mile to base truck cost
        avg_auto_cost = self.daily_results[
            (
                "Other Travel Metrics (for reference only)",
                "Auto Average Stats",
                "Average Auto OpCost ($2000) per Mile",
            )
        ]
        base_avg_auto_cost = self.base_results.daily_results[
            (
                "Other Travel Metrics (for reference only)",
                "Auto Average Stats",
                "Average Auto OpCost ($2000) per Mile",
        ...
        base_truck_cost = self.base_results.daily_results[
            cat1, cat2, "Truck - Computed"
        ]
        self.daily_results[cat1, cat2, "Truck - Computed"] = base_truck_cost * (
            1.0 + pct_change_cost
        )
        self.daily_results[
            "Accessibility Benefits (other)",
            "Non-Household",
            "Cost - Truck ($2000) - Computed",
        ] = base_truck_cost * (1.0 + pct_change_cost)

        cat1 = "Health Benefits"
        cat2 = "Noise"
        self.daily_results[cat1, cat2, "Truck VMT - Computed"] = (
            1.0 + pct_change_vmt
        ) * base_truck_vmt
    ...
    def calculateConsumerSurplus(
        ...
        if ("Zero Logsum TAZs" in config) or ("Zero Negative Logsum TAZs" in config):
            ...
            if "Zero Negative Logsum TAZs" in config:
                zero_neg_taz_list = RunResults.parseNumList(
                    str(config["Zero Negative Logsum TAZs"])
                )
                print(
                    "Zeroing out negative diffs for tazs {}".format(zero_neg_taz_list)
                )
        ...
        mandatoryAccessibilities = pd.merge(
            mandatoryAccessibilities, base_mandatoryAccessibilities, how="left"
        )
        mandatoryAccessibilities["diff_dclogsum"] = (
            mandatoryAccessibilities["scen_dclogsum"]
            - mandatoryAccessibilities["base_dclogsum"]
        )

        # zero out negative diffs if directed
        if len(zero_neg_taz_list) > 0:
            mandatoryAccessibilities.loc[
                (mandatoryAccessibilities.taz.isin(zero_neg_taz_list))
                & (mandatoryAccessibilities.diff_dclogsum < 0),
                "diff_dclogsum",
            ] = 0.0
        ...
        if len(zero_taz_list) > 0:
            mandatoryAccessibilities.loc[
                mandatoryAccessibilities.taz.isin(zero_taz_list), "diff_dclogsum"
            ] = 0.0

        mandatoryAccessibilities["logsum_diff_minutes"] = (
            mandatoryAccessibilities.diff_dclogsum / 0.0134
        )
        ...
        if mand_ldm_max < 0.00001:
            ...
            mandatoryAccessibilities["ldm_ratio"] = (
                mandatoryAccessibilities["logsum_diff_minutes"].abs() / mand_ldm_max
            )  # how big is the magnitude compared to max magnitude?
            mandatoryAccessibilities["ldm_mult"] = 1.0 / (
                1.0
                + numpy.exp(
                    -(mandatoryAccessibilities["ldm_ratio"] - RunResults.CEM_THRESHOLD)
                    / RunResults.CEM_SHALLOW
                )
            )
            mandatoryAccessibilities["ldm_cem"] = (
                mandatoryAccessibilities["logsum_diff_minutes"]
                * mandatoryAccessibilities["ldm_mult"]
            )

        # This too
        nonmandatoryAccessibilities = pd.merge(
            nonmandatoryAccessibilities, base_nonmandatoryAccessibilities, how="left"
        )
        nonmandatoryAccessibilities["diff_dclogsum"] = (
            nonmandatoryAccessibilities["scen_dclogsum"]
            - nonmandatoryAccessibilities["base_dclogsum"]
        )

        # zero out negative diffs if directed
        if len(zero_neg_taz_list) > 0:
            nonmandatoryAccessibilities.loc[
                (nonmandatoryAccessibilities.taz.isin(zero_neg_taz_list))
                & (nonmandatoryAccessibilities.diff_dclogsum < 0),
                "diff_dclogsum",
            ] = 0.0
        ...
        if len(zero_taz_list) > 0:
            nonmandatoryAccessibilities.loc[
                nonmandatoryAccessibilities.taz.isin(zero_taz_list), "diff_dclogsum"
            ] = 0.0

        nonmandatoryAccessibilities["logsum_diff_minutes"] = (
            nonmandatoryAccessibilities["diff_dclogsum"] / 0.0175
        )

        # Cliff Effect Mitigation
        nonmm_ldm_max = nonmandatoryAccessibilities["logsum_diff_minutes"].abs().max()
        nonmandatoryAccessibilities["ldm_ratio"] = (
            nonmandatoryAccessibilities["logsum_diff_minutes"].abs() / nonmm_ldm_max
        )  # how big is the magnitude compared to max magnitude?
        nonmandatoryAccessibilities["ldm_mult"] = 1.0 / (
            1.0
            + numpy.exp(
                -(nonmandatoryAccessibilities["ldm_ratio"] - RunResults.CEM_THRESHOLD)
                / RunResults.CEM_SHALLOW
            )
        )
        nonmandatoryAccessibilities["ldm_cem"] = (
            nonmandatoryAccessibilities["logsum_diff_minutes"]
            * nonmandatoryAccessibilities["ldm_mult"]
        )

        # merge scenario + base accessbiity markets data
        accessibilityMarkets = pd.merge(
            accessibilityMarkets, base_accessibilityMarkets, how="left"
        )

        mandatoryAccess = pd.merge(
            mandatoryAccessibilities, accessibilityMarkets, how="left"
        )
        mandatoryAccess.fillna(0, inplace=True)

        nonmandatoryAccess = pd.merge(
            nonmandatoryAccessibilities, accessibilityMarkets, how="left"
        )
        ...
        for inclabel in ["lowInc", "medInc", "highInc", "veryHighInc"]:
            daily_results[(cat1, cat2, inclabel)] = (
                mandatoryAccess.loc[
        ...
        for inclabel in ["lowInc", "medInc", "highInc", "veryHighInc"]:
            daily_results[(cat1, cat2, inclabel)] = (
                nonmandatoryAccess.loc[
                    mandatoryAccess.incQ_label == inclabel, "CS diff all"
                ].sum()
                / 60.0
            )
        ...
        debug_cem_df = pd.concat(
            objs=[
                mandatoryAccess.rename(columns={"CS diff work/school": "CS diff min"}),
                nonmandatoryAccess.rename(columns={"CS diff all": "CS diff min"}),
            ],
            axis="index",
            sort=True,
        )
        ...
        mandatoryAccess["CS diff work/school"] = (
            0.5 * mandatoryAccess.base_num_workers_students
            + 0.5 * mandatoryAccess.scen_num_workers_students
        ) * mandatoryAccess.logsum_diff_minutes
        for inclabel in ["lowInc", "medInc", "highInc", "veryHighInc"]:
            daily_results[(cat1, cat2, inclabel)] = (
                mandatoryAccess.loc[
                    mandatoryAccess.incQ_label == inclabel, "CS diff work/school"
        ...
        nonmandatoryAccess["CS diff all"] = (
            0.5 * nonmandatoryAccess.base_num_persons
            + 0.5 * nonmandatoryAccess.scen_num_persons
        ) * nonmandatoryAccess.logsum_diff_minutes
        for inclabel in ["lowInc", "medInc", "highInc", "veryHighInc"]:
            daily_results[(cat1, cat2, inclabel)] = (
                nonmandatoryAccess.loc[
                    mandatoryAccess.incQ_label == inclabel, "CS diff all"
        ...
        if debug_dir:
            ...
            debug_df = pd.concat(
                objs=[debug_cem_df, debug_nocem_df], axis="index", sort=True
            )
            ...
            cs_tableau_filename = os.path.join(
                debug_dir,
                "consumer_surplus_{}.twb".format(config["Foldername - Future"]),
            )
    ...
    def calculateDailyMetrics(self):
        ...
        vmt_byclass = self.vmt_vht_metrics.sum(
            level="vehicle class"
        )  # vehicles, not people
        transit_byclass = self.transit_times_by_acc_mode_egr.loc["all"].sum(
            level="Mode"
        )
        transit_byaceg = self.transit_times_by_acc_mode_egr.loc["all"].sum(
            level=["Access", "Egress"]
        )
        ...
        if self.base_results:
            (
                self.mandatoryAccessibilities,
                self.nonMandatoryAccessibilities,
                self.mandatoryAccess,
                self.nonmandatoryAccess,
            ) = RunResults.calculateConsumerSurplus(
                self.config,
                daily_results,
                self.mandatoryAccessibilities,
                ...
                debug_dir=os.path.join(self.config["Project Run Dir"], "..", "logsums"),
            )  # project run dir includes is run_dir\OUTPUT\metrics
        ##########################################################################################
        ...
        daily_results[(cat1, cat2, "Time - Auto (PHT) - IX/EX")] = (
            auto_byclass.loc[
                ["da_ix", "datoll_ix", "sr2_ix", "sr2toll_ix", "sr3_ix", "sr3toll_ix"],
                "Person Minutes",
            ].sum()
            / 60.0
        )
        ...
        daily_results[(cat1, cat2, "Time - Truck (Computed VHT)")] = vmt_byclass.loc[
            ["sm", "smt", "hv", "hvt"], "VHT"
        ].sum()
        ...
        daily_results[
            (cat1, cat2, "Cost - Truck ($2000) - Computed")
        ] = self.roadways_df["total truck cost"].sum()

        cat2 = "Adding Back Societal Transfers"
        self.transit_times_by_mode_income["Total Cost"] = (
            self.transit_times_by_mode_income["Daily Trips"]
            * self.transit_times_by_mode_income["Avg Cost"]
        )
        daily_results[
            (cat1, cat2, "Transit Fares ($2000)")
        ] = self.transit_times_by_mode_income["Total Cost"].sum()
        ...
        quick_summary["Transit Fares (2000$)"] = daily_results[
            (cat1, cat2, "Transit Fares ($2000)")
        ]
        quick_summary["Transit Fares (2019$)"] = (
            daily_results[(cat1, cat2, "Transit Fares ($2000)")] * 1.6
        )

        quick_summary["Bridge Tolls (2000$): Auto HH"] = daily_results[
            (cat1, cat2, "Auto Households - Bridge Tolls ($2000)")
        ]
        quick_summary["Bridge Tolls (2000$): Total"] = (
            quick_summary["Bridge Tolls (2000$): Auto HH"] + bridge_tolls_nonhh
        )
        quick_summary["Bridge Tolls (2019$): Auto HH"] = (
            quick_summary["Bridge Tolls (2000$): Auto HH"] * 1.6
        )
        quick_summary["Bridge Tolls (2019$): Total"] = (
            quick_summary["Bridge Tolls (2000$): Total"] * 1.6
        )

        quick_summary["Value Tolls (2000$): Auto HH"] = daily_results[
            (cat1, cat2, "Auto Households - Value Tolls ($2000)")
        ]
        quick_summary["Value Tolls (2000$): Total"] = (
            daily_results[(cat1, cat2, "Auto Households - Value Tolls ($2000)")]
            + value_tolls_nonhh
        )
        quick_summary["Value Tolls (2019$): Auto HH"] = (
            quick_summary["Value Tolls (2000$): Auto HH"] * 1.6
        )
        quick_summary["Value Tolls (2019$): Total"] = (
            quick_summary["Value Tolls (2000$): Total"] * 1.6
        )
        ...
        daily_results[(cat1, cat2, "Auto (Person Hours)")] = (
            ...
            + vmt_byclass.loc[
                ["s2", "s2t", "s2av"], "Non-Recurring Freeway Delay"
            ].sum()
            * 2
            + vmt_byclass.loc[
                ["s3", "s3t", "s3av"], "Non-Recurring Freeway Delay"
            ].sum()
            * 3.5
        )
        daily_results[(cat1, cat2, "Truck (Computed VH)")] = vmt_byclass.loc[
            ["sm", "smt", "hv", "hvt"], "Non-Recurring Freeway Delay"
        ].sum()
        ...
        daily_results[(cat1, cat2, "BART (incl eBART)")] = (
            self.crowding_df.loc[
                self.crowding_df["SYSTEM"] == "BART", "effective_ivtt_metrolinx_max2pt5"
            ].sum()
            - self.crowding_df.loc[
                self.crowding_df["SYSTEM"] == "BART", "ivtt_hours"
            ].sum()
            + self.crowding_df.loc[
                self.crowding_df["SYSTEM"] == "EBART",
                "effective_ivtt_metrolinx_max2pt5",
            ].sum()
            - self.crowding_df.loc[
                self.crowding_df["SYSTEM"] == "EBART", "ivtt_hours"
            ].sum()
        )

        daily_results[(cat1, cat2, "Caltrain")] = (
            self.crowding_df.loc[
                self.crowding_df["SYSTEM"] == "Caltrain",
                "effective_ivtt_metrolinx_max2pt5",
            ].sum()
            - self.crowding_df.loc[
                self.crowding_df["SYSTEM"] == "Caltrain", "ivtt_hours"
            ].sum()
        )

        daily_results[(cat1, cat2, "SF Muni")] = (
            self.crowding_df.loc[
                self.crowding_df["SYSTEM"] == "SF MUNI",
                "effective_ivtt_metrolinx_max2pt5",
            ].sum()
            - self.crowding_df.loc[
                self.crowding_df["SYSTEM"] == "SF MUNI", "ivtt_hours"
            ].sum()
            + self.crowding_df.loc[
                self.crowding_df["SYSTEM"] == "SF Muni Cable Car",
                "effective_ivtt_metrolinx_max2pt5",
            ].sum()
            - self.crowding_df.loc[
                self.crowding_df["SYSTEM"] == "SF Muni Cable Car", "ivtt_hours"
            ].sum()
            + self.crowding_df.loc[
                self.crowding_df["SYSTEM"] == "SF Muni Local",
                "effective_ivtt_metrolinx_max2pt5",
            ].sum()
            - self.crowding_df.loc[
                self.crowding_df["SYSTEM"] == "SF Muni Local", "ivtt_hours"
            ].sum()
            + self.crowding_df.loc[
                self.crowding_df["SYSTEM"] == "SF Muni Metro",
                "effective_ivtt_metrolinx_max2pt5",
            ].sum()
            - self.crowding_df.loc[
                self.crowding_df["SYSTEM"] == "SF Muni Metro", "ivtt_hours"
            ].sum()
        )
        daily_results[(cat1, cat2, "VTA LRT")] = (
            self.crowding_df.loc[
                self.crowding_df["SYSTEM"] == "VTA LRT",
                "effective_ivtt_metrolinx_max2pt5",
            ].sum()
            - self.crowding_df.loc[
                self.crowding_df["SYSTEM"] == "VTA LRT", "ivtt_hours"
            ].sum()
        )

        daily_results[(cat1, cat2, "AC Transit")] = (
            self.crowding_df.loc[
                self.crowding_df["SYSTEM"] == "AC Transit",
                "effective_ivtt_metrolinx_max2pt5",
            ].sum()
            - self.crowding_df.loc[
                self.crowding_df["SYSTEM"] == "AC Transit", "ivtt_hours"
            ].sum()
        )

        daily_results[(cat1, cat2, "Other")] = (
            self.crowding_df["effective_ivtt_metrolinx_max2pt5"].sum()
            - self.crowding_df["ivtt_hours"].sum()
            - daily_results[(cat1, cat2, "BART (incl eBART)")]
            - daily_results[(cat1, cat2, "SF Muni")]
            - daily_results[(cat1, cat2, "Caltrain")]
            - daily_results[(cat1, cat2, "VTA LRT")]
            ...
            - (
                self.crowding_df.loc[
                    self.crowding_df["SYSTEM"] == "VTA Shuttle",
                    "effective_ivtt_metrolinx_max2pt5",
                ].sum()
                - self.crowding_df.loc[
                    self.crowding_df["SYSTEM"] == "VTA Shuttle", "ivtt_hours"
                ].sum()
            )
            - (
                self.crowding_df.loc[
                    self.crowding_df["SYSTEM"] == "Broadway Shuttle",
                    "effective_ivtt_metrolinx_max2pt5",
                ].sum()
                - self.crowding_df.loc[
                    self.crowding_df["SYSTEM"] == "Broadway Shuttle", "ivtt_hours"
                ].sum()
            )
            - (
                self.crowding_df.loc[
                    self.crowding_df["SYSTEM"] == "Caltrain Shuttle",
                    "effective_ivtt_metrolinx_max2pt5",
                ].sum()
                - self.crowding_df.loc[
                    self.crowding_df["SYSTEM"] == "Caltrain Shuttle", "ivtt_hours"
                ].sum()
            )
            - (
                self.crowding_df.loc[
                    self.crowding_df["SYSTEM"] == "Stanford Shuttle",
                    "effective_ivtt_metrolinx_max2pt5",
                ].sum()
                - self.crowding_df.loc[
                    self.crowding_df["SYSTEM"] == "Stanford Shuttle", "ivtt_hours"
                ].sum()
            )
            - (
                self.crowding_df.loc[
                    self.crowding_df["SYSTEM"] == "Emery Go Round",
                    "effective_ivtt_metrolinx_max2pt5",
                ].sum()
                - self.crowding_df.loc[
                    self.crowding_df["SYSTEM"] == "Emery Go Round", "ivtt_hours"
                ].sum()
            )
        )
        ...
        if "base_dir" in self.config.keys():
            ...
            daily_results[(cat1, cat2, "Wetland")] = self.natural_land.get(
                "wetland (acres)"
            )
            daily_results[(cat1, cat2, "Forestland")] = self.natural_land.get(
                "forestland (acres)"
            )
            daily_results[(cat1, cat2, "Pastureland")] = self.natural_land.get(
                "pastureland (acres)"
            )
            daily_results[(cat1, cat2, "Agricultural Land")] = self.natural_land.get(
        ...
        daily_results[(cat1, cat2, "Motor Vehicle")] = vmt_byclass.loc[
        ...
        daily_results[(cat1, cat2, "Property Damage")] = vmt_byclass.loc[
            :, "Motor Vehicle Property"
        ].sum()

        if "base_dir" in self.config.keys():
            cat2 = "CRF-based reduction in Collisions"
            daily_results[(cat1, cat2, "Fatalities")] = -self.collisions_switrs.get(
                "annual_fatalities_reduction"
            )
            daily_results[(cat1, cat2, "Injuries")] = -self.collisions_switrs.get(
                "annual_injuries_reduction"
            )
            # except:
        ...
        daily_results[(cat1, cat2, "NOX (tons)")] = (
            vmt_byclass.loc[:, "W_NOx"].sum() * RunResults.METRIC_TONS_TO_US_TONS
            + vmt_byclass.loc[:, "S_NOx"].sum() * RunResults.METRIC_TONS_TO_US_TONS
        )
        daily_results[(cat1, cat2, "SO2 (tons)")] = (
            vmt_byclass.loc[:, "SOx"].sum() * RunResults.METRIC_TONS_TO_US_TONS
        )

        # http://en.wikipedia.org/wiki/Volatile_organic_compound
        daily_results[(cat1, cat2, "VOC: Acetaldehyde (metric tons)")] = (
            vmt_byclass.loc[:, "Acetaldehyde"].sum() * RunResults.METRIC_TONS_TO_US_TONS
        )
        daily_results[(cat1, cat2, "VOC: Benzene (metric tons)")] = (
            vmt_byclass.loc[:, "Benzene"].sum() * RunResults.METRIC_TONS_TO_US_TONS
        )
        daily_results[(cat1, cat2, "VOC: 1,3-Butadiene (metric tons)")] = (
            vmt_byclass.loc[:, "Butadiene"].sum() * RunResults.METRIC_TONS_TO_US_TONS
        )
        daily_results[(cat1, cat2, "VOC: Formaldehyde (metric tons)")] = (
            vmt_byclass.loc[:, "Formaldehyde"].sum() * RunResults.METRIC_TONS_TO_US_TONS
        )
        daily_results[(cat1, cat2, "All other VOC (metric tons)")] = (
            vmt_byclass.loc[:, "ROG"].sum() * RunResults.METRIC_TONS_TO_US_TONS
            - daily_results[(cat1, cat2, "VOC: Acetaldehyde (metric tons)")]
            - daily_results[(cat1, cat2, "VOC: Benzene (metric tons)")]
            - daily_results[(cat1, cat2, "VOC: 1,3-Butadiene (metric tons)")]
            - daily_results[(cat1, cat2, "VOC: Formaldehyde (metric tons)")]
        ...
        active_cat2 = cat2
        nonmot_byclass_2064 = self.nonmot_times.loc["20-64"].sum(
            level="Mode"
        )  # person trips
        nonmot_byclass_2074 = self.nonmot_times.loc["20-74"].sum(
            level="Mode"
        )  # person trips
        transit_byaceg_2074 = self.transit_times_by_acc_mode_egr.loc["20-74"].sum(
            level=["Access", "Egress"]
        )

        daily_results[(cat1, cat2, "Bike (20-64yrs cyclists)")] = (
            nonmot_byclass_2064.loc["Bike", "Total Time (Hours)"]
            * 60.0
            / self.unique_active_travelers["unique_cyclists_2064"]
        )
        daily_results[(cat1, cat2, "Walk (20-74yrs walkers)")] = (
            nonmot_byclass_2074.loc["Walk", "Total Time (Hours)"]
            * 60.0
            / self.unique_active_travelers["unique_walkers_2074"]
        )
        daily_results[(cat1, cat2, "Transit (20-74yrs transit riders)")] = (
            (
                transit_byaceg_2074.loc[:, "Walk acc & egr hours"].sum()
                + transit_byaceg_2074.loc[:, "Aux walk hours"].sum()
            )
            * 60.0
            / self.unique_active_travelers["unique_transiters_2074"]
        )

        cat2 = "Active Individuals (Morbidity)"
        # Really these are active addults
        daily_results[(cat1, cat2, "Total")] = self.unique_active_travelers[
            "number_active_adults"
        ]
        ...
        daily_results[(cat1, cat2, "Bike (20-64yrs cyclists)")] = (
            daily_results[(cat1, active_cat2, "Bike (20-64yrs cyclists)")]
            * 5.0
            / RunResults.CYCLING_REF_WEEKLY_MIN
        ) * (1.0 - RunResults.CYCLING_RELATIVE_RISK)
        daily_results[(cat1, cat2, "Walk (20-74yrs walkers)")] = (
            daily_results[(cat1, active_cat2, "Walk (20-74yrs walkers)")]
            * 5.0
            / RunResults.WALKING_REF_WEEKLY_MIN
        ) * (1.0 - RunResults.WALKING_RELATIVE_RISK)
        daily_results[(cat1, cat2, "Transit (20-74yrs transit riders)")] = (
            daily_results[(cat1, active_cat2, "Transit (20-74yrs transit riders)")]
            * 5.0
            / RunResults.WALKING_REF_WEEKLY_MIN
        ) * (1.0 - RunResults.WALKING_RELATIVE_RISK)
        ...
        daily_results[(cat1, cat2, "Bike (20-64yrs cyclists)")] = (
            daily_results[(cat1, epda_cat2, "Bike (20-64yrs cyclists)")]
            * (float(RunResults.BAY_AREA_MORTALITY_RATE_2064YRS) / 100000.0)
            * self.unique_active_travelers["unique_cyclists_2064"]
            / 10.0
        )
        daily_results[(cat1, cat2, "Walk (20-74yrs walkers)")] = (
            daily_results[(cat1, epda_cat2, "Walk (20-74yrs walkers)")]
            * (float(RunResults.BAY_AREA_MORTALITY_RATE_2074YRS) / 100000.0)
            * self.unique_active_travelers["unique_walkers_2074"]
            / 10.0
        )
        daily_results[(cat1, cat2, "Transit (20-74yrs transit riders)")] = (
            daily_results[(cat1, epda_cat2, "Transit (20-74yrs transit riders)")]
            * (float(RunResults.BAY_AREA_MORTALITY_RATE_2074YRS) / 100000.0)
            * self.unique_active_travelers["unique_transiters_2074"]
            / 10.0
        )
        ...
        # Noise
        cat2 = "Noise"
        daily_results[(cat1, cat2, "Auto VMT")] = vmt_byclass.loc[
            ["da", "dat", "s2", "s2t", "s3", "s3t", "daav", "s2av", "s3av"], "VMT"
        ].sum()
        # computed will get overwritten if base results
        daily_results[(cat1, cat2, "Truck VMT - Computed")] = vmt_byclass.loc[
            ["sm", "smt", "hv", "hvt"], "VMT"
        ].sum()
        ...
        daily_results[(cat1, cat2, "SOV (PHT)")] = vmt_byclass.loc[
            ["da", "dat", "daav"], "VHT"
        ].sum()
        daily_results[(cat1, cat2, "HOV2 (PHT)")] = (
            vmt_byclass.loc[["s2", "s2t", "s2av"], "VHT"].sum() * 2
        )
        daily_results[(cat1, cat2, "HOV3+ (PHT)")] = (
            vmt_byclass.loc[["s3", "s3t", "s3av"], "VHT"].sum() * 3.5
        )

        quick_summary["Auto SOV Hours (Person Hours)"] = vmt_byclass.loc[
            ["da", "dat", "daav"], "VHT"
        ].sum()
        quick_summary["Auto HOV2 Hours (Person Hours)"] = (
            vmt_byclass.loc[["s2", "s2t", "s2av"], "VHT"].sum() * 2
        )
        quick_summary["Auto HOV3 Hours (Person Hours)"] = (
            vmt_byclass.loc[["s3", "s3t", "s3av"], "VHT"].sum() * 3.5
        )
        quick_summary["Auto Hours (Person Hours)"] = (
            vmt_byclass.loc[["da", "dat", "daav"], "VHT"].sum()
            + vmt_byclass.loc[["s2", "s2t", "s2av"], "VHT"].sum() * 2
            + vmt_byclass.loc[["s3", "s3t", "s3av"], "VHT"].sum() * 3.5
        )
        ...
        cat2 = "Truck Hours"
        # computed will get overwritten if base results
        daily_results[(cat1, cat2, "Truck (Computed VHT)")] = vmt_byclass.loc[
            ["sm", "smt", "hv", "hvt"], "VHT"
        ].sum()
        daily_results[(cat1, cat2, "Truck (Modeled VHT)")] = vmt_byclass.loc[
            ["sm", "smt", "hv", "hvt"], "VHT"
        ].sum()
        # quick summary
        quick_summary["Vehicle Hours traveled -- SOV"] = vmt_byclass.loc[
            ["da", "dat", "daav"], "VHT"
        ].sum()
        quick_summary["Vehicle Hours traveled -- HOV2"] = vmt_byclass.loc[
            ["s2", "s2t", "s2av"], "VHT"
        ].sum()
        quick_summary["Vehicle Hours traveled -- HOV3"] = vmt_byclass.loc[
            ["s3", "s3t", "s3av"], "VHT"
        ].sum()
        quick_summary["Vehicle Hours traveled -- Trucks"] = vmt_byclass.loc[
            ["sm", "smt", "hv", "hvt"], "VHT"
        ].sum()
        ...
        daily_results[(cat1, cat2, "Truck (Modeled VH)")] = vmt_byclass.loc[
            ["sm", "smt", "hv", "hvt"], "Non-Recurring Freeway Delay"
        ].sum()
        # quick summary
        quick_summary[
            "Vehicle hours of non-recurring delay - passenger"
        ] = vmt_byclass.loc[
            ["da", "dat", "s2", "s2t", "s3", "s3t", "daav", "s2av", "s3av"],
            "Non-Recurring Freeway Delay",
        ].sum()
        quick_summary[
            "Vehicle hours of non-recurring delay - commercial (raw)"
        ] = vmt_byclass.loc[
            ["sm", "smt", "hv", "hvt"], "Non-Recurring Freeway Delay"
        ].sum()

        cat2 = "Transit In-Vehicle (Hours)"
        daily_results[(cat1, cat2, "Local Bus")] = transit_byclass.loc[
            "loc", "In-vehicle hours"
        ]
        daily_results[(cat1, cat2, "Light Rail/Ferry")] = transit_byclass.loc[
            "lrf", "In-vehicle hours"
        ]
        daily_results[(cat1, cat2, "Express Bus")] = transit_byclass.loc[
            "exp", "In-vehicle hours"
        ]
        daily_results[(cat1, cat2, "Heavy Rail")] = transit_byclass.loc[
            "hvy", "In-vehicle hours"
        ]
        daily_results[(cat1, cat2, "Commuter Rail")] = transit_byclass.loc[
            "com", "In-vehicle hours"
        ]
        quick_summary[
            "Transit person hours of travel -- in-vehicle"
        ] = transit_byclass.loc[:, "In-vehicle hours"].sum()
        quick_summary[
            "Transit person hours of travel -- out-of-vehicle"
        ] = transit_byclass.loc[:, "Out-of-vehicle hours"].sum()

        cat2 = "Transit Out-of-Vehicle (Hours)"
        daily_results[(cat1, cat2, "Walk Access+Egress")] = (
            transit_byclass.loc[:, "Walk acc & egr hours"].sum()
            + transit_byclass.loc[:, "Aux walk hours"].sum()
        )
        daily_results[(cat1, cat2, "Drive Access+Egress")] = transit_byclass.loc[
            :, "Drive acc & egr hours"
        ].sum()
        daily_results[(cat1, cat2, "Wait")] = (
            transit_byclass.loc[:, "Init wait hours"].sum()
            + transit_byclass.loc[:, "Xfer wait hours"].sum()
        )

        # Out-of-Vehicle adjustment
        auto_person_trips = auto_byclass.loc[
            [
                "da",
                "datoll",
                "da_av_toll",
        ...
        if self.is_base_dir:
            ...
            self.ovtt_adjustment = (
                transit_byclass.loc[
                    self.config.loc["Project Mode"], "Out-of-vehicle hours"
                ]
            ...
            raise Exception(
                "Invalid Project Mode:'%s'; Should be one of 'road','com','hvy','exp','lrf','loc'"
        ...
        if self.config.loc["Project Mode"] in ["com", "hvy", "exp", "lrf", "loc"]:
            ...
            daily_results[(cat1, cat2, "OVTT Adjustment")] = (
        ...
        daily_results[(cat1, cat2, "Walk")] = nonmot_byclass.loc[
            "Walk", "Total Time (Hours)"
        ]
        daily_results[(cat1, cat2, "Bike")] = nonmot_byclass.loc[
            "Bike", "Total Time (Hours)"
        ]

        cat1 = "Other Travel Metrics (for reference only)"

        cat2 = "Auto Average Stats"
        daily_results[(cat1, cat2, "Average Auto Speed")] = auto_byclass.loc[
            auto_byclass.index != "truck", "Vehicle Miles"
        ].sum() / (
            auto_byclass.loc[auto_byclass.index != "truck", "Vehicle Minutes"].sum()
            / 60.0
        )
        daily_results[(cat1, cat2, "Average Truck Speed")] = auto_byclass.loc[
            auto_byclass.index == "truck", "Vehicle Miles"
        ].sum() / (
            auto_byclass.loc[auto_byclass.index == "truck", "Vehicle Minutes"].sum()
            / 60.0
        ...
        cat2 = "VMT by class"
        daily_results[(cat1, cat2, "Auto non-AV")] = vmt_byclass.loc[
            ["da", "dat", "s2", "s2t", "s3", "s3t"], "VMT"
        ].sum()
        daily_results[(cat1, cat2, "Auto AV")] = vmt_byclass.loc[
            ["daav", "s2av", "s3av"], "VMT"
        ].sum()
        # computed will get overwritten if base results
        daily_results[(cat1, cat2, "Truck - Computed")] = vmt_byclass.loc[
            ["sm", "smt", "hv", "hvt"], "VMT"
        ].sum()
        ...
        # quick summary
        quick_summary["VMT -- passenger non-AV vehicles"] = vmt_byclass.loc[
            ["da", "dat", "s2", "s2t", "s3", "s3t"], "VMT"
        ].sum()
        quick_summary["VMT -- passenger AV vehicles"] = vmt_byclass.loc[
            ["daav", "s2av", "s3av"], "VMT"
        ].sum()
        quick_summary["VMT -- passenger all vehicles"] = vmt_byclass.loc[
            ["da", "dat", "s2", "s2t", "s3", "s3t", "daav", "s2av", "s3av"], "VMT"
        ].sum()
        quick_summary["VMT -- trucks"] = vmt_byclass.loc[
            ["sm", "smt", "hv", "hvt"], "VMT"
        ].sum()
        quick_summary["VMT -- all vehicles"] = vmt_byclass.loc[
            [
                "da",
                "dat",
                "s2",
        ...
        cat2 = "VMT by mode"
        daily_results[(cat1, cat2, "Auto SOV")] = auto_byclass.loc[
            [
                "da",
                "datoll",
                "da_av_toll",
        ...
        daily_results[(cat1, cat2, "Auto HOV2")] = auto_byclass.loc[
            [
                "sr2",
                "sr2toll",
                "s2_av_toll",
        ...
        daily_results[(cat1, cat2, "Auto HOV3+")] = auto_byclass.loc[
            [
                "sr3",
                "sr3toll",
                "s3_av_toll",
        ...
        daily_results[(cat1, cat2, "Auto ZOV")] = auto_byclass.loc[
            ["owned_zpv"], "Vehicle Miles"
        ].sum()
        daily_results[(cat1, cat2, "Auto TNC/Taxi")] = auto_byclass.loc[
            ["tnc_single", "tnc_shared", "taxi"], "Vehicle Miles"
        ].sum()
        daily_results[(cat1, cat2, "Auto TNC/Taxi ZOV")] = auto_byclass.loc[
            ["zpv_tnc"], "Vehicle Miles"
        ].sum()
        daily_results[(cat1, cat2, "Truck")] = auto_byclass.loc[
            "truck", "Vehicle Miles"
        ].sum()
        ...
        quick_summary["VMT -- Auto Total"] = (
            daily_results[(cat1, cat2, "Auto SOV")]
            + daily_results[(cat1, cat2, "Auto HOV2")]
            + daily_results[(cat1, cat2, "Auto HOV3+")]
            + daily_results[(cat1, cat2, "Auto ZOV")]
        )

        quick_summary["VMT -- TNC non-ZOV"] = auto_byclass.loc[
            ["tnc_single", "tnc_shared"], "Vehicle Miles"
        ].sum()
        quick_summary["VMT -- TNC ZOV"] = auto_byclass.loc[
            ["zpv_tnc"], "Vehicle Miles"
        ].sum()
        quick_summary["VMT -- TNC Total"] = auto_byclass.loc[
            ["tnc_single", "tnc_shared", "zpv_tnc"], "Vehicle Miles"
        ].sum()

        quick_summary["VMT -- ZOV Total"] = auto_byclass.loc[
            ["owned_zpv", "zpv_tnc"], "Vehicle Miles"
        ].sum()
        ...
        daily_results[(cat1, cat2, "Auto TNC/Taxi")] = auto_byclass.loc[
            ["tnc_single", "tnc_shared", "taxi"], "Daily Vehicle Trips"
        ].sum()
        daily_results[(cat1, cat2, "Auto ZOV")] = auto_byclass.loc[
            ["owned_zpv", "zpv_tnc"], "Daily Vehicle Trips"
        ].sum()
        ...
        daily_results[(cat1, cat2, "Interregional IX/EX")] = auto_byclass.loc[
            ["da_ix", "datoll_ix", "sr2_ix", "sr2toll_ix", "sr3_ix", "sr3toll_ix"],
            "Daily Vehicle Trips",
        ].sum()
        daily_results[(cat1, cat2, "Airport")] = auto_byclass.loc[
            [
                "da_air",
                "datoll_air",
                "sr2_air",
        ...
        daily_results[(cat1, cat2, "Auto TNC/Taxi")] = auto_byclass.loc[
            ["tnc_single", "tnc_shared", "taxi"], "Daily Person Trips"
        ].sum()
        daily_results[(cat1, cat2, "Auto ZOV")] = auto_byclass.loc[
            ["owned_zpv", "zpv_tnc"], "Daily Person Trips"
        ].sum()
        daily_results[(cat1, cat2, "Transit (Drive) Trips")] = transit_byaceg.loc[
            [("wlk", "drv"), ("drv", "wlk")], "Transit Trips"
        ].sum()
        daily_results[(cat1, cat2, "Transit (Walk) Trips")] = transit_byaceg.loc[
            ("wlk", "wlk"), "Transit Trips"
        ].sum()
        daily_results[(cat1, cat2, "Walk")] = nonmot_byclass.loc["Walk", "Daily Trips"]
        daily_results[(cat1, cat2, "Bike")] = nonmot_byclass.loc["Bike", "Daily Trips"]
        ...
        total_transittrips = (
            daily_results[(cat1, cat2, "Transit (Drive) Trips")]
        ...
        quick_summary["Person trips (total)"] = (
            auto_byclass.loc[
                [
                    "da",
                    "datoll",
        ...
        daily_results[(cat1, cat2, "Total")] = (
            total_autotrips
            * RunResults.ANNUALIZATION
            / RunResults.YEARLY_AUTO_TRIPS_PER_AUTO
        )
        ...
        daily_results[(cat1, cat2, "Auto TNC/Taxi")] = (
            0.01
            * auto_byclass.loc[["tnc_single", "tnc_shared", "taxi"], "Total Cost"].sum()
        )
        daily_results[(cat1, cat2, "Auto TNC/Taxi ZOV")] = max(
            0.01 * auto_byclass.loc["zpv_tnc", "Total Cost"].sum(), 1
        )

        # computed will get overwritten if base results
        daily_results[(cat1, cat2, "Truck - Computed")] = self.roadways_df[
            "total truck cost"
        ].sum()
        ...
        for countynum, countyname in RunResults.COUNTY_NUM_TO_NAME.iteritems():
            daily_results[
                (cat1, cat2, "($2000) Work Tours to %s" % countyname)
            ] = self.parking_costs.loc[
                (self.parking_costs.parking_category == "Work")
                & (self.parking_costs.dest_county == countynum),
                "parking_cost",
            ].sum()
        for countynum, countyname in RunResults.COUNTY_NUM_TO_NAME.iteritems():
            daily_results[
                (cat1, cat2, "($2000) Non-Work Tours to %s" % countyname)
            ] = self.parking_costs.loc[
                (self.parking_costs.parking_category == "Non-Work")
                & (self.parking_costs.dest_county == countynum),
                "parking_cost",
            ].sum()
        ...
        daily_results[(cat1, cat2, "BART")] = self.transit_boards_miles.loc[
            "hvy", "Daily Boardings"
        ].sum()
        daily_results[(cat1, cat2, "Commuter Rail")] = self.transit_boards_miles.loc[
            "com", "Daily Boardings"
        ].sum()
        daily_results[
            (cat1, cat2, "Light Rail / Ferry")
        ] = self.transit_boards_miles.loc["lrf", "Daily Boardings"].sum()
        daily_results[(cat1, cat2, "Express Bus")] = self.transit_boards_miles.loc[
            "exp", "Daily Boardings"
        ].sum()
        daily_results[(cat1, cat2, "Local Bus")] = self.transit_boards_miles.loc[
            "loc", "Daily Boardings"
        ].sum()
        ...
        transit_systems = [
            ...
            "SamTrans Local",
            "Dumbarton Express",
            "Golden Gate Transit Expre",
            "Golden Gate Transit Local",
            "County Connection Local",
            "WestCAT Express",
            "WestCAT Local",
            "Vallejo Transit Express",
            "Fairfield and Suisun Tran",
            ...
            "Vallejo Transit",
            "Union City Transit",
            "SMART Temporary Express",
            "VTA Community Bus",
            "San Leandro Links",
            "Palo Alto & Menlo Park Sh",  #'Berkeley','VTA Shuttle','Broadway Shuttle'  'Emery Go Round', 'Stanford Shuttle', 'Caltrain Shuttle', \
            "High Speed Rail",
            "ACE",
            "Amtrak Capitol Corridor",
            "Vallejo Baylink Ferry",
            "East Bay Ferries",
            "Golden Gate Ferry - Sausa",
            "Golden Gate Ferry - Larks",
            "Tiburon Ferry",
        ]

        for system in transit_systems:
            # crowding penalty = effective ivtt with crowding multiplier minus actual ivtt
            daily_results[(cat1, cat2, system)] = (
                self.crowding_df.loc[
                    self.crowding_df["SYSTEM"] == system,
                    "effective_ivtt_metrolinx_max2pt5",
                ].sum()
                - self.crowding_df.loc[
                    self.crowding_df["SYSTEM"] == system, "ivtt_hours"
                ].sum()
            )
        ...
        quick_summary["Transit boardings"] = self.transit_boards_miles.loc[
            :, "Daily Boardings"
        ].sum()
        quick_summary["BART boardings"] = self.transit_boards_miles.loc[
            "hvy", "Daily Boardings"
        ].sum()

        quick_summary["VTOLL Paths in AM - datoll"] = (
            float(self.auto_times.loc[("inc1", "datoll"), "VTOLL nonzero AM"])
            + float(self.auto_times.loc[("inc1", "da_av_toll"), "VTOLL nonzero AM"])
        ) * 4
        quick_summary["VTOLL Paths in AM - sr2toll"] = (
            float(self.auto_times.loc[("inc1", "sr2toll"), "VTOLL nonzero AM"])
            + float(self.auto_times.loc[("inc1", "s2_av_toll"), "VTOLL nonzero AM"])
        ) * 4
        quick_summary["VTOLL Paths in AM - sr3toll"] = (
            float(self.auto_times.loc[("inc1", "sr3toll"), "VTOLL nonzero AM"])
            + float(self.auto_times.loc[("inc1", "s3_av_toll"), "VTOLL nonzero AM"])
        ) * 4
        quick_summary["VTOLL Paths in MD - datoll"] = (
            float(self.auto_times.loc[("inc1", "datoll"), "VTOLL nonzero MD"])
            + float(self.auto_times.loc[("inc1", "da_av_toll"), "VTOLL nonzero MD"])
        ) * 4
        quick_summary["VTOLL Paths in MD - sr2toll"] = (
            float(self.auto_times.loc[("inc1", "sr2toll"), "VTOLL nonzero MD"])
            + float(self.auto_times.loc[("inc1", "s2_av_toll"), "VTOLL nonzero MD"])
        ) * 4
        quick_summary["VTOLL Paths in MD - sr3toll"] = (
            float(self.auto_times.loc[("inc1", "sr3toll"), "VTOLL nonzero MD"])
            + float(self.auto_times.loc[("inc1", "s3_av_toll"), "VTOLL nonzero MD"])
        ) * 4

        quick_summary["Peak Vehicle Volume AM Bay Bridge"] = self.roadways_df.loc[
            (self.roadways_df["a"] == 2783) & (self.roadways_df["b"] == 6972),
            "volAM_tot",
        ].sum()
        quick_summary["Peak Vehicle Volume AM San Mateo Bridge"] = self.roadways_df.loc[
            (self.roadways_df["a"] == 3650) & (self.roadways_df["b"] == 6381),
            "volAM_tot",
        ].sum()
        quick_summary["Peak Vehicle Volume AM Dumbarton Bridge"] = self.roadways_df.loc[
            (self.roadways_df["a"] == 3880) & (self.roadways_df["b"] == 5938),
            "volAM_tot",
        ].sum()
        quick_summary[
            "Peak Vehicle Volume AM Golden Gate Bridge"
        ] = self.roadways_df.loc[
            (self.roadways_df["a"] == 7339) & (self.roadways_df["b"] == 7322),
            "volAM_tot",
        ].sum()
        quick_summary["Peak Vehicle Volume AM Rich SanRaf Bridge"] = (
            self.roadways_df.loc[
                (self.roadways_df["a"] == 2358) & (self.roadways_df["b"] == 2342),
                "volAM_tot",
            ].sum()
            + self.roadways_df.loc[
                (self.roadways_df["a"] == 10607) & (self.roadways_df["b"] == 13811),
                "volAM_tot",
            ].sum()
        )
        quick_summary["Peak Vehicle Volume AM SR37"] = (
            self.roadways_df.loc[
                (self.roadways_df["a"] == 11726) & (self.roadways_df["b"] == 11739),
                "volAM_tot",
            ].sum()
            + self.roadways_df.loc[
                (self.roadways_df["a"] == 12481) & (self.roadways_df["b"] == 12496),
                "volAM_tot",
            ].sum()
            + self.roadways_df.loc[
                (self.roadways_df["a"] == 12489) & (self.roadways_df["b"] == 12500),
                "volAM_tot",
            ].sum()
        )
        quick_summary["Peak Vehicle Volume AM SR37 alternate"] = self.roadways_df.loc[
            (self.roadways_df["a"] == 8639) & (self.roadways_df["b"] == 8640),
            "volAM_tot",
        ].sum()
        quick_summary["Peak Vehicle Volume AM Carquinez Bridge"] = (
            self.roadways_df.loc[
                (self.roadways_df["a"] == 11634) & (self.roadways_df["b"] == 2179),
                "volAM_tot",
            ].sum()
            + self.roadways_df.loc[
                (self.roadways_df["a"] == 9285) & (self.roadways_df["b"] == 9225),
                "volAM_tot",
            ].sum()
        )
        quick_summary["Peak Vehicle Volume AM Benicia Bridge"] = self.roadways_df.loc[
            (self.roadways_df["a"] == 2131) & (self.roadways_df["b"] == 2136),
            "volAM_tot",
        ].sum()
        quick_summary["Peak Vehicle Volume AM Antioch Bridge"] = self.roadways_df.loc[
            (self.roadways_df["a"] == 1621) & (self.roadways_df["b"] == 1674),
            "volAM_tot",
        ].sum()
        quick_summary["Peak Vehicle Volume AM SR237"] = (
            self.roadways_df.loc[
                (self.roadways_df["a"] == 5862) & (self.roadways_df["b"] == 5869),
                "volAM_tot",
            ].sum()
            + self.roadways_df.loc[
                (self.roadways_df["a"] == 20198) & (self.roadways_df["b"] == 9048),
                "volAM_tot",
            ].sum()
        )
        quick_summary[
            "Peak Vehicle Volume AM Southern Crossing"
        ] = self.roadways_df.loc[
            (self.roadways_df["a"] == 10821) & (self.roadways_df["b"] == 10815),
            "volAM_tot",
        ].sum()
        quick_summary["Peak Vehicle Volume AM Mid-Bay Bridge"] = (
            self.roadways_df.loc[
                (self.roadways_df["a"] == 10947) & (self.roadways_df["b"] == 10951),
                "volAM_tot",
            ].sum()
            + self.roadways_df.loc[
                (self.roadways_df["a"] == 10948) & (self.roadways_df["b"] == 10952),
                "volAM_tot",
            ].sum()
        )

        self.roadways_df["vol24HR_tot"] = (
            self.roadways_df["volEA_tot"]
            + self.roadways_df["volAM_tot"]
            + self.roadways_df["volMD_tot"]
            + self.roadways_df["volPM_tot"]
            + self.roadways_df["volEV_tot"]
        )

        quick_summary["Daily Vehicle Volume Bay Bridge"] = self.roadways_df.loc[
            (self.roadways_df["a"] == 2783) & (self.roadways_df["b"] == 6972),
            "vol24HR_tot",
        ].sum()
        quick_summary["Daily Vehicle Volume San Mateo Bridge"] = self.roadways_df.loc[
            (self.roadways_df["a"] == 3650) & (self.roadways_df["b"] == 6381),
            "vol24HR_tot",
        ].sum()
        quick_summary["Daily Vehicle Volume Dumbarton Bridge"] = self.roadways_df.loc[
            (self.roadways_df["a"] == 3880) & (self.roadways_df["b"] == 5938),
            "vol24HR_tot",
        ].sum()
        quick_summary["Daily Vehicle Volume Golden Gate Bridge"] = self.roadways_df.loc[
            (self.roadways_df["a"] == 7339) & (self.roadways_df["b"] == 7322),
            "vol24HR_tot",
        ].sum()
        quick_summary["Daily Vehicle Volume Rich SanRaf Bridge"] = (
            self.roadways_df.loc[
                (self.roadways_df["a"] == 2358) & (self.roadways_df["b"] == 2342),
                "vol24HR_tot",
            ].sum()
        ...
        quick_summary["RSR Bridge EB"] = (
            self.roadways_df.loc[
                (self.roadways_df["a"] == 7853) & (self.roadways_df["b"] == 2341),
                "vol24HR_tot",
            ].sum()
            + self.roadways_df.loc[
                (self.roadways_df["a"] == 13813) & (self.roadways_df["b"] == 13810),
                "vol24HR_tot",
            ].sum()
        )

        quick_summary["RSR Bridge WB SOV"] = self.roadways_df.loc[
            (self.roadways_df["a"] == 2358) & (self.roadways_df["b"] == 2342),
            "vol24HR_tot",
        ].sum()
        quick_summary["RSR Bridge EB SOV"] = self.roadways_df.loc[
            (self.roadways_df["a"] == 7853) & (self.roadways_df["b"] == 2341),
            "vol24HR_tot",
        ].sum()
        quick_summary["RSR Bridge WB HOV"] = self.roadways_df.loc[
            (self.roadways_df["a"] == 10607) & (self.roadways_df["b"] == 13811),
            "vol24HR_tot",
        ].sum()
        quick_summary["RSR Bridge EB HOV"] = self.roadways_df.loc[
            (self.roadways_df["a"] == 13813) & (self.roadways_df["b"] == 13810),
            "vol24HR_tot",
        ].sum()

        quick_summary["Daily Vehicle Volume SR37"] = (
            self.roadways_df.loc[
                (self.roadways_df["a"] == 11726) & (self.roadways_df["b"] == 11739),
                "vol24HR_tot",
            ].sum()
        ...
        quick_summary["Daily Vehicle Volume SR37 GP"] = (
            self.roadways_df.loc[
                (self.roadways_df["a"] == 11726) & (self.roadways_df["b"] == 11739),
                "vol24HR_tot",
            ].sum()
        ...
        quick_summary["Daily Vehicle Volume SR37 HOV"] = self.roadways_df.loc[
            (self.roadways_df["a"] == 12489) & (self.roadways_df["b"] == 12500),
            "vol24HR_tot",
        ].sum()

        quick_summary["Daily Vehicle Volume SR37 alternate"] = self.roadways_df.loc[
            (self.roadways_df["a"] == 8639) & (self.roadways_df["b"] == 8640),
            "vol24HR_tot",
        ].sum()
        quick_summary["Daily Vehicle Volume Carquinez Bridge"] = (
            self.roadways_df.loc[
                (self.roadways_df["a"] == 11634) & (self.roadways_df["b"] == 2179),
                "vol24HR_tot",
            ].sum()
        ...
        quick_summary["Daily Vehicle Volume Benicia Bridge"] = self.roadways_df.loc[
            (self.roadways_df["a"] == 2131) & (self.roadways_df["b"] == 2136),
            "vol24HR_tot",
        ].sum()
        quick_summary["Daily Vehicle Volume Antioch Bridge"] = self.roadways_df.loc[
            (self.roadways_df["a"] == 1621) & (self.roadways_df["b"] == 1674),
            "vol24HR_tot",
        ].sum()
        quick_summary["Daily Vehicle Volume SR237"] = (
            self.roadways_df.loc[
                (self.roadways_df["a"] == 5862) & (self.roadways_df["b"] == 5869),
                "vol24HR_tot",
            ].sum()
        ...
        quick_summary["Daily Vehicle Volume Southern Crossing"] = self.roadways_df.loc[
            (self.roadways_df["a"] == 10821) & (self.roadways_df["b"] == 10815),
            "vol24HR_tot",
        ].sum()
        quick_summary["Daily Vehicle Volume Mid-Bay Bridge"] = (
            self.roadways_df.loc[
                (self.roadways_df["a"] == 10947) & (self.roadways_df["b"] == 10951),
                "vol24HR_tot",
            ].sum()
        ...
        quick_summary[
            "Peak Transbay Ridership AM BART Tunnel 1"
        ] = self.crowding_complete_df.loc[
            (self.crowding_complete_df["A"] == 15510)
            & (self.crowding_complete_df["B"] == 15511)
            & (self.crowding_complete_df["period"] == "AM"),
            "AB_VOL",
        ].sum()
        quick_summary[
            "Peak Transbay Ridership AM AC Transit Bay Bridge"
        ] = self.crowding_complete_df.loc[
            (self.crowding_complete_df["A"] == 2783)
            & (self.crowding_complete_df["B"] == 6972)
            & (self.crowding_complete_df["period"] == "AM"),
            "AB_VOL",
        ].sum()
        quick_summary["Peak Transbay Ridership AM BART Tunnel 2"] = max(
            self.crowding_complete_df.loc[
                (self.crowding_complete_df["A"] == 15553)
                & (self.crowding_complete_df["B"] == 15554)
                & (self.crowding_complete_df["period"] == "AM"),
                "AB_VOL",
            ].sum(),
            self.crowding_complete_df.loc[
                (self.crowding_complete_df["A"] == 15553)
                & (self.crowding_complete_df["B"] == 15556)
                & (self.crowding_complete_df["period"] == "AM"),
                "AB_VOL",
            ].sum(),
            self.crowding_complete_df.loc[
                (self.crowding_complete_df["A"] == 15553)
                & (self.crowding_complete_df["B"] == 20755)
                & (self.crowding_complete_df["period"] == "AM"),
                "AB_VOL",
            ].sum(),
        )
        quick_summary[
            "Peak Transbay Ridership AM Reg Rail Tunnel"
        ] = self.crowding_complete_df.loc[
            (self.crowding_complete_df["A"] == 14648)
            & (self.crowding_complete_df["B"] == 13654)
            & (self.crowding_complete_df["period"] == "AM"),
            "AB_VOL",
        ].sum()

        quick_summary["Daily Boardings - BART"] = (
            self.crowding_df.loc[self.crowding_df["SYSTEM"] == "BART", "AB_BRDA"].sum()
            + self.crowding_df.loc[
                self.crowding_df["SYSTEM"] == "EBART", "AB_BRDA"
            ].sum()
        )
        quick_summary["Daily Boardings - Caltrain"] = self.crowding_df.loc[
            self.crowding_df["SYSTEM"] == "Caltrain", "AB_BRDA"
        ].sum()
        quick_summary["Daily Boardings - Reg Rail"] = (
            self.crowding_df.loc[
                self.crowding_df["SYSTEM"] == "Caltrain", "AB_BRDA"
            ].sum()
            + self.crowding_df.loc[
                self.crowding_df["SYSTEM"] == "Amtrak Capitol Corridor", "AB_BRDA"
            ].sum()
        )
        quick_summary["Daily Boardings - AC Transit Transbay"] = self.crowding_df.loc[
            self.crowding_df["SYSTEM"] == "AC Transit", "AB_BRDA"
        ].sum()
        ...
        if "AM path count" in transit_byclass.columns.values:
            for mode in ["com", "hvy", "exp", "lrf", "loc"]:
                quick_summary["trn %s Paths in AM" % mode] = transit_byclass.loc[
                    mode, "AM path count"
                ].sum()
                quick_summary["trn %s Paths in MD" % mode] = transit_byclass.loc[
                    mode, "MD path count"
                ].sum()

        idx = pd.MultiIndex.from_tuples(
            daily_results.keys(), names=["category1", "category2", "variable_name"]
        )
    ...
    def calculateBenefitCosts(self, project_dir, all_projects_dir):
        ...
        if not self.is_base_dir and self.config.loc["base_dir"]:
            ...
            self.config["Project Compare ID"] = "%s vs %s" % (
                self.config.loc["Project ID"],
                self.config["Base Project ID"],
            )
            # print("base_match = [%s]" % base_match.group(0))
            workbook_name = "BC_%s_base%s.xlsx" % (
                self.config.loc["Project ID"],
                self.config["Base Project ID"],
            )
            csv_name = "BC_%s_base%s.csv" % (
                self.config.loc["Project ID"],
                self.config["Base Project ID"],
            )
        ...
        if self.base_dir:
            if self.config.loc["Compare"] == "baseline-scenario":
                base_minus_scen = self.writeBCWorksheet(
                    workbook, scen_minus_baseline=False
                )
        workbook.close()
        ...
        if self.base_dir:
            ...
            book.close()
            app.kill()
            df_lifecycleben = pd.read_excel(
                BC_detail_workbook, sheet_name="benefit_streams", header=None
            )

            # Getting 2060 Lifecycle benefits by category
            df_lifecycleben_2060 = df_lifecycleben
            df_lifecycleben_2060 = df_lifecycleben_2060.drop(
                df_lifecycleben_2060.index[[0, 1, 2, 3, 4]]
            )
            df_lifecycleben_2060 = df_lifecycleben_2060[[0, 1, 2, 4]]
            df_lifecycleben_2060.insert(
                3, "variable", "Lifecycle Benefits 2060 (PV 2019$)"
            )
            df_lifecycleben_2060.columns = [
                "category1",
                "category2",
                "category3",
                "variable_name",
            ...
            df_lifecycleben_2060 = df_lifecycleben_2060.set_index(
                ["category1", "category2", "category3", "variable_name"]
            )
            ...
            df_lifecycleben_2080 = df_lifecycleben_2080.drop(
                df_lifecycleben_2080.index[[0, 1, 2, 3, 4]]
            )
            df_lifecycleben_2080 = df_lifecycleben_2080[[0, 1, 2, 5]]
            df_lifecycleben_2080.insert(
                3, "variable", "Lifecycle Benefits 2080 (PV 2019$)"
            )
            df_lifecycleben_2080.columns = [
                "category1",
                "category2",
                "category3",
                "variable_name",
            ...
            df_lifecycleben_2080 = df_lifecycleben_2080.set_index(
                ["category1", "category2", "category3", "variable_name"]
            )
            ...
            bc_overall_tuples = [
                (
                    ...
                    "Horizon Year",
                    "bc_overall",
                    "Total Horizon Yr Benefit (2019$)",
                ),
                (
                    "bc_overall",
                    "Horizon Year",
                    "bc_overall",
                    "Total Horizon Yr Cost (2019$)",
                ),
                ...
                (
                    ...
                    "2060",
                    "bc_overall",
                    "Total Lifecycle Benefit (PV 2019$)",
                ),
            ...
            df_bc = pd.read_excel(
                BC_detail_workbook, sheet_name="scenario-baseline", header=None
            )
            bc_overall_array = numpy.asarray(
                [
                    df_bc.iloc[12, 8],
                    df_bc.iloc[13, 8],
                    df_bc.iloc[14, 8],
            ...
            )  # these are the cell locations of all higher level b/c metrics
            ...
            costs_tuples = [
                ("costs", "Lifecycle Costs (PV 2019$)", "2025-60", "Initial Capital"),
                ("costs", "Lifecycle Costs (PV 2019$)", "2025-60", "O&M"),
                (
                    "costs",
                    "Lifecycle Costs (PV 2019$)",
                    "2025-60",
                    "Rehab + Replacement",
                ),
            ...
            df_costs = pd.read_excel(
                BC_detail_workbook, sheet_name="cost_streams", header=None
            )
            costs_array = numpy.asarray(
                [
                    df_costs.iloc[2, 5],
                    df_costs.iloc[3, 5],
                    df_costs.iloc[1, 9],
            ...
            gp_tuples = [
                ("Guiding Principles", "Meets", "All", "All"),
    ...
    def writeBCWorksheet(self, workbook, scen_minus_baseline=True):
        ...
        worksheet.write(
            0,
            0,
            "This workbook is written by the script %s.  " % os.path.realpath(__file__)
            + "DO NOT CHANGE THE WORKBOOK, CHANGE THE SCRIPT.",
            format_red,
        )
        # Info rows
        format_label = workbook.add_format(
            {"align": "right", "indent": 1, "valign": "vcenter"}
        )
        format_highlight = workbook.add_format({"bg_color": "yellow"})
        format_highlight_file = workbook.add_format(
            {"bg_color": "yellow", "font_size": 8, "text_wrap": True}
        )
        format_highlight_money = workbook.add_format(
            {
                "bg_color": "yellow",
        ...
        worksheet.merge_range(
            1, 1, 1, 4, os.path.realpath(self.rundir), format_highlight_file
        )
        ...
        for key in RunResults.REQUIRED_KEYS:
            ...
            config_key = "%s" % key
            try:
                val = float(self.config.loc[config_key])
            except:
                val = self.config.loc[config_key]
        ...
        if self.base_dir:
            ...
            worksheet.merge_range(
                row, 1, row, 4, os.path.realpath(self.base_dir), format_highlight_file
            )
        ...
        if self.base_dir:
            ...
            worksheet.write(
                row, 10, "Analysis Period\nBenefit\n(2025-60)\n(2019$)", format_header
            )
            worksheet.write(
                row, 12, "Analysis Period\nBenefit\n(2025-80)\n(2019$)", format_header
            )
        ...
        format_cat1_sum = workbook.add_format(
            {
                "bold": True,
                "bg_color": "#C5D9F1",
        ...
        format_cat2 = workbook.add_format(
            {"bold": True, "indent": 1, "bg_color": "#D9D9D9"}
        )
        format_cat2_big = workbook.add_format(
            {"bg_color": "#92D050", "num_format": "#,##0"}
        )
        format_cat2_lil = workbook.add_format(
            {"bg_color": "#92D050", "num_format": "#,##0.000"}
        )
        ...
        format_cat2b_big = workbook.add_format(
            {"bg_color": "#FCD5B4", "num_format": "#,##0"}
        )
        format_cat2b_lil = workbook.add_format(
            {"bg_color": "#FCD5B4", "num_format": "#,##0.000"}
        )
        format_cat2d_big = workbook.add_format(
            {"bg_color": "#D9D9D9", "num_format": "#,##0"}
        )
        format_cat2d_lil = workbook.add_format(
            {"bg_color": "#D9D9D9", "num_format": "#,##0.000"}
        )

        format_benval_lil = workbook.add_format(
            {
                "num_format": '_($* #,##0.00_);_($* (#,##0.00);_($* "-"??_);_(@_)',
                "font_color": "#808080",
            }
        )
        format_benval_big = workbook.add_format(
            {
                "num_format": '_($* #,##0_);_($* (#,##0);_($* "-"??_);_(@_)',
                "font_color": "#808080",
            }
        )
        format_ann_ben = workbook.add_format(
            {"num_format": '_($* #,##0_);_($* (#,##0);_($* "-"??_);_(@_)'}
        )
        format_cat2_ben = workbook.add_format(
            {
                "num_format": '_($* #,##0_);_($* (#,##0);_($* "-"??_);_(@_)',
                "bg_color": "#D9D9D9",
            }
        )
        format_bc_header = workbook.add_format(
            {"bg_color": "#92D050", "align": "right", "bold": True}
        )
        format_bc_header_left = workbook.add_format(
            {"bg_color": "#92D050", "align": "left", "bold": True}
        )
        format_bc_header_center = workbook.add_format(
            {"bg_color": "#92D050", "align": "center", "bold": True}
        )

        format_bc_money = workbook.add_format(
            {
                "bg_color": "#92D050",
                "bold": True,
                "num_format": '_(\$* #,##0"M"_);_(\$* (#,##0"M");_(\$* "-"??_);_(@_)',
        ...
        format_bc_ratio = workbook.add_format(
            {"bg_color": "#92D050", "bold": True, "num_format": "0.00"}
        )

        format_equity = workbook.add_format({"bg_color": "#92D050", "bold": True})
        format_equityscore = workbook.add_format(
            {"bg_color": "#92D050", "bold": True, "num_format": "0%"}
        )
        format_equity_headers = workbook.add_format(
            {"bg_color": "#D9D9D9", "align": "left", "bold": True}
        )
        format_equity_headers_center = workbook.add_format(
            {"bg_color": "#D9D9D9", "align": "center", "bold": True}
        )
        #'bg_color':'#92D050'
        format_equitypop = workbook.add_format(
            {
                "bg_color": "#D9D9D9",
                "num_format": '_(* #,##0.0"M"_);_(* (#,##0.0"M");_(* "-"??_);_(@_)',
            }
        )
        format_equityben = workbook.add_format(
            {
                "bg_color": "#D9D9D9",
                "num_format": '_($* #,##0.00_);_($* (#,##0.00);_($* "-"??_);_(@_)',
            }
        )
        format_equitypct = workbook.add_format(
            {"bg_color": "#92D050", "bold": True, "align": "center", "num_format": "0%"}
        )
        ...
        for key, value in colA.daily_results.iteritems():
            ...
            if (key[0], key[1], key[2]) in self.BENEFIT_VALUATION:
                ...
                ("for reference only" in key[0])
                | ("Avg Minutes Active Transport per Person" in key[1])
            ...
            if (key[0], key[1], key[2]) in RunResults.ANNUALIZATION_FACTOR:
                annualization = RunResults.ANNUALIZATION_FACTOR[
                    (key[0], key[1], key[2])
                ]
                ...
                if self.base_dir:
                    ...
                    if (cat1_cellPV1) and ("for reference only" not in cat1):
                        worksheet.write(
                            cat1_cellPV1,
                            "=SUM(%s)/1000000"
                            % str(",").join(cat1_sumsPV1[cat1_cellPV1]),
                            format_cat1_sum,
                        )
                    ...
                    if (cat1_cellPV2) and ("for reference only" not in cat1):
                        worksheet.write(
                            cat1_cellPV2,
                            "=SUM(%s)/1000000"
                            % str(",").join(cat1_sumsPV2[cat1_cellPV2]),
                            format_cat1_sum,
                        )
            ...
            if cat2 != key[1]:
                ...
                if not already_diff:
                    worksheet.write(
                        ...
                        "=SUM(%s)"
                        % xl_range(
                            row + 1, 1, row + len(colA.daily_results[cat1][cat2]), 1
                        ),
                        format_cat2_lil
                        if (cat1, cat2) in self.lil_cats
                        else format_cat2_big,
                    )
                else:
                ...
                if self.base_dir:
                    if cat1 in colB.daily_results and cat2 in colB.daily_results[cat1]:
                        worksheet.write(
                            ...
                            "=SUM(%s)"
                            % xl_range(
                                row + 1, 2, row + len(colB.daily_results[cat1][cat2]), 2
                            ),
                            format_cat2b_lil
                            if (cat1, cat2) in self.lil_cats
                            else format_cat2b_big,
                        )
                    ...
                    if already_annual and already_diff:
                        ...
                        worksheet.write(
                            ...
                            "=SUM(%s)"
                            % xl_range(
                                row + 1, 4, row + len(colA.daily_results[cat1][cat2]), 4
                            ),
                            format_cat2d_lil
                            if (cat1, cat2) in self.lil_cats
                            else format_cat2d_big,
                        ...
                        already_annual
                        and cat1 in colB.daily_results
                        and cat2 in colB.daily_results[cat1]
                    ):
                        worksheet.write(
                            row,
                            3,  # diff daily
                        ...
                        worksheet.write(
                            ...
                            "=SUM(%s)"
                            % xl_range(
                                row + 1, 3, row + len(colA.daily_results[cat1][cat2]), 3
                            ),
                            format_cat2d_lil
                            if (cat1, cat2) in self.lil_cats
                            else format_cat2d_big,
                    ...
                    if valuation != None:
                        worksheet.write(
                            ...
                            "=SUM(%s)"
                            % xl_range(
                                row + 1, 8, row + len(colA.daily_results[cat1][cat2]), 8
                            ),
                            format_cat2_ben,
                        )
                        worksheet.write(
                            ...
                            % xl_range(
                                row + 1,
                                10,
                                row + len(colA.daily_results[cat1][cat2]),
                                10,
                            ),  # check this 10 at the end
                            format_cat2_ben,
                        )
                        worksheet.write(
                            row,
            ...
            if self.base_dir:
                ...
                if cat1 in colB.daily_results and cat2 in colB.daily_results[cat1]:
                    ...
                    nominal_diff = 0
                    bc_metrics[(cat1, cat2, key[2], colB_header)] = colB.daily_results[
                        cat1
                    ][cat2][key[2]]
                ...
                if already_annual and already_diff:
                    ...
                    nominal_diff = (
                        colA.daily_results[cat1][cat2][key[2]]
                        - colB.daily_results[cat1][cat2][key[2]]
                    )
                    bc_metrics[(cat1, cat2, key[2], "Annual Difference")] = nominal_diff
                    ...
                    if not already_diff:
                        ...
                        bc_metrics[(cat1, cat2, key[2], "Daily Difference")] = (
                            colA.daily_results[cat1][cat2][key[2]]
                            - colB.daily_results[cat1][cat2][key[2]]
                        )
                        if colB.daily_results[cat1][cat2][key[2]] != 0:
                            bc_metrics[
                                (cat1, cat2, key[2], "Daily Percent Difference")
                            ] = (
                                bc_metrics[(cat1, cat2, key[2], "Daily Difference")]
                                / colB.daily_results[cat1][cat2][key[2]]
                            )
                    ...
                    bc_metrics[(cat1, cat2, key[2], "Annual Difference")] = (
                        annualization
                        * bc_metrics[(cat1, cat2, key[2], "Daily Difference")]
                    )
                    ...
                    if already_diff:
                        nominal_diff = (
                            annualization * colA.daily_results[cat1][cat2][key[2]]
                        )
                    else:
                        nominal_diff = annualization * (
                            colA.daily_results[cat1][cat2][key[2]]
                            - colB.daily_results[cat1][cat2][key[2]]
                        )
                ...
                if valuation != None:
                    worksheet.write(
                        ...
                        valuation,
                        format_benval_lil
                        if abs(valuation) < 1000
                        else format_benval_big,
                    )
                    worksheet.write(
                        row,
                        8,  # annual benefit
                        "=%s*%s"
                        % (xl_rowcol_to_cell(row, 4), xl_rowcol_to_cell(row, 6)),
                        format_ann_ben,
                    )
                    row_list.append(row)
                    bc_metrics[(cat1, cat2, key[2], "Horizon Yr Benefit (2019$)")] = (
                        valuation * nominal_diff
                    )
        ...
        if self.base_dir:
            imp_years = self.writeCostsWorksheet(
                workbook, bc_metrics, scen_minus_baseline=False
            )
        ...
        if self.base_dir:
            if self.config.loc["Compare"] == "scenario-baseline":
                bc_metrics_annual_benefit = self.writeBenefitStreamsWorksheet(
                    workbook, bc_metrics, imp_years
                )
            else:
                bc_metrics_annual_benefit = self.writeBenefitStreamsWorksheet(
                    workbook, bc_metrics, imp_years, scen_minus_baseline=False
                )
        ...
        for key, value in colA.daily_results.iteritems():
            ...
            if self.base_dir:
                if "for reference only" not in key[0]:
                    if ((key[0], key[1]) in self.BENEFIT_VALUATION) or (
                        (key[0], key[1], key[2]) in self.BENEFIT_VALUATION
                    ):
                        worksheet.write(
                            row,
                            10,  # PV 2025-2060
                            "=benefit_streams!%s"
                            % bc_metrics_annual_benefit.get(
                                [(key[0], key[1], key[2], "NPV2025-60cell")][0]
                            ),
                            format_ann_ben,
                        )
                        worksheet.write(
                            ...
                            "=benefit_streams!%s"
                            % bc_metrics_annual_benefit.get(
                                [(key[0], key[1], key[2], "NPV2025-80cell")][0]
                            ),
                            format_ann_ben,
                        )
            row += 1
        ...
        # Equity Score
        if self.base_dir:
            population_lowinc = self.accessibilityMarkets.loc[
                self.accessibilityMarkets["incQ_label"] == "lowInc", "scen_num_persons"
            ].sum()
            population_medinc = self.accessibilityMarkets.loc[
                self.accessibilityMarkets["incQ_label"] == "medInc", "scen_num_persons"
            ].sum()
            population_highinc = self.accessibilityMarkets.loc[
                self.accessibilityMarkets["incQ_label"] == "highInc", "scen_num_persons"
            ].sum()
            population_veryhighinc = self.accessibilityMarkets.loc[
                self.accessibilityMarkets["incQ_label"] == "veryHighInc",
                "scen_num_persons",
            ].sum()
            ...
            worksheet.write(
                ...
                format_equity_headers,
            )
            worksheet.write(
                TABLE_HEADER_ROW - 3, 0, "Total Population", format_equity_headers
            )
            worksheet.write(
                TABLE_HEADER_ROW - 2,
                0,
                "Avg Accessibility Benefit per Individual",
                format_equity_headers,
            )

            worksheet.write(
                TABLE_HEADER_ROW - 4, 1, "LowInc", format_equity_headers_center
            )
            worksheet.write(
                TABLE_HEADER_ROW - 4, 2, "MedInc", format_equity_headers_center
            )
            worksheet.write(
                TABLE_HEADER_ROW - 4, 3, "HighInc", format_equity_headers_center
            )
            worksheet.write(
                TABLE_HEADER_ROW - 4, 4, "VeryHighInc", format_equity_headers_center
            )

            worksheet.write(
                TABLE_HEADER_ROW - 3,
                1,
                float(population_lowinc) / 1000000,
                format_equitypop,
            )
            worksheet.write(
                TABLE_HEADER_ROW - 3,
                2,
                float(population_medinc) / 1000000,
                format_equitypop,
            )
            worksheet.write(
                TABLE_HEADER_ROW - 3,
                3,
                float(population_highinc) / 1000000,
                format_equitypop,
            )
            worksheet.write(
                TABLE_HEADER_ROW - 3,
                4,
                float(population_veryhighinc) / 1000000,
                format_equitypop,
            )
            ...
            worksheet.write(
                ...
                1,
                "=(I20+I25) / (%s*1000000)"
                % xl_rowcol_to_cell(TABLE_HEADER_ROW - 3, 1),
                format_equityben,
            )
            worksheet.write(
                TABLE_HEADER_ROW - 2,
                2,
                "=(I21+I26) / (%s*1000000)"
                % xl_rowcol_to_cell(TABLE_HEADER_ROW - 3, 2),
                format_equityben,
            )
            worksheet.write(
                TABLE_HEADER_ROW - 2,
                3,
                "=(I22+I27) / (%s*1000000)"
                % xl_rowcol_to_cell(TABLE_HEADER_ROW - 3, 3),
                format_equityben,
            )
            worksheet.write(
                TABLE_HEADER_ROW - 2,
                4,
                "=(I23+I28) / (%s*1000000)"
                % xl_rowcol_to_cell(TABLE_HEADER_ROW - 3, 4),
                format_equityben,
            )

            worksheet.write(
                TABLE_HEADER_ROW - 5, 0, "Equity Score", format_bc_header_left
            )
            worksheet.write(
                TABLE_HEADER_ROW - 5,
                1,
                "=IF((MAX(%s,0)+MAX(%s,0)+MAX(%s,0)+MAX(%s,0))=0,\
        ...
        if self.base_dir:
            ...
            worksheet.write(
                TABLE_HEADER_ROW - 5, 8, "Horizon Year", format_bc_header_left
            )
            worksheet.write(
                TABLE_HEADER_ROW - 5, 10, "NPV 2025-60", format_bc_header_left
            )
            worksheet.write(
                TABLE_HEADER_ROW - 5, 12, "NPV 2025-80", format_bc_header_left
            )
            ...
            worksheet.write(
                TABLE_HEADER_ROW - 4,
                8,
                "=SUM(%s)" % str(",").join(sum_indices),
                format_bc_money,
            )
            worksheet.write(
                TABLE_HEADER_ROW - 4,
                10,
                "=SUM(%s)" % str(",").join(sum_indices_PV1),
                format_bc_money,
            )
            worksheet.write(
                TABLE_HEADER_ROW - 4,
                12,
                "=SUM(%s)" % str(",").join(sum_indices_PV2),
                format_bc_money,
            )

            # getting costs
            worksheet.write(
                TABLE_HEADER_ROW - 3, 8, "=cost_streams!E2/1000000", format_bc_money
            )
            worksheet.write(
                TABLE_HEADER_ROW - 3, 10, "=cost_streams!F2/1000000", format_bc_money
            )
            worksheet.write(
                TABLE_HEADER_ROW - 3, 12, "=cost_streams!G2/1000000", format_bc_money
            )
            ...
            for col in [8, 10, 12]:
                worksheet.write(
                    ...
                    "=%s/%s"
                    % (
                        xl_rowcol_to_cell(TABLE_HEADER_ROW - 4, col),
                        xl_rowcol_to_cell(TABLE_HEADER_ROW - 3, col),
                    ),
                    format_bc_ratio,
                )
        ...
        worksheet.insert_image(
            3,
            10,
            os.path.join(os.path.dirname(os.path.abspath(__file__)), "King-cobra.png"),
    ...
    def writeProxiesWorksheet(self, workbook):
        ...
        df_proxies = pd.read_excel(
            self.ppa_master_input, sheet_name="stream_proxies", header=0
        )
        df_proxies = df_proxies.loc[df_proxies["Future"] == self.config.loc["Future"]]
        df_proxies = df_proxies.loc[
            (df_proxies["Project Type"] == self.config.loc["Project Type"])
            | (df_proxies["Project Type"] == "All")
        ]
        df_proxies = df_proxies.drop(
            ["Future", "Project Type", "Base2015", "Base2030", "Base2050"], axis=1
        )
        ...
        worksheet.write(
            row,
            0,
            "This sheet represents the proxies obtained from baseline no-project scenarios and is used to extrapolate project benefit streams. These proxies are for:",
            format_basicstats,
        )
    ...
    def writeBenefitStreamsWorksheet(
        self, workbook, bc_metrics, imp_years, scen_minus_baseline=True
    ):
        """
        ...
        format_2060 = workbook.add_format(
            {
                "num_format": '_($* #,##0_);_($* (#,##0);_($* "-"??_);_(@_)',
                "bg_color": "#D9D9D9",
                "bold": True,
        ...
        format_npv = workbook.add_format(
            {
                "num_format": '_($* #,##0_);_($* (#,##0);_($* "-"??_);_(@_)',
                "bg_color": "#C5D9F1",
                "bold": True,
        ...
        worksheet.write(
            row,
            0,
            "This sheet forecasts benefit streams based on the Horizon Year 2050 value output by the model, and proxies at every 10 year interval sourced from intermediate year model runs for no-project scenario",
            format_basicstats,
        )
        for col in range(1, 8):
            worksheet.write(row, col, "", format_basicstats)
        ...
        for key, val in bc_metrics_annual_benefit.iteritems():
            ...
            worksheet.write(
                ...
                "=NPV(%s,%s:%s)"
                % (
                    xl_rowcol_to_cell(DISCOUNT_RATE_ROW, 1),
                    xl_rowcol_to_cell(row, 6),
                    xl_rowcol_to_cell(row, 41),
                ),
                format_npv,
            )
            worksheet.write(
            ...
            bc_metrics_withNPV[
                (key[0], key[1], key[2], "NPV2025-60cell")
            ] = xl_rowcol_to_cell(row, 4)
            bc_metrics_withNPV[
                (key[0], key[1], key[2], "NPV2025-80cell")
            ...
            while (
                col <= 61
            ):  # multiplying annual horizon year benefit times proxies from year 2025-2080
                worksheet.write(
                    row,
                    col,
                    "=%s*proxies!%s"
                    % (xl_rowcol_to_cell(row, 3), xl_rowcol_to_cell(row, col + 7)),
    ...
    def writeCostsWorksheet(self, workbook, bc_metrics, scen_minus_baseline=True):
        ...
        format_costs = workbook.add_format(
            {"num_format": '_($* #,##0_);_($* (#,##0);_($* "-"??_);_(@_)'}
        )
        format_2060 = workbook.add_format(
            {
        ...
        format_remainder = workbook.add_format(
            {
                "num_format": '_($* #,##0_);_($* (#,##0);_($* "-"??_);_(@_)',
                "bg_color": "#D9D9D9",
                "bold": True,
        ...
        format_basicstats = workbook.add_format({"bg_color": "#FFFFC0", "bold": True})
        format_total_num = workbook.add_format(
            {
                "bg_color": "#92D050",
                "bold": True,
                "num_format": '_($* #,##0_);_($* (#,##0);_($* "-"??_);_(@_)',
        ...
        format_total_num_M = workbook.add_format(
            {
                "bg_color": "#92D050",
                "bold": True,
                "num_format": '_(\$* #,##0"M"_);_(\$* (#,##0"M");_(\$* "-"??_);_(@_)',
        ...
        for key, val in self.proj_costs.iteritems():
            if key in self.asset_life.keys():  # if key is a capital cost
                ...
                worksheet.write(
                    row,
                    4,
                    "=%s/%s" % (xl_rowcol_to_cell(row, 3), (xl_rowcol_to_cell(row, 2))),
                    format_remainder,
                )
                i = 7
                while i < (7 + imp_years):
                    worksheet.write(
                        ...
                        % (
                            ...
                            xl_rowcol_to_cell(IMP_YEAR_ROW, 1),
                        ),
                        format_2060 if (i in [42, 62]) else format_costs,
                    )
                    i += 1
                while i <= 62:  # filling in empty cells with 0
                    worksheet.write(
                        row, i, 0, format_2060 if (i in [42, 62]) else format_costs
                    )
                    i += 1
                row += 1
                ...
                worksheet.write(row, 3, val, format_input)  # cost input
                worksheet.write(
                    row, 4, "=%s" % (xl_rowcol_to_cell(row, 3)), format_remainder
                )
                i = 7
                ...
                for j in range(i, 63):
                    worksheet.write(
                        row,
                        j,
                        "=%s" % (xl_rowcol_to_cell(row, 3)),
                        format_2060 if (j in [42, 62]) else format_costs,
                    )
                row += 1
            else:
                pass
        ...
        for key, val in self.proj_costs.iteritems():
            ...
            if key == "Road - Pavement":
                ...
                i = (
                    7 + imp_years + 4
                )  # Rehab costs: 10% of initial investment cost at year 5
                ...
                while i <= 62:  #              30% of initial investment cost at year 20
                    if j == 1:
                        worksheet.write(
                            row,
                            i,
                            "=%s*0.1" % (xl_rowcol_to_cell(row, 3)),
                            format_2060 if (i in [42, 62]) else format_costs,
                        )
                        col = i + 1
                        while (
                            col < (i + 5) and col <= 62
                        ):  # filling in empty cells with 0
                            worksheet.write(
                                row,
                                col,
                                0,
                                format_2060 if (col in [42, 62]) else format_costs,
                            )
                            col += 1
                        j = 2
                        i += 5
                ...
                i = (
                    7 + imp_years + 4
                )  # Rehab costs: 20% of initial investment cost at year 5
                ...
                key in self.asset_life.keys()
            ):  # if key is Capital costs apart from Road pavement
                asset_life_value = self.asset_life.get(key)
                ...
                i = 7
                while (
                    i < (7 + imp_years + asset_life_value - 1) and i <= 62
                ):  # filling in empty cells with 0
                    worksheet.write(
                        row, i, 0, format_2060 if (i in [42, 62]) else format_costs
                    )
                ...
                col_2080 = (
                    7 + imp_years + asset_life_value - 1
                )  # Replacement cost = initial cost every turnover of asset life
                col_2060 = col_2080
                i = 0
                while col_2080 <= 62:  # calculating replacement costs until 2080
                    worksheet.write(
                        ...
                        col_2080,
                        "=%s" % (xl_rowcol_to_cell(row, 3)),
                        format_2060 if (col_2080 in [42, 62]) else format_costs,
                    )
                    col = col_2080 + 1
                    while (
                        col < (col_2080 + asset_life_value) and col <= 62
                    ):  # filling in empty cells with 0
                        worksheet.write(
                            row,
                            col,
                ...
                while (
                    col_2060 <= 42
                ):  # to be able to calculate remaining asset value at 2060
        ...
        worksheet.write(
            ...
            "=sum(%s:%s)"
            % (
                xl_rowcol_to_cell(TABLE_HEADER_ROW + 1, 63),
                xl_rowcol_to_cell(row - 1, 63),
            ),
            format_remainder,
        )
        worksheet.write(
            ...
            "=sum(%s:%s)"
            % (
                xl_rowcol_to_cell(TABLE_HEADER_ROW + 1, 64),
                xl_rowcol_to_cell(row - 1, 64),
            ),
            format_remainder,
        )
        row += 1
        ...
        for row_num in range(TABLE_HEADER_ROW + 1, row - 1):
            worksheet.write(
                ...
                % (
                    xl_rowcol_to_cell(DISCOUNT_RATE_ROW, 1),
                    xl_rowcol_to_cell(row_num, 7),
                    xl_rowcol_to_cell(row_num, 42),
                ),
                format_totals,
            )
            worksheet.write(
        ...
        worksheet.write(
            ...
            5,
            "=%s/((1+%s)^35)"
            % (xl_rowcol_to_cell(row - 1, 63), xl_rowcol_to_cell(DISCOUNT_RATE_ROW, 1)),
            format_totals,
        )
        worksheet.write(
            row - 1,
            6,
            "=%s/((1+%s)^55)"
            % (xl_rowcol_to_cell(row - 1, 64), xl_rowcol_to_cell(DISCOUNT_RATE_ROW, 1)),
            format_totals,
        )
        ...
        worksheet.write(
            ...
            "=sum(%s:%s) - %s"
            % (
                xl_rowcol_to_cell(TABLE_HEADER_ROW + 1, 4),
                xl_rowcol_to_cell(row - 2, 4),
        ...
        worksheet.write(
            ...
            "=sum(%s:%s) - %s"
            % (
                xl_rowcol_to_cell(TABLE_HEADER_ROW + 1, 5),
                xl_rowcol_to_cell(row - 2, 5),
        ...
        worksheet.write(
            ...
            "=sum(%s:%s) - %s"
            % (
                xl_rowcol_to_cell(TABLE_HEADER_ROW + 1, 6),
                xl_rowcol_to_cell(row - 2, 6),
        ...
        worksheet.write(
            ...
            % (
                xl_rowcol_to_cell(TABLE_HEADER_ROW + 1, 4),
                xl_rowcol_to_cell(TABLE_HEADER_ROW + 20, 4),
            ),
            format_total_num,
        )
        worksheet.write(
            ...
            % (
                xl_rowcol_to_cell(TABLE_HEADER_ROW + 1, 5),
                xl_rowcol_to_cell(TABLE_HEADER_ROW + 20, 5),
            ),
            format_total_num,
        )
        worksheet.write(
            ...
            % (
                xl_rowcol_to_cell(TABLE_HEADER_ROW + 1, 6),
                xl_rowcol_to_cell(TABLE_HEADER_ROW + 20, 6),
            ),
            format_total_num,
        )
        ...
        # O&M Cost totals
        worksheet.write(
            3, 4, "=%s" % xl_rowcol_to_cell(TABLE_HEADER_ROW + 21, 4), format_total_num
        )
        worksheet.write(
            3, 5, "=%s" % xl_rowcol_to_cell(TABLE_HEADER_ROW + 21, 5), format_total_num
        )
        worksheet.write(
            3, 6, "=%s" % xl_rowcol_to_cell(TABLE_HEADER_ROW + 21, 6), format_total_num
        )
        ...
        worksheet.write(
            ...
            "=sum(%s:%s)"
            % (
                xl_rowcol_to_cell(TABLE_HEADER_ROW + 22, 5),
                xl_rowcol_to_cell(row - 2, 5),
            ),
            format_total_num,
        )
        worksheet.write(
            ...
            "=sum(%s:%s)"
            % (
                xl_rowcol_to_cell(TABLE_HEADER_ROW + 22, 6),
                xl_rowcol_to_cell(row - 2, 6),
            ),
            format_total_num,
        )
        ...
        worksheet.write(
            ...
            "=sum(%s:%s)"
            % (
                xl_rowcol_to_cell(TABLE_HEADER_ROW + 1, 3),
                xl_rowcol_to_cell(TABLE_HEADER_ROW + 20, 3),
            ),
            format_total_num,
        )
        worksheet.write(
            2, 13, "=%s" % xl_rowcol_to_cell(TABLE_HEADER_ROW + 21, 3), format_total_num
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--discount_rate", help="The discount rate.", type=float, default=0.03
    )
    parser.add_argument("project_dir", help="The directory with the run results csvs.")
    parser.add_argument(
        "all_projects_dir",
    ...
    if rr.base_dir:
        quicksummary_csv = os.path.join(
            os.getcwd(),
            args.all_projects_dir,
            "quicksummary_%s_base%s.csv"
            % (rr.config.loc["Project ID"], rr.config.loc["base_dir"]),
        )
    else:
        quicksummary_csv = os.path.join(
            os.getcwd(),
            args.all_projects_dir,
            "quicksummary_base%s.csv" % rr.config.loc["Project ID"],
        )
