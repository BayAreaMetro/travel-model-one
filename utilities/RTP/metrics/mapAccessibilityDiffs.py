...
def read_accessibilities(proj_dir, mandatory, col_prefix, include_output_dir):
    ...
    filename = os.path.join(
        proj_dir,
        "OUTPUT" if include_output_dir else "",
        "logsums",
        "mandatoryAccessibilities.csv"
        if mandatory
        else "nonMandatoryAccessibilities.csv",
    )
    ...
    acc_df.loc[
        acc_df["autoSuff_label"].str.endswith("_noAV"), "autoSuff_label"
    ] = acc_df["autoSuff_label"].str[:-5]
    acc_df.loc[acc_df["autoSuff_label"].str.endswith("_AV"), "hasAV"] = 1
    acc_df.loc[acc_df["autoSuff_label"].str.endswith("_AV"), "autoSuff_label"] = acc_df[
        "autoSuff_label"
    ].str[:-3]
    ...
    acc_df.rename(
        columns={0: "%s_dclogsum" % col_prefix, "subzone": "walk_subzone"}, inplace=True
    )
    return acc_df
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--include_output_dir",
        action="store_true",
        help="Assume metrics etc in OUTPUT subdir",
    )
    parser.add_argument(
        "run_dir", type=str, help="Directory with model run results OUPUT\metrics, etc"
    )
    parser.add_argument(
        "base_dir",
        type=str,
        help="Directory with base model run results OUPUT\metrics, etc",
    )

    my_args = parser.parse_args()

    # read the scenario and base accessibilities
    scen_mand_acc = read_accessibilities(
        my_args.run_dir, True, "scen", my_args.include_output_dir
    )
    scen_nonm_acc = read_accessibilities(
        my_args.run_dir, False, "scen", my_args.include_output_dir
    )
    base_mand_acc = read_accessibilities(
        my_args.base_dir, True, "base", my_args.include_output_dir
    )
    base_nonm_acc = read_accessibilities(
        my_args.base_dir, False, "base", my_args.include_output_dir
    )
    ...
    print(
        "scen_mand_acc length {} head\n{}".format(
            len(scen_mand_acc), scen_mand_acc.head()
        )
    )
    print(
        "scen_nonm_acc length {} head\n{}".format(
            len(scen_nonm_acc), scen_nonm_acc.head()
        )
    )
    print(
        "base_mand_acc length {} head\n{}".format(
            len(base_mand_acc), base_mand_acc.head()
        )
    )
    print(
        "base_nonm_acc length {} head\n{}".format(
            len(base_nonm_acc), base_nonm_acc.head()
        )
    )
    ...
    (
        ...
        mandatoryAccess,
        nonMandatoryAccess,
    ) = RunResults.RunResults.calculateConsumerSurplus(
        config,
        daily_results,
        scen_mand_acc,
        base_mand_acc,
        ...
        debug_dir=os.path.join(
            my_args.run_dir, "OUTPUT" if my_args.include_output_dir else "", "logsums"
        ),
    )
