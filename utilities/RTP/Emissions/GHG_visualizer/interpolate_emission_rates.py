...
InputEmissionRates_df = pd.read_excel(
    os.path.join(os.getcwd(), "yr2035_passveh_CO2_rates.xlsx"),
    header=1,
    index_col=1,
    skiprows=2,
    nrows=10,
...
for c in counties:
    for mph in range(1, max_speed):
        ...
        if mph >= 5 and mph < 70:
            Interpolated_df.loc[mph, c] = (
                InputEmissionRates_df.loc[c, ref_top]
                - InputEmissionRates_df.loc[c, ref_bottom]
            ) / 5 * (mph - quotient) + InputEmissionRates_df.loc[c, ref_bottom]
        ...
        if mph < 5:  # use 10 mph and 5 mph to extrapolate
            ...
            Interpolated_df.loc[mph, c] = (
                ...
                / 5
            ) * -1 * (5 - mph) + InputEmissionRates_df.loc[c, ref_bottom]
        ...
        if mph >= 70:  # use 65 mph and 70 mph to extrapolate
            ...
            Interpolated_df.loc[mph, c] = (
                ...
                / 5
            ) * (mph - 70) + InputEmissionRates_df.loc[c, ref_top]
