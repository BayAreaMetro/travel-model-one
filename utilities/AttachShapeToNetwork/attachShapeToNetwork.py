...
def readCubeNetwork(filename):
    ...
    for row in reader:
        ...
        if row_num < 6:
            logger.debug(
                "row %d: A = %5d (%f, %f)," % (row_num, a, nodes[a][0], nodes[a][1])
            )
            logger.debug("       B = %5d (%f, %f)" % (b, nodes[b][0], nodes[b][1]))
        linestrings.append(
            (
...
def readShapefile(filename, shp_fieldnames):
    ...
    logger.info(
        "Read %d shapes from %s, cleaned %d" % (len(shape_data), filename, cleaned)
    )
...
def joinCubeLinksToShapes(cube_linestrings, shapefile_data):
    ...
    for link_idx in range(len(cube_linestrings)):
        ...
        for idx in range(len(shapefile_data)):
            ...
            if linestring.bounds[0] > polygon.bounds[2]:
                continue  # min > max
            if linestring.bounds[1] > polygon.bounds[3]:
                continue
            if linestring.bounds[2] < polygon.bounds[0]:
                continue  # max < min
            if linestring.bounds[3] < polygon.bounds[1]:
                continue
        ...
        if maxintline_idx == -1:
            logger.warn(
                "No match found for linestring %5d - %5d"
...
def writeCubeNetworkWithNewCols(
    cubenet_infilename,
    cube_linestrings,
    line_to_shapeidx,
    shapefile_data,
    ...
    if tail.find(".") >= 0:
        tail = tail[: tail.find(".")]  # strip the suffix
    ...
    for link_idx, shape_idx in line_to_shapeidx.iteritems():
        # a,b,
        newcol_file.write(
            "%d,%d" % (cube_linestrings[link_idx][1], cube_linestrings[link_idx][2])
        )
        # remainder
        for sf_idx in range(len(shp_fieldnames)):
            # quote strings
    ...
    script_file = open(script_filename, "w")
    script_file.write(
        CUBE_JOINCOLS_SCRIPT
        % (cubenet_infilename, newcol_filename, varlist, cube_outfilename)
    )
    script_file.close()
    logger.info("Wrote %s" % script_filename)
...
if __name__ == "__main__":
    ...
    if len(SHAPE_FIELDS) != len(CUBE_FIELDS):
        logger.fatal(
            "Mismatching number of shape fileds (%s) and cube fields (%s)"
