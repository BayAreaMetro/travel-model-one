...
TOURMC_RE_TXT         = "(Utility Expressions for Tour Mode Choice Logsum calculation for ([ A-Za-z\-_]+) Location Choice   HH=(\d+), PersonNum=(\d+), PersonType=([ A-Za-z\-]+), destTaz=(\d+) destWalkSubzone=(\d+))"
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if match:
            ...
            ptype   = match.group(6)
            destTaz = int(match.group(7))
            destSubz= int(match.group(8))
            print("Found Usual Work Location Choice Tour Mode Choice logsum info for purpose={} hh={} persnum={} ptype={} destTaz={} destWalkSubzone={}".format(
                  purpose, hh, persnum, ptype, destTaz, destSubz))

            (new_lines_read,df) = read_tour_mode_choice_logsum_lines(log_fo, "UsualWorkLocChoice", purpose, hh, persnum, ptype, destTaz, destSubz, args.base_or_build, args.log_file)
        ...
        if match:
            ...
            print("Found Usual Work Location Choice info for purpose={} hh={} persnum={} ptype={} tournum={}".format(purpose, hh, persnum, ptype, tournum))

            # read the rest of the relevant lines
            (new_lines_read,output_dir) = read_destination_choice_lines(log_fo, "UsualWorkLocChoice", purpose, hh, persnum, ptype, tournum, args.base_or_build, args.log_file)
            lines_read += new_lines_read
