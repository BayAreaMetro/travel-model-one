"""
Build the crosswalk that maps FROM zones to TO zones with multiple weight columns.

Crosswalk output (data/interim/crosswalk.csv)
─────────────────────────────────────────────
  from_zone_id   – FROM zone ID (1-based)
  to_zone_id     – TO zone ID  (1-based; internal zone or gateway)
  area           – area-overlap weight  (always present)
  <weight_name>  – one column per entry in config weights.variables

Weight logic
────────────
For a FROM zone that overlaps TO zones j₁, j₂, j₃ …

  area weight:
      area[j] = internal_frac × overlap_area[j] / Σ_k overlap_area[k]

  variable weight (e.g. 'production'):
      production[j] = internal_frac × variable[j] / Σ_k variable[k]
      → pure ratio of TO-zone variable values; area does NOT enter the formula.
      If all overlapping TO zones have variable = 0, falls back to the area weight.

  internal_frac = Σ overlap_area / total FROM-zone area  (always area-based)
  external_frac = 1 − internal_frac → assigned to the nearest gateway in all columns.

Polygon zones (external — no overlap with TO region)
    All weight columns = 1.0, routed to nearest gateway.

TLN point zones (internal)
    All weight columns = 1.0, routed to the containing TO zone.

TLN point zones (external)
    All weight columns = 1.0, routed to nearest gateway.

Unknown zone IDs
    Absent from crosswalk; trips involving them are dropped during projection.

Run from project root:
    python -m src.data.build_crosswalk
"""
