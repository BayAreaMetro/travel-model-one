...
MC_TOTAL_UTILITY_RE = re.compile(
    "{}{}".format(DATE_LOG_TYPE_RE_TXT, MC_TOTAL_UTILITY_RE_TXT)
)


def read_tour_mode_choice_logsum_lines(
    file_object,
    type_str,
    purpose,
    hh,
...
def read_trip_mode_choice_lines(
    file_object,
    type_str,
    trip_mc_od,
    purpose,
    ...
    if purpose != "othmaint":
        print(
            "read_trip_mode_choice_lines supports othmaint purpose only currently; skipping {}".format(
                purpose
            )
        )
        return (0, 0)
...
def read_destination_choice_lines(
    file_object, type_str, purpose, hh, persnum, ptype, tournum, base_or_build, log_file
):
    """
    ...
    for utils_read in range(1, len(ROW_NAMES) + 1):
        ...
        for dest_alt in range(1, NUM_DEST_ALT + 1):
            ...
            row_alt_dict["dest subzone"] = int(
                dest_alt - (row_alt_dict["dest taz"] - 1) * NUM_SUBZONES - 1
            )
            row_alt_dict["coefficient"] = float(match.group(full_idx + 1))
            row_alt_dict["variable"] = match.group(full_idx + 2)
            if row_alt_dict["variable"] == "Infinity":
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "base_or_build", choices=["base", "build"], help="For output file name"
    )
    parser.add_argument(
        "log_file", metavar="event-node0-something.log", help="Log file to parse"
    )
    ...
    trip_mc_od_re = re.compile(
        "{}{}".format(DATE_LOG_TYPE_RE_TXT, TRIPMC_ORIGDEST_RE_TXT)
    )
    ...
    while True:
        ...
        if match:
            ...
            print(
                "Found Tour Mode Choice logsum info for purpose={} hh={} persnum={} ptype={} destTaz={} destWalkSubzone={}".format(
                    purpose, hh, persnum, ptype, destTaz, destSubz
                )
            )

            (new_lines_read, df) = read_tour_mode_choice_logsum_lines(
                log_fo,
                type_str,
                purpose,
                hh,
        ...
        if match:
            ...
            print(
                "Found {} Destination Choice info for purpose={} hh={} persnum={} ptype={} tournum={}".format(
                    dctype, purpose, hh, persnum, ptype, tournum
                )
            )
            ...
            (new_lines_read, output_dir) = read_destination_choice_lines(
                log_fo,
                type_str,
                purpose,
                hh,
        ...
        if match:
            ...
            print(
                "Found Trip Mode Choice info for hh={} persnum={} ptype=[{}] tour_purpose={} tour_id={} stopdest_purpose={} stop_id={}".format(
                    hh, persnum, ptype, tour_purpose, tour_id, stopdest_purpose, stop_id
                )
            )

            (new_lines_read, df) = read_trip_mode_choice_lines(
                log_fo,
                type_str,
                trip_mc_od,
                tour_purpose,
