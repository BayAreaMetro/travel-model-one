 ...
 */
public class CompressedMessagePool {

    private CompressedMessage[] objects;
    private int head;


    public CompressedMessagePool(int size) {
        if (size <= 0)
            size = 1;

        head = 0;
        objects = new CompressedMessage[size];
    }
    ...
    public CompressedMessage getMessage() {
        ...
        synchronized( this ) {
            ...
            if (head > 0) {
                ...
                msg = new CompressedMessage("new_from_pool");
            }
        }
        return msg;
    }

    public void returnMessage(CompressedMessage msg) {
        synchronized( this ) {
    ...
    private synchronized void expandPool() {
        ...
        CompressedMessage[] newObjectPool;

        //Allocate a new array of objects
        newObjectPool = new CompressedMessage[objects.length * 2];
