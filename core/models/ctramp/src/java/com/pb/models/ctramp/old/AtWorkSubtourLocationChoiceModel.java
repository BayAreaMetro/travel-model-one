...
public class AtWorkSubtourLocationChoiceModel {

    public static Logger logger = Logger.getLogger(AtWorkSubtourLocationChoiceModel.class);

    public static final String PROPERTIES_DC_SOA_SAMPLE_SIZE        = "AtWorkSubtourLocationChoice.SampleOfAlternatives.SampleSize";
    ...
    public AtWorkSubtourLocationChoiceModel(ResourceBundle resourceBundle, MatrixDataServerIf ms, ModelStructure modelStructure, TazDataIf tazDataManager, CtrampDmuFactoryIf dmuFactory ){
        ...
        int purposeIndexOffset = modelStructure.getDcSizeArrayCategoryIndexOffset( ModelStructure.AT_WORK_CATEGORY );
        int end = purposeIndexOffset + modelStructure.getNumDcSizeArrayCategorySegments( ModelStructure.AT_WORK_CATEGORY );

        int index = 0;
        for ( int i=purposeIndexOffset; i < end; i++ ) {
            int underscoreIndex = dcSizeArrayPurposes[i].indexOf('_');
            if ( underscoreIndex > 0 ) {
                String purposeString = dcSizeArrayPurposes[i].substring(0,underscoreIndex);
                String segmentString = dcSizeArrayPurposes[i].substring(underscoreIndex+1);
                if ( purposeString.equalsIgnoreCase( segmentString ) )
                    purposeIndexToNameMap.put( index, purposeString );
                else
                    purposeIndexToNameMap.put( index, purposeString + "_" + segmentString );
            }
            else {
                purposeIndexToNameMap.put( index, dcSizeArrayPurposes[i] );
            }
        }
    ...
    public void runAtWorkSubtourLocationChoiceModel (HouseholdDataManagerIf householdDataManager) {
    ...
    private void runUnConstrainedLocationChoiceModel (HouseholdDataManagerIf householdDataManager) {
        ...
        // get the list of purpose names by which size terms for this location choice model are segmented
        String[] dcSizeArrayPurposeList = new String[purposeIndexToNameMap.size()];
        for ( int i=0; i < dcSizeArrayPurposeList.length; i++ )
            dcSizeArrayPurposeList[i] = purposeIndexToNameMap.get(i);



        String[] modelPurposeList = modelStructure.getDcModelPurposeList( ModelStructure.AT_WORK_CATEGORY );
        
        double[][][] finalModeledDestChoiceLocationsByDestZone = new double[modelPurposeList.length][][];
        for( int i=0; i < modelPurposeList.length; i++ ) {
        ...
        dcSizeObj.setupDestChoiceSize( resourceBundle, projectDirectory, ModelStructure.AT_WORK_CATEGORY );
	    ...
	    int[][][] originLocationsByWorkZone = new int[modelPurposeList.length][][];

	    // loop through the purposes and extract out subjects
	    for(int i=0;i<modelPurposeList.length;++i){
		    String purposeString = modelPurposeList[i];
		    originLocationsByWorkZone[i] = householdDataManager.getAtWorkSubtoursByWorkZoneSubZone( purposeString );
	    }

	    // balance the size variables
        int start = modelStructure.getDcSizeArrayCategoryIndexOffset( ModelStructure.AT_WORK_CATEGORY );
        int end = start + modelStructure.getNumDcSizeArrayCategorySegments( ModelStructure.AT_WORK_CATEGORY );
	    dcSizeObj.balanceSizeVariables(originLocationsByWorkZone, modelPurposeList, start, end);

	    
        HashMap<String, String> propertyMap = ResourceUtil.changeResourceBundleIntoHashMap(resourceBundle);
        DestChoiceModelFactory modelFactory = DestChoiceModelFactory.getDestChoiceModelFactoryInstance( propertyMap, ms, modelStructure, ModelStructure.AT_WORK_CATEGORY, tazDataManager, dcSizeObj, uecFileName, soaUecFileName, soaSampleSize, modeChoiceUecFileName, dmuFactory );         
        ...
        dcSizeObj.reportMaxDiff( -1, finalModeledDestChoiceLocationsByDestZone, modelPurposeList );
        dcSizeObj.updateSizeVariables( modelPurposeList, start, end );
