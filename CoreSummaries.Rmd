...
indiv_tours     <- read.table(file=paste0("indivTourData_",ITER,".csv"), header=TRUE, sep=",")
...
add_cost <- function(timeCode_val, tours, reverse_od=FALSE) {
  ...
  costSkims   <- read.table(file = paste0("CostSkimsDatabase",TIMECODE_LABELS[timeCode_val],".csv"), header=TRUE, sep=",")
  ...
  costSkims   <- mutate(costSkims,
                        ...
                        da       = ifelse(da    == -999,NA,da    ),
                        daToll   = ifelse(daToll== -999,NA,daToll),
                        s2       = ifelse(s2    == -999,NA,s2    ),
                        s2Toll   = ifelse(s2Toll== -999,NA,s2Toll),
                        s3       = ifelse(s3    == -999,NA,s3    ),
                        s3Toll   = ifelse(s3Toll== -999,NA,s3Toll),
                        wTrnW    = ifelse(wTrnW == -999,NA,wTrnW ),
                        dTrnW    = ifelse(dTrnW == -999,NA,dTrnW ),
                        wTrnD    = ifelse(wTrnD == -999,NA,wTrnD ))
  ...
  tours <- mutate(tours,
                  cost2=((timeCode==timeCode_val)*(costMode==1)*da    ) +
                        ...
                        ((timeCode==timeCode_val)*(costMode==10)*dTrnW)
                  )
  ...
  tours <- select(tours, -da, -daToll, -s2, -s2Toll, -s3, -s3Toll, -wTrnW, -dTrnW, -wTrnD, -cost2) # rid new variables
}
```
...
add_distance <- function(timeCode_val, tours) {
  ...
  distSkims   <- read.table(file = paste0("DistanceSkimsDatabase",TIMECODE_LABELS[timeCode_val],".csv"), header=TRUE, sep=",")
  # rename columns for join -- reversing if needed (return trip)
...
add_active <- function(timeCode_val, trips) {
  ...
  activeSkims <- read.table(file = paste0("ActiveTimeSkimsDatabase",TIMECODE_LABELS[timeCode_val],".csv"), header=TRUE, sep=",")
...
commute_tours   <- select(indiv_tours, hh_id, person_id, orig_taz, dest_taz, tour_purpose, start_hour, end_hour, tour_mode)
...
commute_tours   <- mutate(commute_tours, timeCode=timeCodeHw, cost=0.0)
...
# Get the superdistrict, parking cost from tazdata
commute_tours   <- left_join(commute_tours,
                             mutate(tazData, dest_taz=taz) %.% select(dest_taz, SD, PRKCST),
                             by=c("dest_taz"))
...
# summarise
commute_summary <- summarise(group_by(commute_tours, SD, tour_mode), 
                             freq         = n(),
                             totCost      = mean(totCost),
                             cost         = mean(cost),
                             parking_cost = mean(parking_cost),
                             distance     = mean(distance))
write.table(commute_summary, "summary/CommuteByEmploymentLocation.csv", sep=",", row.names=FALSE)

# TODO: something is going wrong.  Start with
# commute_tours[(commute_tours$SD==26)&(commute_tours$tour_mode==2),]
# It doesn't match the xlsx
```
