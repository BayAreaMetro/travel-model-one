...
# append person type, activity_pattern, value_of_time from persons
trips_by_person <- left_join(trips_dcast, select(persons, hh_id, person_id, ptype, 
                                                 activity_pattern, value_of_time), by=c("hh_id","person_id"))
...
trips_by_person <- mutate(trips_by_person, 
                          zeroAuto=(autos==0),
                          atHomeA=1.0*(activity_pattern=='H'))
# todo: understand "atHomeA" variable.  It looks like active is not set... But what about auto trips?
...
write.table(active_summary, "active_summary.csv", sep=",")
