...
indiv_tours     <- read.table(file=file.path(TARGET_DIR,"modelfiles",paste0("indivTourData_",ITER,".csv")), 
                              header=TRUE, sep=",")
indiv_tours     <- tbl_df(indiv_tours)

# add income from household table
indiv_tours     <- left_join(indiv_tours, select(households, hh_id, incQ, incQ_label), by=c("hh_id"))
```
...
joint_trips     <- read.table(file=file.path(TARGET_DIR,"modelfiles",
                                             paste0("jointTripData_",ITER,".csv")), header=TRUE, sep=",")
...
joint_tours    <- read.table(file=file.path(TARGET_DIR,"modelfiles",
                                            paste0("jointTourData_",ITER,".csv")), header=TRUE, sep=",")
...
joint_person_trips <- select(joint_person_trips, hh_id, person_id, tour_id, orig_taz, dest_taz, trip_mode,
                             tour_purpose, orig_purpose, dest_purpose, depart_hour)
# cleanup
...
joint_person_trips <- mutate(joint_person_trips, joint=TRUE)
indiv_trips        <- mutate(indiv_trips,        joint=FALSE)
...
rownames(aos_wc_table)        <- LOOKUP_WALK_SUBZONE$walk_subzone_label
...
commute_tours   <- select(indiv_tours, hh_id, person_id, orig_taz, dest_taz, tour_purpose,
                          start_hour, end_hour, tour_mode, incQ, incQ_label)
...
commute_tours   <- left_join(commute_tours, 
                             select(mutate(LOOKUP_TIMEPERIOD, timeCodeHwNum=timeCodeNum, timeCodeHw=timeperiod_abbrev), 
                                    timeCodeHwNum, timeCodeHw))
commute_tours   <- left_join(commute_tours, 
                             select(mutate(LOOKUP_TIMEPERIOD, timeCodeWhNum=timeCodeNum, timeCodeWh=timeperiod_abbrev), 
...
commute_tours   <- left_join(commute_tours,
                             mutate(tazData, dest_taz=taz) %.% select(dest_taz, COUNTY, county_name, SD, PRKCST),
                             by=c("dest_taz"))
...
# summarise
commute_summary <- summarise(group_by(commute_tours, COUNTY, county_name, SD, tour_mode), 
                             freq         = n(),
...
commute_inc_summary_byjob <- summarise(group_by(commute_tours, COUNTY, county_name, SD, dest_taz, tour_mode, incQ, incQ_label), 
                                       freq         = n(),
                                       totCost      = mean(totCost, na.rm=TRUE),
                                       cost         = mean(cost,    na.rm=TRUE),
                                       parking_cost = mean(parking_cost),
...
commute_tours   <- left_join(commute_tours,
                             mutate(tazData, orig_taz=taz, res_COUNTY=COUNTY, res_county_name=county_name, res_SD=SD) %.% 
                               select(orig_taz, res_COUNTY, res_county_name, res_SD),
                             by=c("orig_taz"))
commute_inc_summary_byres <- summarise(group_by(commute_tours, res_COUNTY, res_county_name, res_SD, orig_taz, tour_mode, incQ, incQ_label), 
                                       freq         = n(),
                                       totCost      = mean(totCost, na.rm=TRUE),
                                       cost         = mean(cost,    na.rm=TRUE),
                                       parking_cost = mean(parking_cost),
...
tripdist_summary          <- summarise(group_by(trips, autoSuff, autoSuff_label, incQ, incQ_label, timeCode, timeperiod_label, trip_mode, tour_purpose),
                                       freq         = n(),
                                       distance     = mean(distance))
